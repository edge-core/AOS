...
netsnmp_udp_fmtaddr(netsnmp_transport *t, void *data, int len)
{
	...
	char tmp[64];
        ...
        #else
        L_INET_Ntoa( to->sin_addr.s_addr, (UI8_T *)tmp);
        #endif
        return strdup(tmp);
    }
}
...
netsnmp_udp_transport(struct sockaddr_in *addr, int local)
{
    ...
    if (local) {
        ...
        memcpy(t->local + 4, (u_char *) & (addr->sin_port), 2);
        t->local_length = 6;
        ...
        memcpy(t->remote + 4, (u_char *) & (addr->sin_port), 2);
        t->remote_length = 6;
...
netsnmp_udp_transport(struct sockaddr_in *addr, int local)
{
    ...
    t->domain_length = netsnmpUDPDomain_len;
       ...
       {
           UI32_T socket_id_temp;
           SNMP_TASK_Get_Socket_ID((I32_T *)&socket_id_temp);
           socket_id=(int)socket_id_temp;
       }
...
SNMPUDPDOMAIN_netsnmp_udp_transport(struct sockaddr_in *addr, int local)
{
    ...
    t->domain = netsnmpUDPDomain;
    t->domain_length = netsnmpUDPDomain_len;
    /* kinghong, 2004/4/29, we already create 5 socket for inform/trap use, we
     * get from snmp_mgr to use the free socket number
     */
    t->sock = SNMP_MGR_GetAvailableInformSocket();
    if (t->sock < 0)
    {
        netsnmp_transport_free(t);
        return NULL;
...
/*2004/4/29 kinghong this function is junk after inform request is import*/
void SNMPUDPDOMAIN_CreateTrapSocket(void)
{
    ...
    if (rc >= 1)
    {
    	return; /*socket already open, don't need to open anymore*/
	}

    /* BODY */

    if ((snmpudpdomain_trap_socket = socket(PF_INET, SOCK_DGRAM, 0)) < 0)
    {
        return;
    } /* End of if */
                    ...
                    const char *inpeername, int remote_port)
{
    ...
    if (inpeername != NULL) {
        ...
        #ifdef VXWORKS
            L_INET_Aton((UI8_T *)peername, &ip_address);
       #endif
...
netsnmp_udp_parse_security(const char *token, char *param, int access_right)
{
   ...
   /* our mercury platform have no EXAMPLE_COMMUNITY and no restrict to it, kinghong*/
    ...
    if ((strcmp(source, "default") == 0)
        ...
        L_INET_Aton((UI8_T *)source, &network);
      //  network = inet_addr(source);
    ...
    if (strmask == NULL || *strmask == '\0') {
        ...
        if (strchr(strmask, '.')) {
             ...
             * Try to interpret mask as a dotted quad.
             */
            L_INET_Aton((UI8_T *)strmask, &mask);
            //mask = inet_addr(strmask);
            if (mask == (unsigned long) -1 &&
                strncmp(strmask, "255.255.255.255", 15) != 0) {
                config_perror("bad mask");
...
{
    register_app_config_handler("com2sec", (void (*)(const char *, char *))netsnmp_udp_parse_security,
...
netsnmp_udp_create_ostring(const u_char * o, size_t o_len, int local)
{
    ...
    if (o_len == 6) {
        ...
        #endif
        memcpy((u_char *) & (addr.sin_port), o + 4, 2);
        return netsnmp_udp_transport(&addr, local);
    }
    return NULL;
}
