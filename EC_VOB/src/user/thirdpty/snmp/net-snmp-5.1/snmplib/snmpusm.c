...
oid             usm3DESPrivProtocol[10]  = { 1, 3, 6, 1, 6, 3, 10, 1, 2, 3 };
 ...
 *	-1	Otherwise.
 *
 *	Determine the initialization vector for the 3DES-CBC encryption.
 *	http://tools.ietf.org/html/draft-reeder-snmpv3-usm-3desede-00#section-5.1.1.1.2
 *
 *	iv is defined as the concatenation of engineBoots and the
 *		salt integer.
 *	The salt integer is incremented.
 ...
 */
int
usm_set_3dessalt(const oid * authtype,
                 unsigned long authtypelen,
                 u_char * iv,
                 size_t * iv_length,
                 u_char * priv_salt, 
                 ...
                 u_char * msgSalt)
{
    size_t  propersize_salt = BYTESIZE(USM_3DES_SALT_LENGTH) / 3;
    int     net_boots;
    int     net_salt_int;
    /*
     * net_* should be encoded in network byte order.  XXX  Why?
     */
    int     iindex;
    u_char  iv_buf[priv_salt_length];
    ...
    memcpy(iv, &net_boots, propersize_salt / 2);
    memcpy(iv + (propersize_salt / 2), &net_salt_int, propersize_salt / 2);
    memcpy(iv, iv_buf, propersize_salt);
...
{                               /* IN/OUT - Len available, len returned. */
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV) {
...
#endif
        else if (ISTRANSFORM(thePrivProtocol, 3DESPriv)) {
            if (!thePrivKey ||
                (usm_set_3dessalt(theAuthProtocol,
                                  theAuthProtocolLength,
                                  salt, 
                                  &salt_length,
                                  thePrivKey + 24, 
                                  ...
                                  &ptr[privParamsOffset]) == -1)) 
            {
                DEBUGMSGTL(("usm", "Can't set 3DES-CBC salt.\n"));
                usm_free_usmStateReference(secStateRef);
                return SNMPERR_USM_GENERICERROR;
            }
        }
        ...
        if (sc_encrypt(thePrivProtocol, thePrivProtocolLength,
                       ...
                       &ptr[dataOffset], (unsigned long *)&encrypted_length)
            != SNMP_ERR_NOERROR) {
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHNOPRIV
        ...
        if (sc_generate_keyed_hash(theAuthProtocol, theAuthProtocolLength,
                                   ...
                                   ptr, ptr_len, temp_sig, (unsigned long *)&temp_sig_len)
            != SNMP_ERR_NOERROR) {
            /*
    ...
    )
{
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHPRIV) {
        ...
        if (ISTRANSFORM(thePrivProtocol, AES128Priv) ||
            ...
            salt_length = BYTESIZE(USM_3DES_SALT_LENGTH) / 3;
            save_salt_length = BYTESIZE(USM_3DES_SALT_LENGTH) / 3;
            save_salt_offset = 0;
            if (!thePrivKey ||
                (usm_set_3dessalt(theAuthProtocol,
                                  theAuthProtocolLength,
        ...
        if (sc_encrypt(thePrivProtocol, thePrivProtocolLength,
                       ...
                       salt, salt_length,
                       scopedPdu, scopedPduLen,
                       ciphertext, (unsigned long *)&ciphertextlen) != SNMP_ERR_NOERROR) {
            DEBUGMSGTL(("usm", "DES-CBC error.\n"));
            usm_free_usmStateReference(secStateRef);
            SNMP_FREE(ciphertext);
            return SNMPERR_USM_ENCRYPTIONERROR;
    ...
    *offset += globalDataLen;
    ...
    if (theSecLevel == SNMP_SEC_LEVEL_AUTHNOPRIV ||
        ...
        if (sc_generate_keyed_hash(theAuthProtocol, theAuthProtocolLength,
                                   ...
                                   temp_sig, (unsigned long *)&temp_sig_len)
            != SNMP_ERR_NOERROR) {
            SNMP_FREE(temp_sig);
            DEBUGMSGTL(("usm", "Signing failed.\n"));
            usm_free_usmStateReference(secStateRef);
...
{                               /* IN     - v3 Message flags.              */
    ...
    if (secLevel == SNMP_SEC_LEVEL_AUTHPRIV) {
        ...
        if (ISTRANSFORM(user->privProtocol, 3DESPriv)) {
            /*
             * From RFC2574:
             *
             * "Before decryption, the encrypted data length is verified.
             ...
             * of the priv_key to obtain the IV.
             */
            iv_length = BYTESIZE(USM_3DES_SALT_LENGTH);
            for (i = 0; i < (int) (iv_length / 3); i ++)
            {
                iv[i] = salt[i] ^ user->privKey[iv_length + i];
            }
            iv_length = iv_length / 3;
        }
#ifdef HAVE_AES
                     ...
                     void *clientarg)
{
    ...
    if (sc_random((u_char *) & salt_integer, (unsigned long *)&salt_integer_len) !=
        SNMPERR_SUCCESS) {
    ...
    noNameUser = usm_create_initial_user("", usmHMACMD5AuthProtocol,
                                         ...
                                         USM_LENGTH_OID_TRANSFORM);
   #ifdef VXWORKS
    /* kinghong patched to prevent crashed if created fail*/
    if (noNameUser)
    {
   #endif
...
/* kinghong added the below function to get a user enginedID*/
int usm_get_engineID_from_user_name(u_char* user_name, u_char* enginedID, size_t *engineIDLen)
{
    struct usmUser *ptr;

    for (ptr = userList; ptr != NULL; ptr = ptr->next)
    {
        if (strcmp(ptr->name, (char *)user_name)==0)
        {
             memcpy(enginedID, ptr->engineID, ptr->engineIDLen);
             *engineIDLen = ptr->engineIDLen;
             return TRUE;
        }
    }
    return FALSE;
                       ...
                       int use_default)
{
    ...
    char            noName[] = "";
#if(SYS_CPNT_CLUSTER==TRUE)
    UI8_T  out_name[SYS_ADPT_MAX_COMM_STR_NAME_LEN+1];
    if (name == NULL)
        name = noName;
    {

        UI32_T member_id;
        /*eli, commader use parsed name which @swXX is removed to search usr table for v3 accessing.()
          */
        if(SNMP_CLUSTER_GetClusterRole()!=CLUSTER_TYPE_COMMANDER||SNMP_CLUSTER_NameParseToMemberId(name,out_name,&member_id)!=TRUE)
        {
...
usm_free_user(struct usmUser *user)
{
...
#if 0
    /* Leo: if the userList is empty, it should return to NULL */
    if (userList == user)
    {
        userList = NULL;
    }
    ...
    #ifdef VXWORKS
     SNMP_MGR_AdjustSnmpV3UserCounter(FALSE);
    #endif
...
usm_create_user(void)
{
     ...
     */
    #ifdef VXWORKS
    SNMP_MGR_GetSnmpV3UserCounter(& snmpv3_user_counter);


    /*kinghong added*/
    if (snmpv3_user_counter >=SNMP_MGR_MAX_NBR_OF_SNMPV3_USER+1) /* There is 1 noname user in the system*/
    ...
    if (newUser == NULL)
        return NULL;
    #ifdef VXWORKS
    SNMP_MGR_AdjustSnmpV3UserCounter(TRUE);
    #endif
    /*
