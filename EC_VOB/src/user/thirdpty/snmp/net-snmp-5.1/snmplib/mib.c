 ...
 * mib.c
 *
 * $Id: mib.c,v 5.47 2003/11/02 12:50:39 nba Exp $
 *
...
{
            UI32_T temp_result;
            SNMP_MGR_ReadIpFromCompl(oidIndex, 0, &temp_result);
            uitmp=(int)temp_result;
}
                     ...
                     oid * objid, size_t objidlen, int width)
{
    ...
    if (!snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)tp->label) ||
        !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)cp) ||
    ...
    while (objidlen > 1) {
        for (; subtree; subtree = subtree->next_peer) {
            if (*objid == subtree->subid) {
                ...
                if (!snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)tmpbuf))
                    return 0;
                pos += len;
                objid++;
                objidlen--;
                     ...
                     struct tree *tp, int width)
{
    ...
    if (tp) {
        ...
        if (!snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)"  -- FROM\t") ||
            !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)str))
            return 0;
        pos = 16+strlen(str);
        ...
        if (tp->tc_index != -1) {
            if (!snmp_strcat(buf, buf_len, out_len, allow_realloc,
                             (const u_char *)"  -- TEXTUAL CONVENTION ") ||
                !snmp_strcat(buf, buf_len, out_len, allow_realloc,
                             (const u_char *)get_tc_descriptor(tp->tc_index)) ||
                !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)"\n"))
                return 0;
        }
        switch (tp->type) {
        ...
        if (tp->hint)
            if (!snmp_strcat(buf, buf_len, out_len, allow_realloc,
                             (const u_char *)"  DISPLAY-HINT\t\"") ||
                !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)tp->hint) ||
                !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)"\"\n"))
                return 0;
        if (tp->units)
            if (!snmp_strcat(buf, buf_len, out_len, allow_realloc,
        ...
        if (tp->augments)
            if (!snmp_strcat(buf, buf_len, out_len, allow_realloc,
                             (const u_char *)"  AUGMENTS\t{ ") ||
                !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)tp->augments) ||
                !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)" }\n"))
                return 0;
        if (tp->indexes) {
        ...
        if (tp->varbinds) {
            ...
            if (tp->type == TYPE_TRAPTYPE) {
                if (!snmp_strcat(buf, buf_len, out_len, allow_realloc,
                    (const u_char *)"  VARIABLES\t{ "))
                    return 0;
            } else {
                if (!snmp_strcat(buf, buf_len, out_len, allow_realloc,
                    (const u_char *)"  OBJECTS\t{ "))
                    return 0;
            }
            pos = 16 + 2;
            while (vp) {
        ...
        if (tp->description)
            if (!snmp_strcat(buf, buf_len, out_len, allow_realloc,
                ...
                !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)tp->description) ||
                !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)"\"\n"))
                return 0;
        if (tp->defaultValue)
            if (!snmp_strcat(buf, buf_len, out_len, allow_realloc,
                             (const u_char *)"  DEFVAL\t{ ") ||
                !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)tp->defaultValue) ||
                !snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)" }\n"))
                return 0;
    } else
        if (!snmp_strcat(buf, buf_len, out_len, allow_realloc, (const u_char *)"No description\n"))
            return 0;
    return 1;
}
