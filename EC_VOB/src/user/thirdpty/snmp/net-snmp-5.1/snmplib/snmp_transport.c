...
#ifdef VXWORKS
/* kinghong add the below API*/
netsnmp_transport *
SNMP_TRANSPORT_netsnmp_tdomain_transport_oid(const oid * dom,
                              size_t dom_len,
    ...
    for (d = domain_list; d != NULL; d = d->next) 
    {
        for (i = 0; d->prefix[i] != NULL; i++) 
        {
            if (netsnmp_oid_equals(dom, dom_len, d->name, d->name_length) == 0) 
            {
               if (6 == o_len)
               {
                    return SNMPUDPDOMAIN_netsnmp_udp_create_ostring(o, o_len, local);
               }
               else if ((18 == o_len) || (22 == o_len))
               {
                    return SNMPUDPDOMAIN_netsnmp_udpipv6_create_ostring(o,o_len,local);
               }
            }
        }
    }
