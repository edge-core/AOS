...
#if 0 /* kinghongv3trap*/
extern u_char *engineID;
extern u_int   engineIDLength;
#endif
                      ...
                      netsnmp_session * in_session)
{
    ...
    if (session->version == SNMP_VERSION_3) {
        ...
        /* kinghong patched, we create the remote engineID and the corresponding remote user automatically
         * when the securityLevel is NOAUTH, otherwise, don't create here, leave it create by user.
         */
        if (slp->session->securityLevel == SNMP_SEC_LEVEL_NOAUTH)
        {
            SNMP_MGR_SnmpV3UserEntry_T ruser_entry;
            SNMP_MGR_SnmpRemoteEngineID_T rengine_entry;

            memcpy(&rengine_entry.snmp_remote_engineID_host, slp->transport->remote,4);
            memcpy(rengine_entry.snmp_remote_engineID, slp->session->securityEngineID, slp->session->securityEngineIDLen);
            rengine_entry.snmp_remote_engineIDLen = slp->session->securityEngineIDLen;
            SNMP_MGR_CreateRemoteEngineID(&rengine_entry);

            strcpy( ruser_entry.snmpv3_user_name, slp->session->securityName);
            strcpy(ruser_entry.snmpv3_user_group_name, slp->session->securityName);
            strcpy(ruser_entry.snmpv3_user_auth_password, "");
            strcpy(ruser_entry.snmpv3_user_priv_password, "");
            ruser_entry.snmpv3_user_security_model = SNMP_MGR_SNMPV3_MODEL_V3;
            ruser_entry.snmpv3_user_security_level=VAL_vacmAccessSecurityLevel_noAuthNoPriv;
            ruser_entry.snmpv3_user_auth_type = SNMP_MGR_SNMPV3_AUTHTYPE_NONE;
            ruser_entry.snmpv3_user_priv_type = SNMP_MGR_SNMPV3_PRIVTYPE_NONE;
            ruser_entry.snmpv3_user_storage_type = VAL_usmUserStorageType_volatile;
            ruser_entry.password_from_config = FALSE;
            SNMP_MGR_CreateSnmpRemoteUser(rengine_entry.snmp_remote_engineID_host, &ruser_entry);
        }
        #if 0
 ...
 */
int
snmp_sess_close_sess_only(void *sessp)
{
    struct session_list *slp = (struct session_list *) sessp;
    netsnmp_transport *transport;
    struct snmp_internal_session *isp;
    ...
    SNMP_MGR_Marked_Inform_Request_Socket(slp->transport->sock,  FALSE);
    ...
    if (transport) {
        /* this function don't close the socket*/
                                              ...
                                              size_t))
{
    ...
    if (slp->session->version == SNMP_VERSION_3)
    {
        DEBUGMSGTL(("snmp_sess_add",
         ...
         if (slp->session->flags & SNMP_FLAGS_DONT_PROBE)
         {
             UI8_T engineID[MAXSIZE_snmpEngineID];
             UI32_T engineID_length;
    	    /* setup the engineID, boots and time
    	     * as the authorative engine
    	     */
    	     SNMP_MGR_GetEngineID( engineID, &engineID_length);
    	     snmpv3_clone_engineID(&slp->session->securityEngineID,
    				  &slp->session->securityEngineIDLen,
    				  engineID, engineID_length);
    	     get_enginetime(slp->session->securityEngineID,
    			   slp->session->securityEngineIDLen,
    			   &slp->session->engineBoots,
    			   &slp->session->engineTime, FALSE);
	     } /* end */
	     else
	     /* kinghong 2004/4/28, for inform request, remote agent is the authorative
	      * engine we should get the engineID from the (remote) User
	      */
	      /*We probe the EngineID first*/
         if (!snmpv3_engineID_probe(slp, in_session))
         {
            /* Get if we already set the remote EngineID*/

              //UI8_T engineID[MAXSIZE_snmpEngineID];
              //UI32_T engineID_length;
              SNMP_MGR_SnmpRemoteEngineID_T remote_engine_entry;

	          /*Step 1 Get the EngineID, User information from remote EngineID Entry*/
	          memcpy(&remote_engine_entry.snmp_remote_engineID_host, slp->transport->remote, 4);

	          if (SNMP_MGR_GetSnmpRemoteEngineIDEntry (&remote_engine_entry)!= SNMP_MGR_ERROR_OK)
	          {
	              snmp_sess_close_sess_only(slp);
                  slp = NULL;
                  return (void *) slp;
	          }
	          ...
	          snmpv3_clone_engineID(&slp->session->securityEngineID,
    		                        &slp->session->securityEngineIDLen,
    				                remote_engine_entry.snmp_remote_engineID, remote_engine_entry.snmp_remote_engineIDLen);
    	      get_enginetime(slp->session->securityEngineID,
    			   slp->session->securityEngineIDLen,
    			   &slp->session->engineBoots,
    			   &slp->session->engineTime, FALSE);
...
/* kinghong added the below API to close the session only*/
int
snmp_close_sess_only(netsnmp_session * session)
{
    struct session_list *slp = NULL, *oslp = NULL;
    ...
    return snmp_sess_close_sess_only((void *) slp);
}
...
#if(SYS_CPNT_CLUSTER==TRUE)
        /*eli, commader uses  name which @swXX is appended  to for sending v3 trap.
          */
    SNMP_CLUSTER_ChangeSecurityNameForRelayTrap(session,pdu);
#endif /*end of #if(SYS_CPNT_CLUSTER==TRUE)*/
    /*
...
#if(SYS_CPNT_CLUSTER==TRUE)
        {
            /*eli,If pdu is the responsed PDU than it should not excect have the responsed PDU
              * for relaying PDU.
              */
...
#if(SYS_CPNT_CLUSTER==TRUE)
        /*eli, commader uses  name which @swXX is appended  to for sending v1/v2 trap.
          */
        SNMP_CLUSTER_ChangeCommunityForRelayTrap(session,pdu);
#endif /*end of #if(SYS_CPNT_CLUSTER==TRUE)*/
...
#if(SYS_CPNT_CLUSTER==TRUE)
/*--------------------------------------------------------------------------
 * ROUTINE NAME - SNMP_API_SendTrapToCommander
 *---------------------------------------------------------------------------
 * PURPOSE:  This function will produce  trap_list for sending trap to commamander
 * INPUT:    template_pdu, trap_session.
 * OUTPUT:   None.
 * RETURN:  TRUE/FALSE
 * NOTE:
 *---------------------------------------------------------------------------
 */
int SNMP_API_SendTrapToCommander(netsnmp_pdu *template_pdu,netsnmp_session *trap_session)
{
    struct session_list trap_list;
    struct snmp_internal_session temp_internal_session;
    netsnmp_pdu *pdu;
    pdu = snmp_clone_pdu(template_pdu);
    memset(&temp_internal_session,0,sizeof(struct snmp_internal_session));
    memset(&trap_list,0,sizeof(struct session_list ));
    trap_list.session=trap_session;
    trap_list.transport = netsnmp_tdomain_transport(trap_session->peername, 0, "udp");
    trap_list.internal = &temp_internal_session;
    if(trap_session->version==SNMP_VERSION_2c)
        template_pdu->command = SNMP_MSG_TRAP2;
    snmp_sess_async_send(&trap_list, pdu,NULL,NULL);
    netsnmp_transport_free(trap_list.transport);
    return 1;
}
#endif /*end of #if(SYS_CPNT_CLUSTER==TRUE)*/
                 ...
                 netsnmp_pdu *pdu, snmp_callback callback, void *cb_data)
{
    ...
    /* If this response packet is too big to send, we will discard this
     * packet and the snmpSilentDrops counter is incremented.
     *
     * Reference:
     * Section 4.2.1., 4.2.2., 4.2.3., 4.2.5. and 4.2.7. in RFC 3416
     */
    if (   (EMSGSIZE == result)
...
snmp_free_pdu(netsnmp_pdu *pdu)
{
  ...
  *          to free the pdu.*/
     ...
     */
     ...
     if ( (pdu->command == 0) && (!SNMP_MGR_IsNetSnmpParseErr()))
     {
        #ifdef VXWORKS
    ...
    SNMP_MGR_SetNetSnmpParseErr(FALSE);
    #endif
}
                     ...
                     u_char * packetptr, int length)
{
  ...
  *          to free the pdu.*/
  if (ret != SNMP_ERR_NOERROR) {
    ...
    #ifdef VXWORKS
    SNMP_MGR_SetNetSnmpParseErr(TRUE);
    #endif
  }
...
#if(SYS_CPNT_CLUSTER==TRUE)
    /* eli,The cammander should check packet for relaying.
      */
    if(SNMP_CLUSTER_GetClusterRole()==CLUSTER_TYPE_COMMANDER&&ret == SNMP_ERR_NOERROR)
    {
        /* eli,If SNMP_CLUSTER_RelayProcess return false, we should handle packet by the normal way.
          */
        if(SNMP_CLUSTER_RelayProcess(sp,pdu)==TRUE)
        {
             /*eli,we already handle pakcet, so we free it and retrun.
               */
            snmp_free_pdu(pdu);
            return 0;
        }
...
#endif /* DISABLE_MIB_LOADING */
        #ifdef VXWORKS
        L_INET_Aton((UI8_T *)value, (UI32_T *)&ltmp);
        #else
