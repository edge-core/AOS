...
/*
 * added for AES 192 and 256
 *  sc_get_priv_properlength(oid *hashtype, u_int hashtype_len):
 *
 * Given a hashing type ("hashtype" and its length hashtype_len), return
 * the length of the hash result.
 *
 ...
 */
int
sc_get_priv_properlength(const oid * hashtype, u_int hashtype_len)
{
    DEBUGTRACE;
    /*
     * Determine transform type hash length.
     */
    if (ISTRANSFORM(hashtype, AES128Priv)) {
        return BYTESIZE(SNMP_TRANS_PRIVLEN_AES128);
    } else if (ISTRANSFORM(hashtype, AES192Priv)) {
        return BYTESIZE(SNMP_TRANS_PRIVLEN_AES192);
    }else if (ISTRANSFORM(hashtype, AES256Priv)) {
        return BYTESIZE(SNMP_TRANS_PRIVLEN_AES256);
    }
    else if(ISTRANSFORM(hashtype, 3DESPriv)){
        return BYTESIZE(SNMP_TRANS_PRIVLEN_3DES);
    }
    return SNMPERR_GENERR;
}
 ...
 */
int
sc_random(u_char * buf, unsigned long * buflen)
#if defined(USE_INTERNAL_MD5) || defined(USE_OPENSSL)
{
    int             rval = SNMPERR_SUCCESS;
#ifndef USE_OPENSSL /* XXX steven.jiang for removing compilation warnings */
#ifdef USE_INTERNAL_MD5
    int             i;
    int             rndval;
    u_char         *ucp = buf;
 ...
 */
int
sc_generate_keyed_hash(const oid * authtype, unsigned long authtypelen,
                       u_char * key, u_int keylen,
                       u_char * message, u_int msglen,
                       u_char * MAC, unsigned long * maclen)
#if  defined(USE_INTERNAL_MD5) || defined(USE_OPENSSL)
{
    int             rval = SNMPERR_SUCCESS;
    int             properlength;
 ...
 */
int
sc_hash(const oid * hashtype, unsigned long hashtypelen, u_char * buf,
        unsigned long buf_len, u_char * MAC, unsigned long * MAC_len)
#if defined(USE_INTERNAL_MD5) || defined(USE_OPENSSL)
{
#ifdef USE_OPENSSL
    int             rval = SNMPERR_SUCCESS;
...
/** do the final pass */
#if defined(OLD_DES)
    EVP_DigestFinal(cptr, MAC,( unsigned int *) MAC_len);
#else /* !OLD_DES */
    if (SSLeay() < 0x907000) {
 ...
 */
int
sc_check_keyed_hash(const oid * authtype, unsigned long authtypelen,
                    u_char * key, u_int keylen,
                    u_char * message, u_int msglen,
...
{
    int             rval = SNMPERR_SUCCESS;
    unsigned long          buf_len = SNMP_MAXBUF_SMALL;
 ...
 */
int
sc_encrypt(const oid * privtype, unsigned long privtypelen,
           u_char * key, u_int keylen,
           u_char * iv, u_int ivlen,
           u_char * plaintext, u_int ptlen,
           u_char * ciphertext, unsigned long * ctlen)
#if defined(USE_OPENSSL)
{
    int             rval = SNMPERR_SUCCESS;
    ...
    DES_key_schedule des_key_sch1;
    DES_key_schedule des_key_sch2;
    DES_key_schedule des_key_sch3;
#else
    DES_key_schedule key_sched_store;
    DES_key_schedule *key_sch = &key_sched_store;
#endif
    ...
    DES_cblock       des_key_struct1;
    DES_cblock       des_key_struct2;
    DES_cblock       des_key_struct3;
#ifdef HAVE_AES
    AES_KEY aes_key;
    int new_ivlen = 0;
#endif
...
#ifdef SNMP_TESTING_CODE
    {
        unsigned long          buf_len = 128, out_len = 0;
    ...
    } else if (ISTRANSFORM(privtype, 3DESPriv)) {
        properlength = BYTESIZE(SNMP_TRANS_PRIVLEN_3DES);
        properlength_iv = BYTESIZE(SNMP_TRANS_PRIVLEN_3DES_IV) / 3;
        pad_size = properlength_iv;
#ifdef HAVE_AES
    ...
    if (ISTRANSFORM(privtype, DESPriv)) {
        ...
        memcpy(des_key_struct1, key + 0, sizeof(key_struct));
        memcpy(des_key_struct2, key + 8, sizeof(key_struct));
        memcpy(des_key_struct3, key + 16, sizeof(key_struct));

        memcpy(my_iv, iv, ivlen);
        /*
         * encrypt the data
         */
        DES_key_sched((const_DES_cblock *)des_key_struct1, &des_key_sch1);
        DES_key_sched((const_DES_cblock *)des_key_struct2, &des_key_sch2);
        DES_key_sched((const_DES_cblock *)des_key_struct3, &des_key_sch3);
        ...
        DES_ede3_cbc_encrypt(plaintext,
                             ...
                             &des_key_sch2,
                             &des_key_sch3,
                             (const_DES_cblock *) my_iv,
                             DES_ENCRYPT);
        if (pad > 0) {
            /*
             * then encrypt the pad block
             */
            DES_ede3_cbc_encrypt(pad_block,
                                 ciphertext + plast,
                                 pad_size,
                                 &des_key_sch1,
                                 &des_key_sch2,
...
#ifdef USE_OPENSSL
{
    ...
    DES_key_schedule key_sched_store;
    DES_key_schedule *key_sch = &key_sched_store;
    DES_key_schedule des_key_sch_1;
    DES_key_schedule des_key_sch_2;
    DES_key_schedule des_key_sch_3;
#else
    DES_key_schedule key_sched_store;
    DES_key_schedule *key_sch = &key_sched_store;
#endif
    ...
    if (ISTRANSFORM(privtype, DESPriv)) {
        ...
        *ptlen = ctlen;
        ...
        memcpy(des_key_struct1, key + 0, sizeof(des_key_struct1));
        memcpy(des_key_struct2, key + 8, sizeof(des_key_struct2));
        memcpy(des_key_struct3, key + 16, sizeof(des_key_struct3));

        memcpy(my_iv, iv, ivlen);
        DES_key_sched((const_DES_cblock *)des_key_struct1, &des_key_sch_1);
        DES_key_sched((const_DES_cblock *)des_key_struct2, &des_key_sch_2);
        DES_key_sched((const_DES_cblock *)des_key_struct3, &des_key_sch_3);

        DES_ede3_cbc_encrypt(ciphertext, // + k,
                             plaintext, // + k,
                             ctlen,
                             &des_key_sch_1,
                             &des_key_sch_2,
