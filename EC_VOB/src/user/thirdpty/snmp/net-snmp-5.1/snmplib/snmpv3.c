...
#include "leaf_3411.h"
#include "leaf_sys.h" /* define SYS_VAL_LOCAL_UNIT_ID */
#include <ctype.h>
#include "sys_cpnt.h"
#if 1 /* workaround to include the prototype embraced by #ifdef NET_SNMP_INCLUDES_H */
...
void
snmpv3_authtype_conf(const char *words, char *cptr)
{
 ...
 */
void
snmpv3_secLevel_conf(const char *words, char *cptr)
{
    char            buf[1024];
    ...
    case 'x':
        if (!strcasecmp(optarg, "DES")) {
            ...
            session->securityPrivProtoLen = USM_PRIV_PROTO_AES256_LEN;
                ...
                session->securityPrivProto = usm3DESPrivProtocol;
                session->securityPrivProtoLen = USM_PRIV_PROTO_3DES_LEN;
        } else {
            fprintf(stderr,
...
int setup_engineID_byLength(u_char *id,int len)
{
    u_char         *bufp = NULL;
    if((len<MINSIZE_snmpEngineID)||(len>MAXSIZE_snmpEngineID))
       return -1;
    if ((bufp = (u_char *) malloc(len)) == NULL) {
        snmp_log_perror("setup_engineID malloc");
        return -1;
...
/*
 *Set engineBoots to the static variable of set_engineBoots.
 *After seting engineBoots, it should be used set_enginetime
 *seting Boots and time in the LCD timing cache
 */
void setup_engineBoots(u_long set_engineBoots)
{
    engineBoots = set_engineBoots;
}

int
setup_engineID(u_char ** eidp, const char *text)
{
    int localsetup = (eidp) ? 0 : 1;
   //  int             enterpriseid = htonl(SYS_ADPT_ENTERPRISE_OID);
   int enterpriseid;
    static  oid            PrivateMIB_module_oid[SYS_ADPT_MAX_OID_STRING_LEN+1];/* MAX_OID_LEN*/
    UI32_T oid_len;
    /*
     * Use local engineID if *eidp == NULL.
     */
    ...
    if (SYS_PMGR_GetSysObjectID(SYS_VAL_LOCAL_UNIT_ID, buffer)!=TRUE)
    {
        snmp_log_perror(" cannot get sysObjectID\n");
        return -1;
    }

	SNMP_MGR_StringToObejctID(PrivateMIB_module_oid, (I8_T *)buffer, &oid_len);

    /* In memory, enterpriseid is presented as network order
     * 1.3.6.1.4.1.X, in which X is the enterprise ID we want to used
     */
    enterpriseid = (int)L_STDLIB_Hton32(PrivateMIB_module_oid[6]);
    ...
    {
        engineIDType = localEngineIDType = ENGINEID_TYPE_MACADDR;
    }
    /*
     * Determine length of the engineID string.
     */

#if (SYS_CPNT_SNMPV3_ENGINE_ID_STYLE == SYS_CPNT_SNMPV3_ENGINE_ID_STYLE_3COM)
    len = 12;
#endif
#if (SYS_CPNT_SNMPV3_ENGINE_ID_STYLE == SYS_CPNT_SNMPV3_ENGINE_ID_STYLE_ACCTON)
  len = 13;
#endif
    /*
     * Allocate memory and store enterprise ID.
   ...
   /* kinghong used the  following rule to generate EngineID,
       1.  The first 4 bytes is  from  Enterprise ID. with the  first bit  is fixed to 1
       2. The 5 bytes is 03 (hex, dec 3)
       3. The  6-11 bytes is from mac-address.
       4. The 12-13 bytes is unused and 0.
       */

    if (localEngineIDType == ENGINEID_TYPE_MACADDR)
     {
           memcpy(bufp, &enterpriseid, 4);      /* XXX Must be 4 bytes! */
        ...
        #if 1
        if (!SWCTRL_POM_GetCpuMac(mac))/*now only 1 unit*/
        {
     ...
     else if (localEngineIDType == ENGINEID_TYPE_TEXT)
     {
     	memcpy ( bufp, text, 13);
     }
     else
 ...
 */
void
engineBoots_conf(const char *words, char *cptr)
{
 ...
 */
void
engineIDType_conf(const char *words, char *cptr)
{
 ...
 */
void
engineIDNic_conf(const char *words, char *cptr)
{
    /*
 ...
 */
void
engineID_conf(const char *words, char *cptr)
{
...
void
version_conf(const char *words, char *cptr)
{
    if ((strcmp(cptr,  "1") == 0) ||
        (strcmp(cptr, "v1") == 0)) {
 ...
 */
void
oldengineID_conf(const char *words, char *cptr)
{
...
snmpv3_local_snmpEngineTime(void)
{
    ...
    #ifdef VXWORKS
    enginetime = now.tv_sec- snmpv3starttime.tv_sec;
    return  enginetime;
    #else
