...
#if defined(USE_OPENSSL) || defined(USE_INTERNAL_MD5)
{
    ...
    rval = sc_hash(hashtype, hashtype_len, buf, nbytes, Kul, (unsigned long *)kul_len);
...
#if defined(USE_OPENSSL) || defined(USE_INTERNAL_MD5)
{
    ...
    rval = sc_random(kcstring, (unsigned long *)&nbytes);
    ...
    if (tmpbuf) {
        ...
        *kcstring_len -= properlength;
        rval = sc_hash(hashtype, hashtype_len, tmpbuf, properlength * 2,
                       kcstring + properlength,(unsigned long *) kcstring_len);
...
#if defined(USE_OPENSSL) || defined(USE_INTERNAL_MD5)
{
    ...
    *newkey_len = properlength;
    ...
    if (tmpbuf) {
        ...
        rval = sc_hash(hashtype, hashtype_len, tmpbuf, properlength * 2,
                       tmp_buf, (unsigned long *)&tmp_buf_len);
