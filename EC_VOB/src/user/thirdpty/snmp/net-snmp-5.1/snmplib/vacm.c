...
#ifdef VXWORKS
/* kinghong added this API */
struct vacm_accessEntry *
vacm_get_accessList(void)
{
    return accessList;
}

struct vacm_groupEntry *
vacm_get_groupList(void)
{
    return groupList;
}

struct vacm_viewEntry *
vacm_get_viewList(void)
{
     return viewList;
}
#endif
...
#ifdef VXWORKS
/* kinghong create this API for get exact a View, for SNMP_MGR check create view use */

struct vacm_viewEntry *
vacm_getExactViewEntry(const char *viewName,
                  oid * viewSubtree, size_t viewSubtreeLen)
{

    struct vacm_viewEntry *vp = NULL;
                     ...
                     oid * viewSubtree, size_t viewSubtreeLen)
{
 ...
 /* kinghong added for limited control*/
     ...
     */
    SNMP_MGR_GetSnmpV3ViewCounter(& view_counter);

    /*kinghong added*/
    if (view_counter >= SNMP_MGR_MAX_NBR_OF_SNMPV3_VIEW)
    {
        if (SNMP_MGR_IsDebugMode())
        {
            SYSFUN_Debug_Printf(" Exceed view counter, counter = %d\n", view_counter);
        }
        return NULL;
    }
    #endif
    ...
    if (glen < 0 || glen > VACM_MAX_STRING)
    {
        if (SNMP_MGR_IsDebugMode())
        {
            SYSFUN_Debug_Printf(" view name len exceed limit, len = %d\n",glen);
        }
        return NULL;
    }
    ...
    if (vp == NULL)
    {
        if (SNMP_MGR_IsDebugMode())
        {
            SYSFUN_Debug_Printf("memory allocate fail\n");
        }
        return NULL;
    }
    vp->reserved =
    ...
    #ifdef VXWORKS
    /*kinghong added*/
    SNMP_MGR_AdjustSnmpV3ViewCounter(TRUE);
    #endif
    return vp;
}
                      ...
                      oid * viewSubtree, size_t viewSubtreeLen)
{
    ...
    free(vp);
    #ifdef VXWORKS
     SNMP_MGR_AdjustSnmpV3ViewCounter(FALSE);
    #endif
    return;
}
...
vacm_destroyAllViewEntries(void)
{
    ...
    #ifdef VXWORKS
    SNMP_MGR_ResetSnmpV3ViewCounter();
    #endif
}
...
vacm_createGroupEntry(int securityModel, const char *securityName)
{
    ...
    #ifdef VXWORKS
  /* kinghong added for limited control*/
    UI32_T  security_to_group_counter;
     ...
     * Get security, source address/netmask and community strings.
     */
    SNMP_MGR_GetSnmpV3SecurityToGroupCounter(& security_to_group_counter);


    /*kinghong added*/
    if (security_to_group_counter >= SNMP_MGR_MAX_NBR_OF_SNMPV3_SECURITYTOGROUP)
        return NULL;
    #endif
    glen = (int) strlen(securityName);
    if (glen < 0 || glen > VACM_MAX_STRING)
    ...
    if (og == NULL)
        ...
        og->next = gp;
    #ifdef VXWORKS
    SNMP_MGR_AdjustSnmpV3SecurityToGroupCounter(TRUE);
    #endif
    return gp;
}
...
vacm_destroyGroupEntry(int securityModel, const char *securityName)
{
    ...
    free(vp);
    #ifdef VXWORKS
      SNMP_MGR_AdjustSnmpV3SecurityToGroupCounter(FALSE);
    #endif
    return;
}
...
vacm_destroyAllGroupEntries(void)
{
    ...
    #ifdef VXWORKS
    SNMP_MGR_ResetSnmpV3SecurityToGroupCounter();
    #endif
}
...
#ifdef VXWORKS
/* kinghong add the below API for Get "exact" access entry, modify the original
 * API by comparing securityLevel not only greater than, but equal to
 */
struct vacm_accessEntry *
vacm_getExactAccessEntry(const char *groupName,
                    const char *contextPrefix,
                    int securityModel, int securityLevel)
{
    struct vacm_accessEntry *vp;
                       ...
                       int securityModel, int securityLevel)
{
     ...
     * Get security, source address/netmask and community strings.
     */
    SNMP_MGR_GetSnmpV3GroupCounter(& group_counter);


    /*kinghong added*/
    if (group_counter >= SNMP_MGR_MAX_NBR_OF_SNMPV3_GROUP)
    ...
    #ifdef VXWORKS
    /* kinghong added*/
    SNMP_MGR_AdjustSnmpV3GroupCounter(TRUE);
    #endif
    return vp;
}
                        ...
                        int securityModel, int securityLevel)
{
    ...
    if (accessList && accessList->securityModel == securityModel
        ...
        /* kinghong, we do not implement contextPrefix*/
    ...
    #ifdef VXWORKS
      /* kinghong added*/
    SNMP_MGR_AdjustSnmpV3GroupCounter(FALSE);
    #endif
    return;
}
...
vacm_destroyAllAccessEntries(void)
{
   ...
   #ifdef VXWORKS
   SNMP_MGR_ResetSnmpV3GroupCounter();
   #endif
}
