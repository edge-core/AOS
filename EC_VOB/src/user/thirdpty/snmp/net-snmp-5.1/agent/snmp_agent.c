...
/* although the code is built on linux, but VXWORKS will be defined in
 * /src/user/apps/snmpmgmt/v3/snmp/include, which is included by
 * /src/user/thirdpty/snmp/net-snmp-5.1/win32/net-snmp
 */
...
#  include <time.h>
# endif
#endif /* end of #if TIME_WITH_SYS_TIME */
#else /* #ifndef VXWORKS */

/* Some toolchains defines CONSTANT like LONG_MAX in <limits.h>,
 * add the code below to compile without error
 */
#ifdef HAVE_LIMITS_H
#include <limits.h>
#endif

/* kinghong added for send authentication failure trap*/
#include "snmp_mgr.h"
#include "sys_cpnt.h"
...
_reorder_getbulk(netsnmp_agent_session *asp)
{
    ...
    /*eli,2005.2.14,If r =0 ,it should return and should not proccess following code[ES4649-ZZ-01061]*/
...
agent_check_and_process(int block)
{
    ...
    if (block != 0 && fakeblock != 0) {
         ...
         */
        // #ifdef VXWORKS
        // tvp->tv_sec = 1; /* kinghong modified timeout from 0 to 1 sec*/
        // #else
        // tvp->tv_sec = 0;
        // #endif
        tvp->tv_sec  = 0;
        tvp->tv_usec = 10000;   /* wait for 10000us (1 ticks) to free CPU for a while */
    }
    ...
    if (count > 0) {
        ...
        switch (count) {
        case 0:
           /* kinghong patched here, we moved the snmp_timeout to the below to fix 
            * memory ran out issue when the agent is in high traffic and cannot do
            * the retry mechiasm.
            snmp_timeout();*/
            break;
        case -1:
    ...
    /* kinghong 2004/5/2 move the snmp_Timeout here to check inform request retry everytime
     * even when count != 0, this will fixed memory ran out issue when the agent is in high
     * traffic and cannot do the retry mechiasm.
     */
     snmp_timeout();
    /*
...
/* When IPv6 is supported, bind port with
 * NETSNMP_TRANSPORT_CONFIG_STR - udp6:[::%0]:161,udp:161.
 * When IPv6 is not supported, bind port with default value
 * sprintf(buf, "udp:%d", SNMP_PORT);
 */
#if (SYS_CPNT_IPV6 == TRUE)
    netsnmp_ds_set_string(NETSNMP_DS_APPLICATION_ID,
                          NETSNMP_DS_AGENT_PORTS,
                          NETSNMP_TRANSPORT_CONFIG_STR);
                   ...
                   netsnmp_pdu *pdu, void *magic)
{
...
#ifdef VXWORKS
#if (SYS_CPNT_MGMT_IP_FLT == TRUE)      
    struct sockaddr *sock_addr_in = (struct sockaddr *)pdu->transport_data;
    L_INET_AddrIp_T   inet_addr;
    memset(&inet_addr, 0, sizeof(inet_addr));
    
    L_INET_SockaddrToInaddr(sock_addr_in, &inet_addr);
#endif
#endif
...
#ifdef VXWORKS
#if (SYS_CPNT_MGMT_IP_FLT == TRUE)     
    if (MGMT_IP_FLT_IsValidIpFilterAddress(MGMT_IP_FLT_SNMP,&inet_addr)==FALSE)
    {
    	 netsnmp_remove_and_free_agent_snmp_session(asp);
         return 0;
    }
    ...
    if ((access_ret = check_access(pdu)) != 0) {
        if (access_ret == VACM_NOSUCHCONTEXT) {
             ...
             */
         #ifdef VXWORKS
         /* kinghong add to gen auth failure trap*/
...
netsnmp_create_subtree_cache(netsnmp_agent_session *asp)
{
    ...
    if (asp->pdu->command == SNMP_MSG_GETBULK) {
        ...
        if ((r = count - n) <= 0) {
            ...
            if (maxresponses == 0)
                maxresponses = SYS_ADPT_SNMP_MAX_REPETITIONS_OF_GET_BULK;   /* more than reasonable default */
...
/*
2004/04/08 Patch by phoebe
Problem description -- ES3526V-60-P2-00112: projx000358: Enterasys RADIUS mib not fully supported
                       bug of SNMP Agent: when processing ONE packet with serveral variable-bindings, 
                       for example, we have the following variable-bindings in ONE packet:
                       snmp set:
                       set variable A to 1
                       set variable B to 2
                       set variable A to 3
                       ...
                       agent will call our code gen function var_xxxx in incorrect sequence:
                       (same variable(same oid) will be put together)
                       set variable A to 1
                       set variable A to 3
                       set variable B to 2                       
                       set variable B to 4 
                       ...
                       set variable A to 3
                       set variable B to 4                                                                                                                                          
Root cause -- please see Problem description            
Resolution -- modify snmp_agent.c, handle_var_requests function,
              and old_api.c, function netsnmp_old_api_helper to fix this bug              
Modify files -- snmp_agent.c, old_api.c   
*/
int searchForIdx=0;
...
handle_var_requests(netsnmp_agent_session *asp)
{
    ...
    netsnmp_request_info *tmpReqinfo=NULL;
    int varCnt=0;
    BOOL_T found=FALSE;
    asp->reqinfo->asp = asp;
     ...
     */
varCnt=asp->vbcount;/*total variable-bindings count*/
    for (searchForIdx = 1; searchForIdx <= varCnt; searchForIdx++) 
    { 
        found=FALSE;        
        for (i = 0; i <= asp->treecache_num; i++) 
        {        
            tmpReqinfo=asp->treecache[i].requests_begin;/*2004/4/8 patch by phoebe:
                                                          one treecache is for one OID,
                                                          if we set twice for one OID in one packet, then
                                                          it will be store in the same treecache,
                                                          for example, we set in the below sequence in one packet:
                                                          (1)set A=1
                                                          (2)set B=2
                                                          (3)set A=3
                                                          (4)set B=4,
                                                          since (1) & (3) is the same oid, 
                                                          then (1) will be stored in treecache[1].requests_begin
                                                          and (2) will be stored in treecache[1].requests_begin->next
                                                          
                                                          since (2) & (4) is the same oid,
                                                          so (3) will be stored in treecache[2].requests_begin
                                                          and (4) will be stored in treecache[2].requests_begin->next
                                                          
                                                          and so on
                                                          */
            for (; tmpReqinfo; tmpReqinfo = tmpReqinfo->next)
            {
                /*2004/4/8 patch by phoebe:
                 tmpReqinfo->index :
                 the original sequence of the variable, 
                 for example, the sequene of "set B=2" is (2) and "set B=4" is (4)*/
                if (tmpReqinfo->index==searchForIdx)
                {                                         
                    found=TRUE;
                    break;
                } 
            }/*end of for (; tmpReqinfo; tmpReqinfo = tmpReqinfo->next)*/
            if (found)
                break;            
        }
        if (found)
