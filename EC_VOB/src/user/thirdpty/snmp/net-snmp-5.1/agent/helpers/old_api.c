...
*/
extern int searchForIdx;/*patch by phoebe: this extern variable is defined in snmp_agent.c*/
                       ...
                       netsnmp_request_info *requests)
{
    ...
    for (; requests; requests = requests->next) {
        if (requests->index!=searchForIdx)/*2004/04/08 Patch by phoebe, for detail, please see comment at the head of this function*/
            continue;
...
{
    oid etsysConfigMgmtChangeRowStatus_oid[]={1,3,6,1,4,1,5624,1,2,16,2,7,1,11};
    if (memcmp(requests->requestvb->name, etsysConfigMgmtChangeRowStatus_oid, 14*sizeof(oid))==0)
    {
        status=write_etsysConfigMgmtChangeRowStatus(reqinfo->mode,
