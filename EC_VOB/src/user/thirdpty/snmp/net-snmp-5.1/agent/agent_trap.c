...
#ifdef VXWORKS
/* kinghong added for get local ip address header*/
#include "sysfun.h"
...
/* kinghong moved this into agent_trap.h files*/
#ifndef VXWORKS
struct trap_sink {
...
#ifdef VXWORKS
/* kinghong add the follow API to get the pointer of snmpNotifyTableStorage */
    ...
    int addr_type, int addr_len, u_char *addr_p )
{
...
#if 1  /* upgraded from layer 2 to layer 3 */
    /* ASF4612MMS-FLF-08-00908
     * SNMP: The source IP address and snmpTrapAddress of trap inform
     * packet is different when the packet is sent by craft port
     * 2011.09.28, *Zhong* Qiyao, <qiyao_zhong>
     *
     * This happens for a layer 3 switch with multiple IP addresses.
     * A separate SNMPv PDU needs to be formed for each trap destination,
     * because it has its own snmpTrapAddress binding (source IP address),
     * So, SNMPv2 PDU composition is delayed
     * from function "netsnmp_send_traps" in file "agent_trap.c"
     * to function "send_notifications" in file "snmpNocifyTable.c".
     *
     * ASF4612MMS-FLF-08-00991
     * SNMP: DUT can not send SNMPv3 Inform Trap
     * 2011.12.07, *Zhong* Qiyao, <qiyao_zhong>
     * This was forgotten in ASF4612MMS-FLF-08-00908.
     * The "command" (i.e. trap vs. inform) needs to be carried over to
     * the cloned PDU.
     */
    template_v2pdu->command = template_v1pdu->command;
#else  /* original code */
    ...
    if (!var && template_v1pdu->community) {
        if (!snmp_varlist_add_variable( &(template_v2pdu->variables),
                 ...
                 strlen((char *)template_v1pdu->community)))
            snmp_log(LOG_WARNING,
                          ...
                          char * context, int flags)
{
    ...
    #ifdef VXWORKS
     /* kinghong added for get local ip address*/
    NETCFG_TYPE_InetRifConfig_T          rif_node;


    memset(&rif_node, 0, sizeof(NETCFG_TYPE_InetRifConfig_T));
    rif_node.ifindex = 0;
    #endif
    ...
    if ( trap == -1 ) {
...
#ifdef VXWORKS
        /* kinghong added for get local ip address*/
        if (NETCFG_POM_IP_GetNextRifConfig(&rif_node) < 0)
        {
            memset( template_v1pdu->agent_addr, 0, sizeof(UI32_T));
        }
        else
        {
            /* Copy local source IP address
             */
            memcpy (template_v1pdu->agent_addr, rif_node.addr.addr, sizeof(UI32_T));
        }
#endif
         ...
         */
        template_v2pdu = convert_v1pdu_to_v2( template_v1pdu, 1 /*ipv4(1)*/,
            4 /*length*/, template_v1pdu->agent_addr);
        if (!template_v2pdu) {
            snmp_log(LOG_WARNING,
	...
	NLM_MGR_NotificationLog(trap, specific, enterprise, enterprise_length, template_v2pdu->variables);
#endif
    ...
    /* Eli: When member happens the trap event, it will continuous send v1 and v2 format
     * trap to the commander.
     * But, the commander can not handle so fast continuous packets.
     * So, we patch to sleep 1/10 second for delayed sending the second trap.
     */
     SYSFUN_Sleep(10);
         ...
         */
    snmp_call_callbacks(SNMP_CALLBACK_APPLICATION,
                        SNMPD_CALLBACK_SEND_TRAP2, template_v1pdu);
#else  /* original code */
    snmp_call_callbacks(SNMP_CALLBACK_APPLICATION,
...
void
snmpd_parse_config_trap2sink(const char *words, char *cptr)
{
    char            tmpbuf[1024];
    char           *sp, *cp, *pp = NULL;
    int             sinkport;
...
void
snmpd_parse_config_informsink(const char *words, char *cptr)
{
    char            tmpbuf[1024];
    char           *sp, *cp, *pp = NULL;
    int             sinkport;
...
void
snmpd_parse_config_trapsess(const char *words, char *cptr)
{
...
void
snmpd_parse_config_trapcommunity(const char *words, char *cptr)
{
