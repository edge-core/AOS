...
/* Currently, VXWORKS is defined in our Linux platform
 * This will cause problem because snprintf in C library
 * takes "..." for last argument instead of va_list
 * So, format like "%d %s" may cause exception because second
 * argument (%s) will take unknown value in this case
 * We mark out the following code and use the original third party
 * code. It can be rolled back after VXWORKS not defined in Linux
 */
void
ag_trace_log(const char *format, ...)
{
#define AG_MAX_MSG_LEN  120
  ...
  snmp_log(LOG_INFO, "%s\n", msg);
#else
  snprintf(msg, AG_MAX_MSG_LEN - 1, format, args);
#endif
...
#ifdef VXWORKS
/* kinghong replace the (debug purposes) code with the real code that can get statistics from system*/
void
SYSTEM_get_eth_statistics(VAR_OID_T *data_source, ETH_STATS_T *where)
{

    SWDRV_RmonStats_T     rmonstats;

    where->ifIndex = data_source->objid[data_source->length - 1];

#if(SYS_CPNT_SYSTEMWIDE_COUNTER == TRUE)
   if (!NMTR_PMGR_GetSystemwideRmonStats(where->ifIndex, &rmonstats))
       memset(&rmonstats, 0, sizeof(rmonstats));
#else
    if (!NMTR_PMGR_GetRmonStats( where->ifIndex, &rmonstats) )
      memset(&rmonstats, 0, sizeof(rmonstats));
#endif

    where->octets =  rmonstats.etherStatsOctets;
    ...
    */
    where->octets_64 = rmonstats.etherStatsOctets;
    where->packets  =  rmonstats.etherStatsPkts;
    where->bcast_pkts  =  rmonstats.etherStatsBroadcastPkts;
    where->mcast_pkts  =  rmonstats.etherStatsMulticastPkts;
    where->crc_align  =  rmonstats.etherStatsCRCAlignErrors;
    where->undersize  =  rmonstats.etherStatsUndersizePkts;
    where->oversize  =  rmonstats.etherStatsOversizePkts;
    where->fragments  =  rmonstats.etherStatsFragments;
    where->jabbers =  rmonstats.etherStatsJabbers;
    where->collisions  =  rmonstats.etherStatsCollisions;

    where->pkts_64  =  rmonstats.etherStatsPkts64Octets;
    where->pkts_65_127  =  rmonstats.etherStatsPkts65to127Octets;
    where->pkts_128_255  =  rmonstats.etherStatsPkts128to255Octets;
    where->pkts_256_511  =  rmonstats.etherStatsPkts256to511Octets;
    where->pkts_512_1023 =  rmonstats.etherStatsPkts512to1023Octets;
    where->pkts_1024_1518 =  rmonstats.etherStatsPkts1024to1518Octets;
    return;

}


void SYSTEM_get_eth_statistics_HC(VAR_OID_T *data_source, ETH_STATS_HC_T *where)
{
    NMTR_MGR_EtherStatsHighCapacityEntry_T entry;

    entry.ether_stats_index = data_source->objid[data_source->length - 1];
    where->ifIndex = data_source->objid[data_source->length - 1];

    if(!NMTR_PMGR_GetEtherStatsHighCapacityEntry(&entry))
    {
        memset(&entry, 0, sizeof(entry));
    }

    where->hc_overflow_pkts = entry.ether_stats_high_capacity_overflow_pkts;
    where->hc_pkts = entry.ether_stats_high_capacity_pkts;
    where->hc_overflow_octets = entry.ether_stats_high_capacity_overflow_octets;
    where->hc_octets = entry.ether_stats_high_capacity_octets;

    where->hc_overflow_pkts_64 = entry.ether_stats_high_capacity_overflow_pkts64_octets;
    where->hc_pkts_64 = entry.ether_stats_high_capacity_pkts64_octets;
    where->hc_overflow_pkts_65_127 = entry.ether_stats_high_capacity_overflow_pkts65to127_octets;
    where->hc_pkts_65_127 = entry.ether_stats_high_capacity_pkts65to127_octets;

    where->hc_overflow_pkts_128_255 = entry.ether_stats_high_capacity_overflow_pkts128to255_octets;
    where->hc_pkts_128_255 = entry.ether_stats_high_capacity_pkts128to255_octets;
    where->hc_overflow_pkts_256_511 = entry.ether_stats_high_capacity_overflow_pkts256to511_octets;
    where->hc_pkts_256_511 = entry.ether_stats_high_capacity_pkts256to511_octets;

    where->hc_overflow_pkts_512_1023 = entry.ether_stats_high_capacity_overflow_pkts512to1023_octets;
    where->hc_pkts_512_1023 = entry.ether_stats_high_capacity_pkts512to1023_octets;
    where->hc_overflow_pkts_1024_1518 = entry.ether_stats_high_capacity_overflow_pkts1024to1518_octets;
    where->hc_pkts_1024_1518 = entry.ether_stats_high_capacity_pkts1024to1518_octets;
