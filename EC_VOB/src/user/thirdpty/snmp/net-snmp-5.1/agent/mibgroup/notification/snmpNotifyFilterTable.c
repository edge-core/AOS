...
struct header_complex_index* SNMP_NOTIFY_FILTER_TABLE_GetNotifyFilterList()
{
   return snmpNotifyFilterTableStorage;
}

struct header_complex_index** SNMP_NOTIFY_FILTER_TABLE_GetNotifyFilterListAddr()
{
   return &snmpNotifyFilterTableStorage;
}
#endif
...
snmpNotifyFilterTable_add(struct snmpNotifyFilterTable_data *thedata)
{
    ...
    SNMP_MGR_GetSnmpV3NotifyFilterCounter(& notify_filter_counter);
    
   
    /*kinghong added*/
    if (notify_filter_counter >= SNMP_MGR_MAX_NBR_OF_NOTIFY_FILTER_ENTRY)
        return SNMPERR_SUCCESS;
    
    SNMP_MGR_AdjustSnmpV3NotifyFilterCounter(TRUE);
