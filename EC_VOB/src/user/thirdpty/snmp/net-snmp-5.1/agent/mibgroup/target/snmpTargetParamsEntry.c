 ...
 */
#ifdef VXWORKS
/* kinghong added the below API to get the *aPtable and the static variable address*/
...
snmpTargetParamTable_dispose(struct targetParamTable_struct *reaped)
{
    ...
    free(reaped);
    #ifdef VXWORKS
    SNMP_MGR_AdjustSnmpV3TargetParamsCounter(FALSE);
    #endif
                               ...
                               struct targetParamTable_struct **listPtr)
{
    ...
    UI32_T  target_params_counter;


    /*
     * Get security, source address/netmask and community strings.
     */
    SNMP_MGR_GetSnmpV3TargetParamsCounter(& target_params_counter);


    /*kinghong added*/
    if (target_params_counter >= SNMP_MGR_MAX_NBR_OF_TARGET_ADDR_PARAMS_ENTRY)
        return;

    SNMP_MGR_AdjustSnmpV3TargetParamsCounter(TRUE);
    #endif
                              ...
                              u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        #ifdef VXWORKS
        // if (long_ret < 0) {
         /* kinghong modified to limit the support MP model to 0 and 1
          since when MP model is 3 will crash*/
                              ...
                              u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        if (params == NULL) {
            ...
            if (   ('\0' != params->secName[0])
                && (0 == L_CHARSET_IsValidUserNameString(params->secName)))
            {
