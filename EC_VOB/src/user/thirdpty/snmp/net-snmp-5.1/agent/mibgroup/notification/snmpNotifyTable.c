...
struct header_complex_index* SNMP_NOTIFY_TABLE_GetNotifyList()
{
   return snmpNotifyTableStorage;
}

struct header_complex_index** SNMP_NOTIFY_TABLE_GetNotifyListAddr()
{
   return &snmpNotifyTableStorage;
}
#endif
...
#ifdef VXWORKS
    UI8_T targetparamsname[MAXSIZE_snmpTargetParamsName+1];
#endif

    void *session_internal_p;
    netsnmp_transport *transport_p;
    netsnmp_pdu *v2pdu_p;

#ifdef VXWORKS
    L_INET_AddrIp_T src_addr_str, dest_addr_str, nexthop_addr_str;
    UI32_T out_ifindex;
    BOOL_T is_ipv4 = FALSE;
#endif

    DEBUGMSGTL(("send_notifications", "starting: pdu=%x, vars=%x\n",
                template_pdu, template_pdu->variables));
#ifdef VXWORKS
  //  SNMP_MGR_GetSemporeID(&sem_id);
   // SYSFUN_GetSem(sem_id, SYSFUN_TIMEOUT_WAIT_FOREVER);
#endif

#if (SYS_CPNT_CLUSTER == TRUE)
    if(SNMP_CLUSTER_GetClusterRole()==CLUSTER_TYPE_ACTIVE_MEMBER)
    {
        SNMP_CLUSTER_SendTrapToCommander(minor,template_pdu);
        /*eli,The member only send traps to cammander, it do not send other.
         */
        return 0;
    }
#endif /*end of #if(SYS_CPNT_CLUSTER==TRUE)*/
    ...
    for (hptr = snmpNotifyTableStorage; hptr; hptr = hptr->next) {
        ...
        if (!nptr->snmpNotifyTag)
          ...
          /* kinghong patched*/
        nptr->snmpNotifyTag[nptr->snmpNotifyTagLen] = '\0';

#if 0 /*kinghongv3trap*/
        sess = get_target_sessions(nptr->snmpNotifyTag, NULL, NULL);
#else
        sess = get_target_sessions(nptr->snmpNotifyTag, nptr->snmpNotifyType, (char *)targetparamsname,
				   NULL, NULL);
#endif
        /*
        ...
        for (sptr = sess; sptr; sptr = sptr->next)
        {
            if (sptr->version == SNMP_VERSION_1 &&
                ...
                {
                    if (SNMP_MGR_CheckTrapSend(sptr, template_pdu->trap_type, template_pdu->specific_type, 
                                               template_pdu->enterprise, template_pdu->enterprise_length, 
                                               (char *)targetparamsname, template_pdu->command) == TRUE)
                    {
                        #ifdef VXWORKS
                        //SYSFUN_GetSem(sem_id, SYSFUN_TIMEOUT_WAIT_FOREVER);
                        #endif
                                           ...
                                           (char *)targetparamsname, template_pdu->command) == TRUE)
                {
                     ...
                     */
                    session_internal_p = snmp_sess_pointer(sptr);
                    transport_p = snmp_sess_transport(session_internal_p);

                    /* GET THE snmpTrapAddress binding.
                     *
                     * Limitation:
                     * 1. This is defined as IpAddress (IPv4 only).  So,
                     * IPv6 addresses cannot be represented.
                     * We fill 0.0.0.0.
                     * 2. The IP address type is not stored in a
                     ...
                     */
                    dest_addr_str.addrlen = transport_p->remote_length - sizeof(UI16_T) /*port*/;

                    if (dest_addr_str.addrlen == SYS_TYPE_IPV4_ADDR_LEN)
                    {
                        is_ipv4 = TRUE;
                        dest_addr_str.type = L_INET_ADDR_TYPE_IPV4;
                        memcpy(dest_addr_str.addr, transport_p->remote,
                            dest_addr_str.addrlen);
                    }
                    else if (dest_addr_str.addrlen == SYS_TYPE_IPV4Z_ADDR_LEN)
                    {
                        is_ipv4 = TRUE;
                        dest_addr_str.type = L_INET_ADDR_TYPE_IPV4Z;
                        memcpy(dest_addr_str.addr, transport_p->remote,
                            dest_addr_str.addrlen - sizeof(UI32_T) /*zone*/);
                        dest_addr_str.zoneid =
                            L_STDLIB_Ntoh32(* (UI32_T *) &transport_p->remote
                                [dest_addr_str.addrlen - sizeof(UI32_T) /*zone*/]);
                    }
                    ...
                    if (is_ipv4)
                    {
                        if (IPAL_RESULT_OK != IPAL_ROUTE_RouteLookup(&dest_addr_str, &src_addr_str, &nexthop_addr_str, &out_ifindex))
                        {
                            DEBUGMSGTL(("send_notifications", "no route: closing...\n"));
                                continue;
                        }
                    }
                    else
                    {
                        memset(&src_addr_str, 0, sizeof(L_INET_AddrIp_T));
                        src_addr_str.type    = L_INET_ADDR_TYPE_IPV4;
                        src_addr_str.addrlen = SYS_TYPE_IPV4_ADDR_LEN;
                    }

                    /* convert v1 PDU to v2 PDU
                     */
                    v2pdu_p = convert_v1pdu_to_v2(template_pdu, src_addr_str.type, src_addr_str.addrlen, src_addr_str.addr);
#else
                    /* non-Accton platform: we don't have the code to get the
                     * source IP address based on destination IP address
                     */
                    v2pdu_p = convert_v1pdu_to_v2(template_pdu, 1 /*ipv4(1)*/, 4 /*length*/,
                        template_pdu->agent_addr);
#endif

                    if (! v2pdu_p) {
                        snmp_log(LOG_WARNING,
                                 "send_notifications: failed to convert v1->v2 template PDU\n");
                        snmp_free_pdu(template_pdu);
                        return -1;
                    }

                    send_trap_to_sess(sptr, v2pdu_p);
                    snmp_free_pdu(v2pdu_p);
...
snmpNotifyTable_add(struct snmpNotifyTable_data *thedata)
{
   ...
   /* kinghong added for limited control*/
     ...
     */
    SNMP_MGR_GetSnmpV3NotifyCounter(& notify_counter);


    /*kinghong added*/
    if (notify_counter >= SNMP_MGR_MAX_NBR_OF_NOTIFY_ENTRY)
        return SNMPERR_SUCCESS;

    SNMP_MGR_AdjustSnmpV3NotifyCounter(TRUE);
                    ...
                    u_char * statP, oid * name, size_t name_len)
{
    ...
    case RESERVE1:
        ...
        if (!snmpTagValid((const char *)var_val, var_val_len)) {
                          ...
                          u_char * statP, oid * name, size_t name_len)
{
    ...
    case ACTION:
        if (StorageTmp == NULL && (set_value == RS_CREATEANDGO ||
            ...
            if(StorageTmp->snmpNotifyRowStatus == RS_ACTIVE)
            {
                SNMP_MGR_VerifyAndCreateProbeConfigDB();
            }
        } else {
            /*
