...
static void remove_double_quote_char_from_str(char *str_p);
...
#define PRIVRW	(SNMPV2ANY | 0x5000)
    ...
    snmpd_register_config_handler("group", (void (*)(const char *, char *))vacm_parse_group,
                                  vacm_free_group,
                                  "name v1|v2c|usm|... security");
    snmpd_register_config_handler("access", (void (*)(const char *, char *))vacm_parse_access,
                                  vacm_free_access,
                                  "name context model level prefx read write notify");
    snmpd_register_config_handler("view", (void (*)(const char *, char *))vacm_parse_view, vacm_free_view,
...
#ifndef VXWORKS
void vacm_parse_group(const char *token, char *param, BOOL_T is_auto_create)
#else
UI32_T vacm_parse_group(const char *token, char *param, BOOL_T is_auto_create)
#endif
{
    ...
    #ifdef VXWORKS
    storageType = strtok(NULL, " \t\n");
    if (!storageType)
    {
        config_perror("missing storageType parameter, setting storage type = nonvolatile");
        istorageType = ST_NONVOLATILE;
    }
    else
    {
        if (strcasecmp(storageType, "other") == 0)
           istorageType = ST_OTHER;
        else if (strcasecmp(storageType, "volatile") == 0)
           istorageType = ST_VOLATILE;
        else if (strcasecmp(storageType, "nonvolatile") == 0)
           istorageType = ST_NONVOLATILE;
        else if (strcasecmp(storageType, "permanent") == 0)
           istorageType = ST_PERMANENT;
        else if (strcasecmp(storageType, "readonly") == 0)
           istorageType = ST_READONLY;
        else
           istorageType = ST_NONVOLATILE;
    }
    #endif
    ...
    /* kinghong modified from SNMP_STORAGE_PERMANENT to SNMP_STORAGE_NONVOLATILE*/
    #ifdef VXWORKS
    gp->storageType = istorageType;
    #else
    ...
    free(gp->reserved);
    gp->reserved = NULL;
    gp->is_auto_destroy = is_auto_create;
...
#ifndef VXWORKS
void vacm_parse_access(const char *token, char *param, BOOL_T is_auto_create)
#else
UI32_T vacm_parse_access(const char *token, char *param, BOOL_T is_auto_create)
#endif
{
    ...
    remove_double_quote_char_from_str(readView);
    ...
    remove_double_quote_char_from_str(writeView);
    ...
    remove_double_quote_char_from_str(notify);
    ...
    /* kinghong modified from SNMP_STORAGE_PERMANENT to SNMP_STORAGE_NONVOLATILE*/
    #ifdef VXWORKS
    ap->storageType = istorageType;
    #else
    ap->storageType = ST_NONVOLATILE;
    #endif
    ap->status = SNMP_ROW_ACTIVE;
    free(ap->reserved);
    ap->reserved = NULL;
    ap->is_auto_destroy = is_auto_create;
...
vacm_parse_view(const char *token, char *param)
{
    ...
    #else
    vp->viewStorageType = istorageType;
    #endif
...
vacm_parse_simple(const char *token, char *confline)
{
    ...
    if (strcmp(token, "rwcommunity") == 0
        ...
        #else
        netsnmp_udp_parse_security("com2sec", line, 0); /*0 represent group specific*/
        #endif /*end of #ifndef VXWORKS*/
    ...
    line[ sizeof(line)-1 ] = 0;
    DEBUGMSGTL((token, "passing: %s %s\n", "access", line));
    vacm_parse_access("access", line, FALSE);
    num++;
}
...
#if(SYS_CPNT_CLUSTER==TRUE)
    /*
     * eli,The member do not check access When it receive the commander's packet.
     */
    if((SNMP_CLUSTER_GetClusterRole()==CLUSTER_TYPE_ACTIVE_MEMBER)&&(SNMP_CLUSTER_CheckFromCommanderIp( pdu)==TRUE))
    {
        return VACM_SUCCESS;
    }
#endif /*end of #if(SYS_CPNT_CLUSTER==TRUE)*/
                   ...
                   size_t * var_len, WriteMethod ** write_method)
{
    ...
    switch (vp->magic) {
    case SECURITYGROUP:
#if (SYS_CPNT_SNMPV3_INHIBIT_CLONE_USER == TRUE)
        *write_method = 0;
#else
                                     ...
                                     oid * name, size_t name_len)
{
    ...
    /*
     * Paul_huang marked, setup value type and length are only checked in RESERVE1 mode.
     */
    if(action == RESERVE1)
    {
    ...
    /*
     * [ES4649-ZZ-01429]
     * Sync from [ES3528MO-PoE-FLF-17-00014] [Problem 1]
     * Paul_huang marked, the return snmp exception message will not process in COMMIT mode.
     */
    ...
    if (action == RESERVE1) {
        ...
        if (geptr != NULL)
        {
            if (long_ret == RS_CREATEANDGO || long_ret == RS_ACTIVE)
            {
                ...
                strncpy(entry.snmp_community_index, &geptr->securityName[1], geptr->securityName[0]);
                /* end of add by kinghong*/
                ...
                /* added by kinghong to create the usmentry auto.*/
                if (( geptr->securityModel ==1 ) || (geptr->securityModel == 2))
                {
                    if (SNMP_MGR_ERROR_OK == SNMP_MGR_GetSnmpCommunityEntry(&entry))
                    {
                        if ( entry.snmp_community_status == VAL_snmpCommunityStatus_active)
                        {
                            SNMP_MGR_SnmpV3UserEntry_T user_entry;
                            memset(&entry, 0, sizeof(user_entry));
                            strcpy(user_entry.snmpv3_user_name, entry.snmp_community_index);
                            if (SNMP_MGR_CheckUsmUserEntry(&user_entry)!= SNMP_MGR_ERROR_OK)
                            {
                               ...
                               * don't need to do anything*/
                                ...
                                {
                                    ret = snmpv3_get_engineID( localEngineID, MAXSIZE_snmpEngineID);
                                    ...
                                    {
                                        memdup(& newuser->engineID, localEngineID, ret);
                                        newuser->engineIDLen = ret;
                                        memdup((u_char **)&newuser->cloneFrom,(const u_char *)zerodotzero,2*sizeof(zerodotzero));
                                        newuser->cloneFromLen =2;
                                        newuser->secName = strdup (&geptr->securityName[1]);
                                        newuser->name = strdup (&geptr->securityName[1]);
                                        newuser->groupName = strdup(geptr->groupName);
                                        newuser->userModel = geptr->securityModel;
                                        newuser->authType = SNMP_MGR_SNMPV3_AUTHTYPE_NONE;
                                        newuser->privType = SNMP_MGR_SNMPV3_PRIVTYPE_NONE;
                                        newuser->authPasswd = NULL;
                                        newuser->privPasswd = NULL;
                                        newuser->userSecLevel = VAL_vacmAccessSecurityLevel_noAuthNoPriv;
                            ...
                            u_char * statP, oid * name, size_t name_len)
{
    ...
    /*
     * [ES4649-ZZ-01429]
     * Sync from [ES3528MO-PoE-FLF-17-00014] [Problem 2]
     * Paul_huang marked, Checking the type and len of set value shouble be in RESERVE11 mode.
     */
    if (action == RESERVE1)
    {
        if (var_val_type != ASN_INTEGER)
         ...
         */
    } else if (action == ACTION)
          {
              ...
              }else if(long_ret == aptr->storageType)
                    {
                        return SNMP_ERR_NOERROR;
                    }
                    else
               ...
               oid ** subtree, size_t * subtreeLen)
{
  ...
  /*kinghong patched, since subtreeL maynot equal to oidLen- viewNameL-1, it should be
    equal to oidIndex[viewNameL+1]+1;*/
    #if 0
    subtreeL = oidLen - viewNameL - 1;  /* the initial name length */
    #endif
    subtreeL = oidIndex[viewNameL+1]+1;
    ...
    *viewNameLen = viewNameL;
    ...
    for (i = 0; i < viewNameL; i++) {
        /* kinghong patched to only allow ASCII printable input */
        /*if (oidIndex[i + 1] > 255) */
        if ((oidIndex[i+1]< 32) || ( oidIndex[i+1] > 126))
        {
            free(*viewName);
            free(*subtree);
            return SNMP_ERR_INCONSISTENTNAME;
    ...
    for (i = 0; i < subtreeL; i++) {
    	/* kinghong patched, an max oid len may exceed 255,
    	   for example, private mib accton is 259, so
    	   patch to 65535 in the current*/
	if (oidIndex[i + viewNameL + 1] > /*255*/65535)
	{
	    free(*viewName);
	    free(*subtree);
	    return SNMP_ERR_INCONSISTENTNAME;
                     ...
                     u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        /* kinghong patched here to use getexactViewEntry to fix cannot create
           equal name but different subtree issue*/

          vptr = vacm_getExactViewEntry(newViewName, newViewSubtree, viewSubtreeLen);
     /*   vptr =
            vacm_getViewEntry(newViewName, newViewSubtree, viewSubtreeLen,
                                 VACM_MODE_IGNORE_MASK);*/
