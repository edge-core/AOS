...
#ifdef VXWORKS
/* kinghong add the following API to get the address of aAddrTable*/
struct targetAddrTable_struct **
get_addrTableAddress(void)
{
    return &aAddrTable;
}
...
snmpTargetAddrTable_dispose(struct targetAddrTable_struct *reaped)
{
    ...
    SNMP_FREE(reaped);
    SNMP_MGR_AdjustSnmpV3TargetAddrCounter(FALSE);
                              ...
                              struct targetAddrTable_struct **listPtr)
{
    ...
    UI32_T  target_addr_counter;


    /*
     * Get security, source address/netmask and community strings.
     */
    SNMP_MGR_GetSnmpV3TargetAddrCounter(& target_addr_counter);


    /*kinghong added*/
    if (target_addr_counter >= SNMP_MGR_MAX_NBR_OF_TARGET_ADDR_ENTRY)
        return FALSE;

     SNMP_MGR_AdjustSnmpV3TargetAddrCounter(TRUE);
                                ...
                                struct targetAddrTable_struct **listPtr)
{
    ...
    netsnmp_session *oldsess = NULL;
    netsnmp_session *psess = NULL;
    /* kinghong patched to remove the target session*/
    oldsess = oldEntry->sess;
    if (oldEntry->sess != NULL)
    {
        snmp_close_sess_only(oldEntry->sess);
        oldEntry->sess = NULL;
    }
    if ((tptr = *listPtr) == 0)
        return;
    ...
    /* eli,2005.3.17,[ES4649-ZZ-01132] patched to set sess->next which refer the target session to NULL,because it have removed*/
    tptr = *listPtr;
    while (tptr!=NULL)
    {
        psess= tptr->sess;
                        ...
                        size_t * var_len, WriteMethod ** write_method)
{
    ...
    static char     string[255];/*kinghong modify 1500 to 255*/
    static oid      objid[32]; /* kinghong modify 128 to 32 */
                             ...
                             u_char * statP, oid * name, size_t name_len)
{
    ...
    UI8_T trapIpAddress[SYS_ADPT_IPV6_ADDR_LEN];
    ...
    if (action == RESERVE1) {
        ...
        memset(&trapIpAddress, 0, sizeof(trapIpAddress));

        if (var_val_len <= (SYS_ADPT_IPV4_ADDR_LEN + SNMP_MGR_TADDR_PORT_LEN))
        {
            memcpy(&trapIpAddress, var_val, SYS_ADPT_IPV4_ADDR_LEN);

            if (!IP_LIB_IsValidForNetworkInterface(trapIpAddress))
            {
        ...
        {
            memcpy(&trapIpAddress, var_val, SYS_ADPT_IPV6_ADDR_LEN);
            ret = IP_LIB_CheckIPv6PrefixForInterface(trapIpAddress, 128);
            if (ret != 0)
            {
                return SNMP_ERR_WRONGVALUE;
            }
...
#if 0    /* allowing set same ip address */    
        /* kinghong patched to not allowing the same IP address Setting in the row*/
        while ( SNMP_MGR_GetNextSnmpTargetAddrTable(&target_addr_entry) == SNMP_MGR_ERROR_OK)
        {
            UI32_T target_add_ip;
            memcpy( &target_add_ip, target_addr_entry.snmp_target_addr_taddress, 4);
            if (target_add_ip == trapIpAddress)
            {
                /* same IP address found, return FALSE*/
                return SNMP_ERR_INCONSISTENTVALUE;
            }
        }
#endif
                            ...
                            u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        if ((long_ret < SYS_ADPT_MIN_SNMP_TARGET_ADDR_TIMEOUT) ||
            (long_ret > SYS_ADPT_MAX_SNMP_TARGET_ADDR_TIMEOUT))
        {
            return SNMP_ERR_WRONGVALUE;
        }
                            ...
                            u_char * statP, oid * name, size_t name_len)
{
    ...
    if (action == RESERVE1) {
        ...
        if (!snmpTagListValid((const char *)var_val, var_val_len)) {
            return SNMP_ERR_WRONGVALUE;
        }
    } else if (action == RESERVE2) {
        snmpTargetAddrOID[snmpTargetAddrOIDLen - 1] =
    ...
    if (action == RESERVE1) {
        ...
        if (target != NULL) {
            ...
            if (value == RS_NOTINSERVICE) {
                if (target->sess != NULL) {
                    snmp_close_sess_only(target->sess);
                    target->sess = NULL;
                }
            }
        }
