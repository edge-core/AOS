                    ...
                    u_char * statP, oid * name, size_t name_len)
{
     ...
     */
    static long     long_ret;
    /* kinghong patched to init the *engineID to NULL*/
    unsigned char  *engineID=NULL;
    size_t          engineIDLen;
    /* kinghong patched to init the *newName to NULL*/
    char           *newName=NULL;
    size_t          nameLen;
    struct usmUser *uptr = NULL;
    ...
    if (action == RESERVE1) {
        ...
        if (uptr != NULL) {
            if (long_ret == RS_DESTROY) {
                 ...
                 *usm_free_user(uptr);
                 */
                /*kinghong, 2004/5/12 we moved the delete action into the snmp_mgr, but
                 *if we cannot delete through mgr, we delete here
                 */
                ...
                if (SNMP_MGR_DeleteSnmpRemoteUser(&entry) != SNMP_MGR_ERROR_OK)
                {
                    usm_remove_user(uptr);
                    usm_free_user(uptr);
                }
        ...
        usm_parse_oid(&name[USM_MIB_LENGTH], name_len - USM_MIB_LENGTH,
                      &engineID, &engineIDLen, (u_char **) & newName,
                      &nameLen);
         /* kinghong patched to prevent crashed, if engineID is NULL.*/
        if (engineID && newName)
        {
            uptr = usm_get_user(engineID, engineIDLen, newName);
        }
