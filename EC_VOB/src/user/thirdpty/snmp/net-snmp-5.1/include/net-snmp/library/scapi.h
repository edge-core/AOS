 ...
 * define for 3DES
 */
#define SNMP_TRANS_PRIVLEN_3DES         256
#define SNMP_TRANS_PRIVLEN_3DES_IV      192
...
/*
 *the original is defined 192, but the max is 128 in snmpusm
 */
...
/*
 *the original is defined 256, but the max is 128 in snmpusm
 */
...
#define SNMP_TRANS_AES_AES256_PADSIZE   128
    ...
    int             sc_get_properlength(const oid * hashtype,
                                        u_int hashtype_len);
    /*
     * added for snmpv3 AES sync. from netsnmp ver.5.7.1
     */
    int             sc_get_priv_properlength(const oid * hashtype,
                                        u_int hashtype_len);
    int             sc_init(void);
    int             sc_shutdown(int majorID, int minorID, void *serverarg,
                                void *clientarg);

    int             sc_random(u_char * buf, unsigned long * buflen);
    ...
    int             sc_check_keyed_hash(const oid * authtype,
                                        unsigned long authtypelen, u_char * key,
                                        u_int keylen, u_char * message,
                                        u_int msglen, u_char * MAC,
                                        u_int maclen);

    int             sc_encrypt(const oid * privtype, unsigned long privtypelen,
                               u_char * key, u_int keylen,
    ...
    int             sc_hash(const oid * hashtype, unsigned long hashtypelen,
                            u_char * buf, unsigned long buf_len,
                            u_char * MAC, unsigned long * MAC_len);
