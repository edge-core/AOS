 ...
 *---------------------------------------------------------------------------
 * PURPOSE:  This function will produce  trap_list for sending trap to commamander for cluster.
 ...
 *---------------------------------------------------------------------------
 */
int SNMP_API_SendTrapToCommander(netsnmp_pdu *template_pdu,netsnmp_session *trap_v_session);
    #endif /*end of #if(SYS_CPNT_CLUSTER==TRUE)*/

/* kinghong added the below API to close the session only*/
int snmp_close_sess_only(netsnmp_session * session);
