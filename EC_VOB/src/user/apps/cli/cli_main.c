...
#include "bdpv_mgr.h"
#endif /* SYS_ADPT_CLI_BACKDOOR_PASSWORD_COMPONENT */
...
static BOOL_T CLI_MAIN_RemoveQuoteMark(Token_T *token_p, UI32_T data_type);
static BOOL_T (*CLI_MAIN_RemoveQuoteMark_fn_p)(Token_T *token_p, UI32_T data_type) = CLI_MAIN_RemoveQuoteMark;
static int (*Check_Arg_By_Data_Type_fn_p) (Token_T *token, UI16_T aidx, UI16_T *match_len, Arg_Ref_T *out) = Check_Arg_By_Data_Type;

static BOOL_T CLI_MAIN_IsValidCommandLineString(const char* cmd_line, const char **err_ptr);
...
*/
    ...
    CLI_LIB_SendLoginLogoutTrap(TRAP_EVENT_LOGIN,
        ctrl_P->CMenu.RemoteSessionType,
        ctrl_P->CMenu.UserName,
        &ctrl_P->CMenu.TelnetIPAddress);

    exit_code = CLI_MAIN_Monitor(ctrl_P);

    CLI_LIB_SendLoginLogoutTrap(TRAP_EVENT_LOGOUT,
        ctrl_P->CMenu.RemoteSessionType,
        ctrl_P->CMenu.UserName,
        &ctrl_P->CMenu.TelnetIPAddress);
    ...
    while (1)
    {
...
#if (SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG == TRUE)
            else if (ctrl_P->sess_type == CLI_TYPE_PARTIAL_PROV)
            {
                ret_config_file = CLI_MAIN_ReadLine_From_XferBuf(cmd_buf, &tmp_xbuf, ctrl_P);
                ...
                {
                    if (TRUE == CLI_MGR_ADDRUNCONFIG_IsTriggerByAddRunconfigTask())
                    {
                        CLI_MGR_ADDRUNCONFIG_Done(tmp_xbuf);
                    ...
                    else
                    {
                        ...
                        {
                            XFER_PMGR_SetPartialProvisionStatus(XFER_MGR_FILE_COPY_SUCCESS);
                        }
                    }
        ...
        */
        ...
        if ( (err == CLI_NO_ERROR) || (err == CLI_ERR_CMD_ARG_AMBIGUOUS) )
        {
            ...
            idx = out.err_ptr - arg_buf  /*- (2 * slash_count)*/ ;  /* cjchu 2001/8/29 */
        ...
        */
        ...
        /* When user press [a] to display, interactive flag will be FALSE
         * should recover interactive flag here.
         */
                     ...
                     UI16_T *cmd_preidx, UI16_T *str_testidx, char *arg_buf_temp)
{
    ...
    if (TRUE != CLI_MAIN_IsValidCommandLineString((const char*) buff, &err_ptr))
    {
        *cmd_idx = cmd_eidx;
        *str_idx = err_ptr - buff;
        *cmd_preidx = cmd_eidx;
...
// dralee 99.1.26
   ...
   if (err > 0)
...
#if (SYS_CPNT_AUTHORIZATION_COMMAND == TRUE)
    if (FALSE == CLI_LIB_IsProvision(ctrl_P))
    {
        memset(&author_request, 0, sizeof(author_request));
        ...
        strncpy(author_request.user_name, ctrl_P->CMenu.UserName, sizeof(author_request.user_name)-1);
        author_request.user_name[ sizeof(author_request.user_name)-1 ] = '\0';
        author_request.client_type = AAA_CLIENT_TYPE_COMMANDS;
        author_request.authen_by_whom = AAA_AUTHEN_BY_UNKNOWN;

        author_request.command_privilege = ui_cmd_lst[cmd_idx].current_privilege;  /* command privilege level */
        strncpy(author_request.command, buff, sizeof(author_request.command)-1);
        author_request.command[ sizeof(author_request.command)-1 ] = '\0';
        ...
        {
            CLI_LIB_PrintStr("Command authorization failed.\r\n");
            return CLI_NO_ERROR;
        }
    ...
    if (FALSE == CLI_LIB_IsProvision(ctrl_P))
    {
       /*call accounting start*/
        ...
        {
            aaa_request.ifindex = CLI_TASK_GetMySessId() - CLI_TASK_MIN_TELNET_SESSION_ID + 1;
            aaa_request.rem_ip_addr = ctrl_P->CMenu.TelnetIPAddress;
        }

        strncpy(aaa_request.user_name, ctrl_P->CMenu.UserName, sizeof(aaa_request.user_name)-1);
        aaa_request.user_name[ sizeof(aaa_request.user_name)-1 ] = '\0';
        aaa_request.client_type = AAA_CLIENT_TYPE_EXEC;
        aaa_request.request_type = AAA_ACC_CMD_START;

        aaa_request.command_privilege = ui_cmd_lst[cmd_idx].current_privilege;  /* command privilege level */
        strncpy(aaa_request.command, buff, sizeof(aaa_request.command)-1);
        aaa_request.command[ sizeof(aaa_request.command)-1 ] = '\0';
   ...
   ui_cmd_lst = CLI_LIB_Get_Cmd_List();
    ...
    {
        /*call accounting end*/
        aaa_request.request_type = AAA_ACC_CMD_STOP;
...
static UI16_T CLI_MAIN_ProcessQuType3(char *buff, UI16_T cmd_eidx, UI16_T arg_eidx, UI16_T str_idx, UI16_T err)
{
    ...
    char  **words;   /* pointer array for showing the matched word */
    char  word_buf[CLI_DEF_MAX_WORDSIZE+1];
    UI16_T word_len;
    ...
    memset(word_buf, 0, sizeof(UI8_T)*(CLI_DEF_MAX_WORDSIZE+1));
    ...
    if((words = (char **)calloc(CLI_DEF_MAX_CMD_NUM, sizeof(char  *))) == NULL)
    {
        return CLI_NO_ERROR;
    }
        ...
        */
        ...
        else
        {
            if( err == CLI_ERR_CMD_INCOMPLETE )
            {
                ...
                {
                case WORD_DATA:
                    SYSFUN_Sprintf(name_string, "<%lu-%lu>", (unsigned long)ui_arg_lst[arg_eidx].min_range, (unsigned long)ui_arg_lst[arg_eidx].max_range);
                    words[widx++] = name_string;
                    break;

                case HEX_DATA:
                    SYSFUN_Sprintf(name_string, "<%lx-%lx>", (unsigned long)ui_arg_lst[arg_eidx].min_range, (unsigned long)ui_arg_lst[arg_eidx].max_range);
                    words[widx++] = name_string;
                    break;
                ...
                case UNIT_COLON_DATA:/*pttch stacking*/
                    ...
                    //else
                    //{
                    //   SYSFUN_Sprintf(name_string, "%lu:", (unsigned long)atoi(buff));
                    //}
                    words[widx++] = name_string;
                    break;

                case RANGE_DATA:
                    SYSFUN_Sprintf(name_string, "<%lu-%lu>        The range of values, by using hyphen",(unsigned long)ui_arg_lst[arg_eidx].min_range, (unsigned long)ui_arg_lst[arg_eidx].max_range);
                    words[widx++] = name_string;
                    break;
                ...
                case LIST_HAVE_UNIT_DATA:
                    ...
                    else
                    {
                        ...
                        ptr++;
                        SYSFUN_Sprintf(name_string, "<1-%ld>   The list of the ports, by using hyphen and comma",(long)SWCTRL_POM_UIGetUnitPortNumber(atoi((char *)ptr)));
                    }
                    words[widx++] = name_string;
                    break;

                case LIST_NO_UNIT_DATA:
                    SYSFUN_Sprintf(name_string, "<%lu-%lu> The list of the values, by using hyphen and comma",(unsigned long)ui_arg_lst[arg_eidx].min_range,(unsigned long)ui_arg_lst[arg_eidx].max_range);
                    words[widx++] = name_string;
                    break;
                ...
                case PVC_DATA:
                    {
                        ...
                        else
                        {
                            ...
                            SYSFUN_Sprintf(tmp_buf1, "%lu>", (unsigned long)SWCTRL_POM_UIGetUnitPortNumber(atoi(unit_ptr)));
            ...
            else
            {
                ...
                case MINUS_DATA:
                    {
                        ...
                        min_range = ui_arg_lst[arg_eidx].min_range;
                        max_range = ui_arg_lst[arg_eidx].max_range;
                        SYSFUN_Sprintf(name_string, "<%ld-%ld>", (long)min_range, (long)max_range);
                        words[widx++] = name_string;
                    }
                    break;
                ...
                case RANGE_DATA:
                    SYSFUN_Sprintf(name_string, "<%lu-%lu>        The range of value, by using hyphen",(unsigned long)ui_arg_lst[arg_eidx].min_range, (unsigned long)ui_arg_lst[arg_eidx].max_range);
                    words[widx++] = name_string;
                    break;
                ...
                case LIST_HAVE_UNIT_DATA:
                    {
                        ...
                        ptr++;
                        SYSFUN_Sprintf(name_string, "<1-%lu>     The list of the port, by using hyphen and comma", (unsigned long)SWCTRL_POM_UIGetUnitPortNumber(atoi((char *)ptr)));
                        words[widx++] = name_string;
                    }
                    break;

                case LIST_NO_UNIT_DATA:
                    SYSFUN_Sprintf(name_string, "<%lu-%lu> The list of the value, by using hyphen and comma",(unsigned long)ui_arg_lst[arg_eidx].min_range,(unsigned long)ui_arg_lst[arg_eidx].max_range);
                    words[widx++] = name_string;
                    break;
                ...
                case PVC_DATA:
                    {
                        ...
                        else
                        {
                            ...
                            SYSFUN_Sprintf(tmp_buf, "%lu>", (unsigned long)SWCTRL_POM_UIGetUnitPortNumber(atoi(unit_ptr)));
...
static BOOL_T
CLI_MAIN_RemoveQuoteMark(
    Token_T *token_p,
    UI32_T data_type)
{
    char temp_buff[CLI_DEF_MAX_BUFSIZE + 1] = {0};
    ...
    memcpy(temp_buff,token_p->p,token_p->len);

    if (CLI_LIB_Check_Is_QuoteStr(temp_buff))
    {
        CLI_LIB_Str_Remove_Quote(temp_buff);
        token_p->len = strlen((char *)temp_buff);
        memcpy(token_p->p,temp_buff,token_p->len);
        token_p->p[token_p->len] = ' ';
    }
#endif
...
#define LISTDATA_INVALID_RANGE 2
                ...
                out->args [ /*ui_arg_lst[aidx].pos*/out->arg_position ] = tokens->p;
                out->arg_position++;
#if (CLI_SUPPORT_QUOTE_STRING_FEATURE == TRUE)

                if (ui_arg_lst[aidx].type == LINE_DATA)
                {
                    UI32_T pos = --out->arg_position;
                    int token_length = 0;
                ...
                out->ret_aidx = aidx;
                aidx = NULL_ARG;
                out->args_full[out->arg_position] = out->args[out->arg_position];
                return TRUE;
            }
            else
            {
        ...
        {
            /*  With gcc optimized option enabled, gcc would always inline
             *  "CLI_MAIN_RemoveQuoteMark"if "Parse_Arguments" direct call it.
             *  Hence, use function pointer to prevent gcc to inline them.
             */
            CLI_MAIN_RemoveQuoteMark_fn_p(tokens, ui_arg_lst[aidx].type);
        }
#endif
        ...
        /*  With gcc optimized option enabled, gcc would always inline
         *  "Check_Arg_By_Data_Type"if "Parse_Arguments" direct call it.
         *  Hence, use function pointer to prevent gcc to inline them.
         */
        ret_value = Check_Arg_By_Data_Type_fn_p (tokens, aidx, (UI16_T *)&match_len, out);
        if(ret_value != ARG_NOMATCH)
        {
            switch(ui_arg_lst[aidx].type)
            {
                ...
                case BGP_EXT_COMMUNITY_IP_DATA:
                case BGP_EXT_COMMUNITY_ASN_DATA:
                    out->args_full[out->arg_position] = tokens->p;
                    break;
                case NO_DATA:
                case COLON_DATA:
                    out->args_full[out->arg_position] = ui_arg_lst[aidx].name;
                     break;
                ...
                case MODE_DATA:
                {
                    ...
                    if (_arg)
                    {
                        ...
                        if (TRUE == CLI_LIB_GetCommandModeStringPtr(_mode, &_mode_str, &_help_msg))
                        {
                            out->args_full[out->arg_position] = _mode_str;
                        }
...
#if (CLI_SUPPORT_QUOTE_STRING_FEATURE == TRUE)
        if((quot_flag == TRUE) && (out->err_ptr != 0) && (ui_arg_lst[aidx].type == CHAR_DATA))
        {
            int i;
        ...
        out->ret_aidx = aidx;
        out->args [ out->arg_position/*ui_arg_lst[aidx].pos*/ ] = tokens->p;
        out->arg_position++;
    ...
    *match_len = 0;        /* no match */
    ...
    switch (ui_arg_lst[aidx].type)
    {
        ...
        case MINUS_DATA:
        {
            ...
            if ((value < 0 && value < (I32_T)ui_arg_lst[aidx].min_range) ||
                (value > 0 && value > ui_arg_lst[aidx].max_range))
            {
                if (out->err_ptr == 0)
                {
                    out->err_ptr = token->p;
        ...
        case LIST_HAVE_UNIT_DATA:
        {
            ...
            CLI_MGR_SaveCmdInfo(atoi(token->p));
        ...
        case PVC_DATA:
        {
            ...
            else
            {
                ...
                /* check value: right */
                if((isdigit(dor_ptr[1])==0) || atoi((dor_ptr+1))> SWCTRL_POM_UIGetUnitPortNumber(atoi(token->p)) || atoi((dor_ptr+1))< (ui_arg_lst[aidx].min_range & 0x0000FFFF))
                {
                    out->err_ptr= (token->p+mid_ptr+1);
                    out->err_kind = CLI_ERR_CMD_INVALID_RANGE_PLUS;
                    return ARG_NOMATCH;
        ...
        case PORT_DATA:
        {
            ...
            else
            {
                ...
                /* check value: right */
                if((isdigit(dor_ptr[1])==0) ||atoi((dor_ptr+1))> (ui_arg_lst[aidx].max_range & 65535 ) || atoi((dor_ptr+1))< (ui_arg_lst[aidx].min_range & 0x0000FFFF))
                {
                    out->err_ptr = (token->p);
                    out->err_kind = CLI_ERR_CMD_INVALID_RANGE_PLUS;
                    return ARG_NOMATCH;
        ...
        case IP_ADDR_DATA : /* {ipv4|ipv6}[%zone_id] */
        {
            if ((sizeof(ip_str) - 1) < token->len)
            {
                out->err_ptr = (token->p);
                out->err_kind = CLI_ERR_CMD_INVALID;
                return ARG_NOMATCH;
            ...
            memcpy(ip_str, token->p, token->len);
            ip_str[token->len] = '\0';
        ...
        case IPV4_ADDR_DATA: /* ipv4[%zone_id] */
        {
            if (L_INET_MAX_IP4ADDR_STR_LEN < token->len)
            {
                out->err_ptr = (token->p);
                out->err_kind = CLI_ERR_CMD_INVALID;
                return ARG_NOMATCH;
        ...
        case IPV6_ADDR_DATA: /* ipv6[%zone_id] */
        {
            if (L_INET_MAX_IP6ADDR_STR_LEN < token->len)
            {
                out->err_ptr = (token->p);
                out->err_kind = CLI_ERR_CMD_INVALID;
                return ARG_NOMATCH;
            ...
            if (L_INET_RETURN_SUCCESS != L_INET_StringToInaddr(L_INET_FORMAT_IPV6_ADDR,
                                                               ip_str,
                                                               (L_INET_Addr_T *) &inaddr,
                                                               sizeof(inaddr)))
            {
        ...
        case IPV6_ADDR_DATA_WITH_SCOPE_ID: /* ipv6%zone_id */
        {
            if (L_INET_MAX_IP6ADDR_STR_LEN < token->len)
            {
                out->err_ptr = (token->p);
            ...
            if (L_INET_RETURN_SUCCESS != L_INET_StringToInaddr(L_INET_FORMAT_IPV6_LINK_LOCAL,
                                                               ip_str,
                                                               (L_INET_Addr_T *) &inaddr,
                                                               sizeof(inaddr)))
            {
        ...
        case IPV6_LINK_LOCAL_ADDR_DATA_NO_SCOPE_ID: /* ipv6(link local) */
        {
            if (L_INET_MAX_IP6ADDR_STR_LEN < token->len)
            {
                out->err_ptr = (token->p);
            ...
            if (L_INET_RETURN_SUCCESS != L_INET_StringToInaddr(L_INET_FORMAT_IPV6_LINK_LOCAL_NO_SCOPE_ID,
                                                               ip_str,
                                                               (L_INET_Addr_T *) &inaddr,
                                                               sizeof(inaddr)))
            {
        ...
        case IPV6_ADDR_RAW_DATA: /* ipv6(global | link-local)[/prifix] */
        {
            if (L_INET_MAX_IP6ADDR_STR_LEN < token->len)
            {
                out->err_ptr = (token->p);
        ...
        case IPV6_PREFIX_DATA: /* ipv6/prefix_length */
        {
            if (L_INET_MAX_IP6ADDR_STR_LEN < token->len)
            {
                out->err_ptr = (token->p);
...
void CLI_MAIN_Enter_DebugMode()
{
   ...
   /* Only admin user can use backdoor
    */
    ctrl_P = CLI_TASK_GetMyWorkingArea();
    if (ctrl_P->CMenu.AccessPrivilege < USERAUTH_ADMIN_USER_PRIVILEGE)
    {
        return;
    }

#ifdef SYS_ADPT_CLI_BACKDOOR_PASSWORD_COMPONENT
    slength = strlen(backDoorStringCLI);
    ...
    slength = sizeof(key_in_password)-1;
    ...
    if (TRUE != BDPV_MGR_Validate((const char *)key_in_password))
    {
        return;
    }
...
static void cli_main_parse_file_header(char *token)
{
    ...
    if (strcmp(type_str, "stackingDB") == 0)
    {
        ...
        for(unit_id = 0 ;unit_id <SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK ; unit_id++)
        {
            ...
            //printf("stacking DB: unit = %lu, module type = %lu\n", (unsigned long)unit_id, (unsigned long)module_types[unit_id]);
        }
...
#if (SYS_CPNT_UNIT_HOT_SWAP == TRUE)
    /* It must not be in Ethernet mode now so that we reset the flag.
     */
...
#if (SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG == TRUE)
    if (CLI_TYPE_PARTIAL_PROV == ctrl_P->sess_type)
    {
        CLI_LIB_PrintStr("\r\nPerforming partial provision ...\r\n");
    }
#endif  /* #if (SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG == TRUE) */
...
void CLI_MAIN_HandleMGR_Thread(UI32_T ticks, UI8_T ch, UI8_T * buff)
{
    ...
    SYSFUN_MsgQ_T        cli_main_ipc_msgq_handle;
    BOOL_T               need_resp;
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME : CLI_MAIN_HandleIPCIOMsg
 *------------------------------------------------------------------------------
 * PURPOSE:
 *    This function will handle SYS_TYPE_CMD_IPCIO msg
 *
 * INPUT:
 *    msg_p
 *
 ...
 *
 * RETURN:
 *    need to send response or not.
 *
 * NOTES:
 *    None.
 *------------------------------------------------------------------------------
 */
BOOL_T CLI_MAIN_HandleIPCIOMsg(SYSFUN_Msg_T *msg_p)
{
    L_IPCIO_Msg_T *l_ipcio_msg_p = (L_IPCIO_Msg_T *)msg_p->msg_buf;
    BOOL_T need_resp = FALSE;

    switch (l_ipcio_msg_p->ipcio_user_info.user_id)
    {
#if (SYS_CPNT_DEBUG == TRUE)
        case SYS_MODULE_DEBUG:
            switch (l_ipcio_msg_p->ipcio_cmd)
            {
                /* user_ext_id: UI32_T sess_id_bmp
                 */
                case L_IPCIO_CMD_PRINT:
                    {
                        UI32_T sess_id_bmp = l_ipcio_msg_p->ipcio_user_info.user_ext_id;
                        UI32_T sess_id;

                        for (sess_id = 0; sess_id_bmp > 0; sess_id++, sess_id_bmp >>= 1)
                        {
                            if (!(sess_id_bmp & 0x1))
                            {
                                continue;
                            ...
                            CLI_IO_DebugMode_Print((void *)(uintptr_t)sess_id, l_ipcio_msg_p->ipcio_arg_buffer);
                        }
                    }
                    break;
        ...
        case SYS_MODULE_BACKDOOR:
            need_resp = BACKDOOR_MGR_HandleIPCIOMsg(SYS_BLD_CLI_GROUP_IPCMSGQ_KEY, msg_p);
            break;
...
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MAIN_IsValidCommandLineString
 * ------------------------------------------------------------------------
 * PURPOSE  : Check the input string is a valid command line string
 *            (0x20-0x7E, except characters defines in SYS_ADPT_CMNLIB_ASCII_PRINT_EXCEPT_CHAR)
 * INPUT    : cmd_line  - input string
 * OUTPUT   : err_ptr   - pointer to error char of the input string if error; else last char '\0' of input string
 * RETURN   : TRUE if an valid command line; else return FALSE.
 * NOTE     : None.
 * ------------------------------------------------------------------------
 */
static BOOL_T CLI_MAIN_IsValidCommandLineString(const char* cmd_line, const char **err_ptr)
{
    ...
    for (*err_ptr = cmd_line; **err_ptr != 0; (*err_ptr)++)
    {
        if (!L_STDLIB_IsAsciiPrint(**err_ptr) ||
            (strchr(SYS_ADPT_CMNLIB_ASCII_PRINT_EXCEPT_CHAR, **err_ptr) != NULL))
        {
            return FALSE;
        }
    }
