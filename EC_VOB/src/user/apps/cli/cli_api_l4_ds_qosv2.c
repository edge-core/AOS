...
#define ERR_MSG_NOT_ALLOW_MIX_SELECTOR      "Failed to set match criteria, because a class map does not allow mixing MAC, IPv4, IPv6 extended, and IPv6 standard rules together.\r\n"
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - CLI_DS_STRING_StrNCpy
 *------------------------------------------------------------------------------
 ...
 * RETURN:   CLI_DS_STRING_E_INSUFFICIENT_BUFFER - dst buffer is too small to
 ...
 *
 *           assert(res == CLI_DS_STRING_E_INSUFFICIENT_BUFFER);
...
#endif /* End of #if (SYS_CPNT_QOS_UI == TRUE)*/
...
#endif /* End of (SYS_CPNT_QOS == SYS_CPNT_QOS_DIFFSERV) */
#endif /* End of #if (SYS_CPNT_QOS_UI == TRUE) */
    ...
    while ( L4_PMGR_QoS_GetNextElementFromClassMap ( cmap_id, &cmap_eid, &rec ) == RULE_TYPE_OK )
    {
        ...
        else if( rec.class_type == RULE_TYPE_CLASS_MF )
        {
            ...
            case RULE_TYPE_MF_PRECEDENCE:
                /* ip precedence */
                sprintf(buff, " Match IP Precedence %lu\r\n", (unsigned long)rec.element.mf_entry.mf_value );
                PROCESS_MORE_FUNC ( buff );
                break;
            ...
            case CONFIRM_ACTION_REMARK_DSCP_TRANSMIT:
                {
                    ...
                    SYSFUN_Snprintf(dscp, sizeof(dscp), " %lu", (unsigned long)pmap_element ->action_entry.confirm_action_value);
            ...
            case EXCEED_ACTION_REMARK_DSCP_TRANSMIT:
                {
                    ...
                    SYSFUN_Snprintf(dscp, sizeof(dscp), " %lu", (unsigned long)pmap_element ->action_entry.exceed_action_value);
                    dscp[sizeof(dscp)-1] = '\0';
            ...
            case VIOLATE_ACTION_REMARK_DSCP_TRANSMIT:
                {
                    ...
                    SYSFUN_Snprintf(dscp, sizeof(dscp), " %lu", (unsigned long)pmap_element->action_entry.violate_action_value);
                    dscp[sizeof(dscp)-1] = '\0';
    ...
    char *cmap_name)
{
    ...
    {		
				...
				ifindex, direction, pmap_id, cmap_id, &counter))
        {
             ...
             sprintf(buff, "%s    Receive Packets: %10lu \r\n", buff, (unsigned long)counter.receive_packets);
             sprintf(buff, "%s       Drop Packets: %10lu \r\n", buff, (unsigned long)counter.drop_packets);
...
UI32_T CLI_API_Show_Policy_Map_Interface(UI16_T cmd_idx, char *arg[], CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    if (arg[arg_pos][0] == 'p' || arg[arg_pos][0] == 'P')
    {
        arg_pos ++;
    ...
    else if (arg[arg_pos][0] == 'e' || arg[arg_pos][0] == 'E')
    {
        arg_pos ++;
    ...
    if (NULL != arg[arg_pos])
    {
        ...
        else if ( (arg[arg_pos][0] == 'o') || (arg[arg_pos][0] == 'O') )
        {
            direction = RULE_TYPE_OUTBOUND;
        }
        else
    ...
    if (RULE_TYPE_BOTH_DIRECTION == direction || RULE_TYPE_INBOUND == direction)
    {
        if ( L4_PMGR_QoS_GetPolicyMapNameByPort(ifindex, RULE_TYPE_INBOUND, pmap_name ) == RULE_TYPE_OK )
        {
            sprintf(buff, "Service-policy %s \r\n", pmap_name );
            PROCESS_MORE( buff );
        }
    ...
    if (RULE_TYPE_BOTH_DIRECTION == direction || RULE_TYPE_OUTBOUND == direction)
    {
        if ( L4_PMGR_QoS_GetPolicyMapNameByPort(ifindex, RULE_TYPE_OUTBOUND, pmap_name ) == RULE_TYPE_OK )
        {
            sprintf(buff, "Service-policy %s \r\n", pmap_name );
            PROCESS_MORE( buff );
        }
...
static UI32_T Show_one_interface_cos_egressdscp2cos(UI32_T lport,UI32_T line_num)
{
    ...
    for (phb=0; phb<=MAX_PHB_VAL; phb++)
    {
        CLI_LIB_PrintStr_1(" %lu  :                    ",(unsigned long)phb);
        for (color=0;color<=MAX_COLOR_VAL;color++)
        {
            ...
            {
                 CLI_LIB_PrintStr_2(" (%lu,%lu)      ", (unsigned long)cos,(unsigned long)cfi);
            }
            else
            {
#if (SYS_CPNT_EH == TRUE)
    ...
    for(;;)
    {
        ...
        if(lport== interface)
        {
            if(protocol == tcp_protocol)
                {
                CLI_LIB_PrintStr_1(" (tcp,%-5lu)                       ",(unsigned long)port);
                }
            else if (protocol == udp_protocol)
                {
                CLI_LIB_PrintStr_1(" (udp,%-5lu)                       ",(unsigned long)port);
                }
            CLI_LIB_PrintStr_2("%-8lu%lu\r\n",(unsigned long)phb,(unsigned long)color);
        }
    }
    return line_num;
}
