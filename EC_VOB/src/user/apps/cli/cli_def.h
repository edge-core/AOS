...
#define CLI_DEF_MAX_WORDSIZE    100     /* one command word max size */
#define CLI_DEF_MAX_ARGS_NUM    20      /* maximum argument number   */
/*jingyan zheng defect ES3628BT-FLF-ZZ-00066*/
#define CLI_DEF_MAX_CMD_NUM     100      /* maximum command/argument number matched when type ? command*/
...
/*92.03.27 pttch add for disconnect session*/
  ...
  /*terminal setting*/
   ...
   UI32_T  unit_id;
   UI8_T   port_id_list[SYS_ADPT_NBR_OF_BYTE_FOR_1BIT_UPORT_LIST];                       /* port id list bit map */
   ...
   UI32_T bgp_as_number;
#endif
#if (SYS_CPNT_NSM_POLICY == TRUE)
   char route_map_name[SYS_ADPT_MAX_ROUTE_MAP_NAME_LENGTH +1];
   BOOL_T route_map_is_permit;
   UI32_T route_map_pref_index;
#endif
...
typedef struct
{
    BOOL_T unit_cmd_flag;   /* The command is unit command (and unit is valid) or not. */
    BOOL_T eth_mode_flag;   /* It is in Ethernet mode or not. */
    UI32_T unit_id;         /* The unit id specified in the command. */
} CLI_CmdInfo_T;
...
#if (SYS_CPNT_UNIT_HOT_SWAP == TRUE)
    /* For saving the commands of the units into device cfg when provision.
     */
    CLI_CmdInfo_T cmd_info;
#endif /* #if (SYS_CPNT_UNIT_HOT_SWAP == TRUE) */

    BOOL_T xfer_in_prog;     /* xfer in progress flag */
    BOOL_T xfer_prog_result; /* xfer progress result  */
...
#if (SYS_CPNT_HASH_SELECTION == TRUE)
    PRIVILEGE_CFG_HASH_SELECTION_MAC_MODE,
    PRIVILEGE_CFG_HASH_SELECTION_IPV4_MODE,
    PRIVILEGE_CFG_HASH_SELECTION_IPV6_MODE,
#endif
    DEBUG_MODE,
