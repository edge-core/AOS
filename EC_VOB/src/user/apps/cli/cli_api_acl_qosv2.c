...
#if ( (SYS_CPNT_DAI == TRUE) && (SYS_CPNT_ACL_UI == TRUE) )
static UI32_T
CLI_ACL_FillARP(
    RULE_TYPE_UI_Ace_Entry_T *ace_p,
    char *arg[]
);
#endif /* (SYS_CPNT_DAI == TRUE) && (SYS_CPNT_ACL_UI == TRUE) */
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
static UI32_T
CLI_ACL_TIME_RANGE_GetDayOfWeek (
    char *day
);

static UI32_T
CLI_ACL_TIME_RANGE_GetMonth (
    char *day
);

static UI32_T
CLI_ACL_TIME_RANGE_FillTimeRangeIndex(
	char *arg[],
    UI16_T *p_time_range_index
);

static void
CLI_ACL_TIME_RANGE_FillPeriodicTime(
    char *arg[],
    RULE_TYPE_TIME_ACL_PERIODIC_TIME_T *periodic
);

static void CLI_ACL_TIME_RANGE_FillAbsoluteTime(
    char *arg[],
    RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T *absolute
);
...
void CLI_ACL_Check_AnyIPV6 (char *buff, UI8_T *ipv6, UI32_T pfx_len)
{
    ...
    else
    {
        ...
        {   /* ipv6-adr/prefix-len */
            ...
            sprintf(ipv6_addr_str, "/%lu", (unsigned long)pfx_len);
            strcat(buff,  ipv6_addr_str);
        }
    }
#endif
...
#endif /* end of #if ( (SYS_CPNT_DAI == TRUE) && (SYS_CPNT_ACL_UI == TRUE) ) */
...
UI32_T CLI_API_Clear_AccessList_Hardware_Counters(UI16_T cmd_idx, char *arg[], CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    /*if it doesn't assign a interface, it means global*/
    ...
    while (NULL != arg[arg_index])
    {
        ...
        else if (CLI_NO_ERROR == CLI_ACL_PrivFillGlobal(&tmp_idx, (const char **) arg) &&
                 tmp_idx != arg_index)
        {
    ...
    UI32_T line_num)
{
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
    char  time_range_string[SYS_ADPT_MAX_LENGTH_OF_TIME_ACL_TABLE_NAME+15] = {0};
    ...
    switch(type)
    {
        ...
        case RULE_TYPE_IP_EXT_ACL:
            ...
            if (ui_ace.ether.vid.op == VAL_diffServMacAceVidOp_equal)
            {
                ...
                {
                    sprintf(addr_str1," VID %lu", (unsigned long)ui_ace.ether.vid.u.s.data);
                    CLI_ACL_STRING_StrCat(&string, addr_str1);
                }
                else
                {
                    sprintf(addr_str1," VID %lu %lu",(unsigned long)ui_ace.ether.vid.u.s.data
            ...
            if (ui_ace.ipv4.vid.op == VAL_diffServMacAceVidOp_equal)
            {
                ...
                {
                    sprintf(addr_str1," MAC VID %lu", (unsigned long)ui_ace.ipv4.vid.u.s.data);
                    CLI_ACL_STRING_StrCat(&string, addr_str1);
                }
                else
                {
                    sprintf(addr_str1," MAC VID %lu %lu",(unsigned long)ui_ace.ipv4.vid.u.s.data
            ...
            if (ui_ace.l4_common.sport.op == VAL_diffServIpAceSourcePortOp_equal)
            {
                ...
                {
                    sprintf(addr_str1," source-port %lu", (unsigned long)ui_ace.l4_common.sport.u.s.data);
                    CLI_ACL_STRING_StrCat(&string, addr_str1);
                }
                else
                {
            ...
            if (ui_ace.l4_common.dport.op == VAL_diffServIpAceSourcePortOp_equal)
            {
                ...
                {
                    sprintf(addr_str1," destination-port %lu", (unsigned long)ui_ace.l4_common.dport.u.s.data);
                    CLI_ACL_STRING_StrCat(&string, addr_str1);
                }
                else
                {
    ...
    if (RULE_TYPE_UNDEF_TIME_RANGE != ui_ace.time_range_index)
    {
        ...
        memset(time_range_string, 0, sizeof(time_range_name));
        L4_PMGR_GetTimeRangeNameByID(ui_ace.time_range_index, time_range_name);
    ...
    {
        sprintf(addr_str1, " (%lu hits)", (unsigned long)ui_ace.counter.total_packets);
    ...
    char *arg[])
{
    ...
    while (arg[arg_position] != NULL)
    {
        switch(arg[arg_position][0])
        {
            ...
            case 't':/*time range*/
            case 'T':
                if (CLI_ERR_INTERNAL == CLI_ACL_TIME_RANGE_FillTimeRangeIndex(
                                            &arg[arg_position],
                                            &ace_p->time_range_index))
                {
                    return CLI_ERR_INTERNAL;
                }
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
    if (CLI_ERR_INTERNAL == CLI_ACL_TIME_RANGE_FillTimeRangeIndex(&arg[arg_pos], &ace_p->time_range_index))
    {
        return CLI_ERR_INTERNAL;
    }
#endif /* end of #if (SYS_CPNT_TIME_BASED_ACL == TRUE) */
    ...
    char *arg[])
{
    ...
    if (arg[arg_position][0] == 'm' || arg[arg_position][0] == 'M')
    {
        ...
        if (arg[arg_position][0] == 'v' || arg[arg_position][0] == 'V')
        {
            ...
            if (CLI_NO_ERROR != CLI_ACL_PrivGetVid(arg, &arg_position,
                                                   &ace_p->ipv4.vid.op,
                                                   &ace_p->ipv4.vid.u.s.data,
                                                   &ace_p->ipv4.vid.u.s.data,
                                                   &ace_p->ipv4.vid.u.s.mask))
            {
                return CLI_ERR_INTERNAL;
            }
        }
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
    if (CLI_ERR_INTERNAL == CLI_ACL_TIME_RANGE_FillTimeRangeIndex(&arg[arg_pos],
                                                                  &ace_p->time_range_index))
    {
        return CLI_ERR_INTERNAL;
    }
    ...
    char *arg[])
{
    ...
    while (arg[arg_pos] != NULL)
    {
        switch ( arg[arg_pos][0] )
        {
            ...
            case 'd': /* dscp or destination */
            case 'D':
                if (arg[arg_pos][1] == 's' || arg[arg_pos][1] == 'S')
                {
...
#if (SYS_CPNT_ACL_IPV6_EXT_TCP_UDP_PORT == TRUE)
                else if (arg[arg_pos][1] == 'e' || arg[arg_pos][1] == 'E')
                {
                    UI32_T tmp_arg = arg_pos+2 ;
                    ...
                    if (arg[tmp_arg] != NULL)
                    {
                        if (!isdigit(arg[tmp_arg][0]))
                        {
                            break;
                        }
                        else
                        {
                            ace_p->l4_common.dport.u.s.mask = atoi(arg[tmp_arg]);
                            arg_pos = arg_pos + 1;
                        }
                    }
                }
            ...
            case 'S':
                {
                    ...
                    if (arg[tmp_arg] != NULL)
                    {
                        ...
                        {
                            ace_p->l4_common.sport.u.s.mask = atoi(arg[tmp_arg]);
                            arg_pos = arg_pos + 1;
                        }
                    }
                    break;
...
UI32_T CLI_API_Show_AccessList_TcamUtilization(UI16_T cmd_idx, char *arg[], CLI_TASK_WorkingArea_T *ctrl_P)
{
...

    {
        ...
        UI32_T max_capability_str_len = 0;
        char   *capability_str_p;
...

        {
            ...
            for (capability_cnt = 0; capability_cnt < _countof(cap_str_table); ++capability_cnt)
            {
                RULE_TYPE_TCAM_CAP_STRING_T *cap_str_p = &cap_str_table[capability_cnt];

                max_capability_str_len += strlen(cap_str_p->description) + 2 /* ", " */;
            }
        }

        capability_str_p = (char *)L_MM_Malloc(max_capability_str_len + 1, L_MM_USER_ID2(SYS_MODULE_CLI, CLI_TYPE_TRACE_ID_CLI_API_SHOW_ACCESSLIST_TCAMUTILIZATION));
        if (NULL == capability_str_p)
        {
            return CLI_ERR_INTERNAL;
        }
        ...
        {
            UI32_T capability_cnt;
            char *cur_capability_str_p = capability_str_p;

            *cur_capability_str_p = '\0';
            ...
            for (; capability_cnt < RULE_TYPE_TCAM_CAP_TOTAL; ++capability_cnt)
            {
                if (RULE_TYPE_IS_BIT_ON(tcam_entry.cap_bitmap.bits, capability_cnt))
                {
                    ...
                    ASSERT((max_capability_str_len - (UI32_T)(cur_capability_str_p - capability_str_p)) >= strlen(cap_str_p->description) + 2);

                    cur_capability_str_p += SYSFUN_Snprintf(cur_capability_str_p,
                                                            max_capability_str_len - (UI32_T)(cur_capability_str_p - capability_str_p),
                                                            "%s, ", cap_str_p->description);
                }
            }

            /* Remove last ", "
             */
            if ((UI32_T)(cur_capability_str_p - capability_str_p) > 2)
            {
                *(cur_capability_str_p - 2) = '\0';
            }
            ...
            CLI_TBL_SetColText(&tcam_tbl, TBL_TCAM_CAP, capability_str_p);
            CLI_TBL_Print(&tcam_tbl);
        }

        CLI_TBL_Print(&tcam_tbl);

        L_MM_Free(capability_str_p);
    }

    return CLI_NO_ERROR;
}

/* cmd : [no] time-range time_range_name */
UI32_T CLI_API_Time_Range(UI16_T cmd_idx, char *arg[], CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    switch(cmd_idx)
    {
        case PRIVILEGE_CFG_GLOBAL_CMD_W1_TIMERANGE: /* create and enter time-range mode */
            /* check if exist */
            if (RULE_TYPE_OK != L4_PMGR_GetTimeRangeIdByName(arg[0], &time_range_index))
            {
                /* not exist, create it */
                if( RULE_TYPE_OK != L4_PMGR_ACL_CreateTimeRange(arg[0]) )
                {
#if (SYS_CPNT_EH == TRUE)
                    CLI_API_Show_Exception_Handeler_Msg();
#else
                    CLI_LIB_PrintStr("Failed to create time range entry\r\n");
#endif
                }
                else
                {
                    /* enter time range mode */
                    ctrl_P->CMenu.AccMode = PRIVILEGE_CFG_TIME_RANGE_MODE;
                    strcpy (ctrl_P->CMenu.time_range_name, arg[0]);
                }
            }
            else
            {
        ...
        case PRIVILEGE_CFG_GLOBAL_CMD_W2_NO_TIMERANGE: /* destory time range */
            if ( RULE_TYPE_OK!= L4_PMGR_ACL_DeleteTimeRange(arg[0]) )
            {
#if (SYS_CPNT_EH == TRUE)
                CLI_API_Show_Exception_Handeler_Msg();
#else
                CLI_LIB_PrintStr("Failed to destroy time range entry\r\n");
#endif
            }
            break;
...
/* cmd : 1.absolute start hours mimutes day month-name year [end hours mimutes day month-name  year]
         2.absolute end hours mimutes day month-name  year
         3.no absolute
 */
UI32_T CLI_API_Absolute(UI16_T cmd_idx, char *arg[], CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T start,end;
    RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T *p_start=NULL, *p_end=NULL;

    /* init */
    memset(&start, 0, sizeof(RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T));
    memset(&end,   0, sizeof(RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T));

    switch(cmd_idx)
    {
        case PRIVILEGE_CFG_TIME_RANGE_CMD_W1_ABSOLUTE: /* set */
            {
                if( arg[0][0]=='s' || arg[0][0]=='S' )/* start time */
                {
                    CLI_ACL_TIME_RANGE_FillAbsoluteTime (arg, &start);
                    p_start = &start;

                    if(arg[6] != NULL)/* end time */
                    {
                        CLI_ACL_TIME_RANGE_FillAbsoluteTime (arg+6, &end);
                        p_end = &end;
                    }
                }
                else/* end time */
                {
                    CLI_ACL_TIME_RANGE_FillAbsoluteTime (arg, &end);
                    p_end = &end;
                }

                /* setting */
                if( RULE_TYPE_OK!=L4_PMGR_ACL_SetTimeRangeAbsolute(ctrl_P->CMenu.time_range_name,p_start,p_end) )
                {
#if (SYS_CPNT_EH == TRUE)
                    CLI_API_Show_Exception_Handeler_Msg();
#else
                    CLI_LIB_PrintStr("Failed to set absolute time and date\r\n");
#endif
                }
            }
            break;

        case PRIVILEGE_CFG_TIME_RANGE_CMD_W2_NO_ABSOLUTE: /* destory */
            if ( RULE_TYPE_OK!= L4_PMGR_ACL_DestoryTimeRangeAbsolute(ctrl_P->CMenu.time_range_name) )
            {
#if (SYS_CPNT_EH == TRUE)
                CLI_API_Show_Exception_Handeler_Msg();
#else
                CLI_LIB_PrintStr("Failed to remove absolute time and date\r\n");
#endif
            }
            break;
...
/* cmd : [no] periodic days-of-the-week start-hours start-mimutes  to [days-of-the-week] end-hours end-mimutes */
UI32_T CLI_API_Periodic(UI16_T cmd_idx, char *arg[], CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    /* fill periodic */
    CLI_ACL_TIME_RANGE_FillPeriodicTime (arg, &periodic);

    switch(cmd_idx)
    {
        /* set */
        case PRIVILEGE_CFG_TIME_RANGE_CMD_W1_PERIODIC:
            {
                if( RULE_TYPE_OK!=L4_PMGR_ACL_SetTimeRangePeriodic(ctrl_P->CMenu.time_range_name, &periodic) )
                {
#if (SYS_CPNT_EH == TRUE)
                    CLI_API_Show_Exception_Handeler_Msg();
#else
                    CLI_LIB_PrintStr("Failed to set periodic time and day\r\n");
#endif
                }
            }
            break;
        ...
        /* destory */
        case PRIVILEGE_CFG_TIME_RANGE_CMD_W2_NO_PERIODIC:
            if ( RULE_TYPE_OK!= L4_PMGR_ACL_DestoryTimeRangePeriodic(ctrl_P->CMenu.time_range_name, &periodic) )
            {
#if (SYS_CPNT_EH == TRUE)
                CLI_API_Show_Exception_Handeler_Msg();
#else
                CLI_LIB_PrintStr("Failed to remove periodic time and day\r\n");
#endif
            }
            break;
...
/*
Console#show time-range test
Time-range test:
  absolute start 08:30 31 March 2009 end 8:30 31 May 2009
  periodic daily 08:00 to 20:00
  periodic Monday 08:00 to Friday 20:00
*/
...
UI32_T CLI_API_Show_Time_Range(UI16_T cmd_idx, char *arg[], CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    UI16_T                          time_range_index_t;
    RULE_TYPE_TIME_ACL_TIME_RANGE_T time_range_entry;

    char  buff[CLI_DEF_MAX_BUFSIZE] = {0};
    BOOL_T is_show_absolute, only_one = FALSE;
    ...
    /* specify which one time range to show */
    if (NULL != arg[0])
    {
        if (RULE_TYPE_OK == L4_PMGR_GetTimeRangeIdByName(arg[0], &time_range_index_t))
        {
            only_one = TRUE;
            if (time_range_index_t > 0)
            {
                time_range_index = time_range_index_t - 1;
            }
        }
        else
        {
    ...
    while ( RULE_TYPE_OK == L4_PMGR_GetNextTimeRange (&time_range_index , &time_range_entry) )
    {
        is_show_absolute = FALSE;

        sprintf (buff, " Time-range %s:\r\n", time_range_entry.name);
        PROCESS_MORE (buff);
        ...
        for (periodic_index = 0; periodic_index <SYS_ADPT_MAX_NBRS_OF_TIME_ACL_PERIODIC; periodic_index++)
        {
            ...
            {
                sprintf(buff, "   periodic %9s %02u:%02u to %9s %02u:%02u \r\n",
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
/* static function for SYS_CPNT_TIME_BASED_ACL -- start
 */

/* Return the day of the week
   255 - Error
 */
static UI32_T CLI_ACL_TIME_RANGE_GetDayOfWeek (char *day)
{
    char fifth_word;
    ...
    switch(day[0])
    {
        case 'D':/*DAILY*/
        case 'd':
            return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_DAILY;

        case 'M':/*monday*/
        case 'm':
            return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_MONDAY;
        case 'T':/*Tue,Thu*/
        case 't':
            if (day[1] == 'U' || day[1] == 'u')
            {
                return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_TUESDAY;
            }
            else
            {
                return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_THURSDAY;
            }
        case 'W':/*wed*/
        case 'w':
            fifth_word = day[5];
            if (fifth_word == 'S' || fifth_word == 's')
            {
                return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_WEDNESDAY;
            }
            else if (fifth_word == 'A' || fifth_word == 'a')
            {
                return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_WEEKDAYS;
            }
            else
            {
                return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_WEEKEND;
            }
        case 'F':/*fri*/
        case 'f':
            return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_FRIDAY;
        case 'S':/*Sat, Sun*/
        case 's':
            if (day[1] == 'A' || day[1] == 'a')
            {
                return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_SATURDAY;
            }
            else
            {
                return RULE_TYPE_TIME_RANGE_TYPE_OF_WK_SUNDAY;
            }
        default:
            return 255;
    }
...
/* Return the month
   0 - Error
 */
static UI32_T CLI_ACL_TIME_RANGE_GetMonth(char *arg)
{
   switch(arg[0])
   {
   case 'a':
...
/* fill absolute time */
static void CLI_ACL_TIME_RANGE_FillAbsoluteTime(char *arg[], RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T *absolute)
{
    absolute->hour   = (char) atoi(arg[1]);
    absolute->minute = (char) atoi(arg[2]);
    absolute->day    = (char) atoi(arg[3]);
    absolute->month  = (char) CLI_ACL_TIME_RANGE_GetMonth(arg[4]);
    absolute->year   = (UI16_T)atoi(arg[5]);
}

/* fill periodic time */
static void CLI_ACL_TIME_RANGE_FillPeriodicTime(char *arg[], RULE_TYPE_TIME_ACL_PERIODIC_TIME_T *periodic)
{
    char   day_of_week;

    /* init */
    memset(periodic, 0, sizeof(RULE_TYPE_TIME_ACL_PERIODIC_TIME_T));

    periodic->start_type_of_wk     = (char) CLI_ACL_TIME_RANGE_GetDayOfWeek(arg[0]);
    periodic->start_hour           = (char) atoi(arg[1]);
    periodic->start_minute         = (char) atoi(arg[2]);

    day_of_week = (char) CLI_ACL_TIME_RANGE_GetDayOfWeek(arg[4]);

    if (255 == day_of_week)
    {
        periodic->end_type_of_wk       = periodic->start_type_of_wk;
        periodic->end_hour             = (char) atoi(arg[4]);
        periodic->end_minute           = (char) atoi(arg[5]);
    }
    else
    {
        periodic->end_type_of_wk       = day_of_week;
        periodic->end_hour             = (char) atoi(arg[5]);
        periodic->end_minute           = (char) atoi(arg[6]);
    }
}

/* fill time range index */
static UI32_T CLI_ACL_TIME_RANGE_FillTimeRangeIndex(char *arg[], UI16_T *p_time_range_index)
{
    UI16_T time_range_index;
    ...
    if (arg [0] != NULL)
    {
        ...
        {
            if (RULE_TYPE_OK != L4_PMGR_GetTimeRangeIdByName(arg[1], &time_range_index))
            {
#if (SYS_CPNT_EH == TRUE)
                CLI_API_Show_Exception_Handeler_Msg ();
#else
                CLI_LIB_PrintStr ("Time range does not exist.\r\n");
#endif
                return CLI_ERR_INTERNAL;
            }

            /* assign */
            *p_time_range_index = time_range_index;
        }
    }

    return CLI_NO_ERROR;
}
/* static function for SYS_CPNT_TIME_BASED_ACL -- end
 */
#endif /* end of #if (SYS_CPNT_TIME_BASED_ACL == TRUE) */
