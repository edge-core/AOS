...
extern CMDLIST   ui_privilege_cfg_control_plane_cmd_list[];
#endif
...
extern CMDLIST   ui_privilege_cfg_line_console_cmd_lst[];
extern CMDLIST   ui_privilege_cfg_line_vty_cmd_lst[];
...
#endif /* (SYS_CPNT_QOS == SYS_CPNT_QOS_MARKER) */
...
extern CMDLIST   ui_privilege_cfg_hash_selection_mac_cmd_lst[];
extern CMDLIST   ui_privilege_cfg_hash_selection_ipv4_cmd_lst[];
extern CMDLIST   ui_privilege_cfg_hash_selection_ipv6_cmd_lst[];
#endif
...
#define _SET_MODE(_mode, _name, _cmd_list, _prompt, _help_msg)              \
...
#if (SYS_CPNT_QOS_V2_CPU_PORT == TRUE)
    _SET_MODE(PRIVILEGE_CFG_CONTROL_PLANE_MODE,
              "interface-control-plane",
              ui_privilege_cfg_control_plane_cmd_list,
              PRIVILEGE_CFG_CONTROL_PLANE_MODE_PROMPT,
              PRIVILEGE_CFG_CONTROL_PLANE_MODE_HELP_MSG);
#endif
    ...
    _SET_MODE(PRIVILEGE_CFG_LINE_CONSOLE_MODE,
              "console",
              ui_privilege_cfg_line_console_cmd_lst,
              PRIVILEGE_CFG_LINE_CONSOLE_MODE_PROMPT,
              PRIVILEGE_CFG_LINE_CONSOLE_MODE_HELP_MSG);

    _SET_MODE(PRIVILEGE_CFG_LINE_VTY_MODE,
              "vty",
              ui_privilege_cfg_line_vty_cmd_lst,
              PRIVILEGE_CFG_LINE_VTY_MODE_PROMPT,
              PRIVILEGE_CFG_LINE_VTY_MODE_HELP_MSG);
#if (SYS_CPNT_XSTP_UI==TRUE)
...
#if (SYS_CPNT_NSM_POLICY == TRUE)
#if 0 /*mark oute for compile pass*/
...
#endif
#endif /* SYS_CPNT_QOS_UI */

#if (SYS_CPNT_HASH_SELECTION == TRUE)
    _SET_MODE(PRIVILEGE_CFG_HASH_SELECTION_MAC_MODE,
              "hash-sel-mac",
              ui_privilege_cfg_hash_selection_mac_cmd_lst,
              PRIVILEGE_CFG_HASH_SELECTION_MAC_PROMPT,
              PRIVILEGE_CFG_HASH_SELECTION_MAC_HELP_MSG);

    _SET_MODE(PRIVILEGE_CFG_HASH_SELECTION_IPV4_MODE,
              "hash-sel-ipv4",
              ui_privilege_cfg_hash_selection_ipv4_cmd_lst,
              PRIVILEGE_CFG_HASH_SELECTION_IPV4_PROMPT,
              PRIVILEGE_CFG_HASH_SELECTION_IPV4_HELP_MSG);

    _SET_MODE(PRIVILEGE_CFG_HASH_SELECTION_IPV6_MODE,
              "hash-sel-ipv6",
              ui_privilege_cfg_hash_selection_ipv6_cmd_lst,
              PRIVILEGE_CFG_HASH_SELECTION_IPV6_PROMPT,
              PRIVILEGE_CFG_HASH_SELECTION_IPV6_HELP_MSG);
#endif
    ...
    if (is_get_reload_in_info == TRUE)
    {
        if (login == FALSE)
        {
            ...
            CLI_LIB_PrintStr_2("Reloading switch in time:\t\t\t%ld hours %ld minutes.\r\n",
                                (long)remain_hours, (long)remain_minutes);
        }
    }
    ...
    if (is_get_reload_at_info == TRUE)
    {
        if (login == FALSE)
        {
            CLI_LIB_PrintStr_2("Reloading switch at time:\t\t\t%s %2ld ",
                                month_ar[reload_at.month-1], (long)reload_at.day);
            CLI_LIB_PrintStr_3("%02ld:%02ld %ld.\r\n",
                                (long)reload_at.hour, (long)reload_at.minute, (long)reload_at.year);
        }
    }
    ...
    switch (ret)
    {
        case SYS_RELOAD_MGR_RELOAD_TIME_INFO_OK:
            ...
            CLI_LIB_PrintStr_2("\r\nThe switch will be rebooted at %s %2ld ",
                                month_ar[reload_time.month-1], (long)reload_time.day);
            CLI_LIB_PrintStr_4("%02ld:%02ld:%02ld %ld.\r\n",
                                (long)reload_time.hour,
                                (long)reload_time.minute,
                                (long)reload_time.second,
                                (long)reload_time.year);
            CLI_LIB_PrintStr_4("Remaining Time: %ld days, %ld hours, %ld minutes, %ld seconds.\r\n",
                                (long)remain_days, (long)remain_hours, (long)remain_minutes, (long)remain_seconds);
            return;
...
void CLI_LIB_ShowReloadNotify(CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    if(ctrl_P->reload_countdown!=0)
    {
        SYSFUN_Sprintf(scratch,
            "\r\n\r\n***\r\n*** --- Reboot in %lu minutes ---\r\n***\r\n",(unsigned long)ctrl_P->reload_countdown);
...
/*------------------------------------------------------------------------
 * ROUTINE NAME - CLI_LIB_ProcessPrintInteractive
 *------------------------------------------------------------------------
 * FUNCTION: Process print interactive.
 * INPUT   : line_num  - display line number
 * OUTPUT  : None
 * RETURN  : line_num  - display line number
 * NOTE    : None
 *------------------------------------------------------------------------
 */
UI32_T
CLI_LIB_ProcessPrintInteractive(
    UI32_T line_num)
{
    UI32_T screen_length;
    UI32_T ret;

    screen_length = CLI_LIB_GetMaxTerminalLineNum();

    if (TRUE == CLI_API_Get_Print_Interactive_Mode())
    {
        if (line_num >= screen_length-1)
        {
            ret = CLI_LIB_ProcMoreFeature();

            switch (ret)
            {
                case 1:  /* space, show next page */
                    line_num = 0;
                    break;

                case 2:  /* enter key, show next line */
                    line_num--;
                    break;

                case 4:  /* 'a' or 'A' keystroke, show all */
                    CLI_API_Set_Print_Interactive_Mode(FALSE);
                    break;

                case CLI_EXIT_SESSION: /* key error */
                    return EXIT_SESSION_MORE;
...
/*------------------------------------------------------------------------
 * ROUTINE NAME - CLI_LIB_PrintStrToScreen
 *------------------------------------------------------------------------
 * FUNCTION: Print string to screen.
 * INPUT   : str_p     - string
 * OUTPUT  : line_num  - display line number
 * RETURN  : line_num  - display line number
 * NOTE    : The display string will be limit with terminal length and width.
 *------------------------------------------------------------------------
 */
UI32_T
CLI_LIB_PrintStrToScreen(
    char *str_p,
    UI32_T line_num)
{
    ...
    UI32_T str_len;
    I32_T  str_width;
    char   scratch[CLI_DEF_MAX_DISP_WIDTH + 1] = {0};
    char   *scratch_p;
    char   *next_p;
    char   *end_p;
    ...
    if (str_p[0] == '\0') /* blank string, do nothing */
        return line_num;

#if (SYS_CPNT_CLI_TERMINAL == TRUE)
    screen_width  = ctrl_P->CMenu.width;
#else
    screen_width  = CLI_DEF_DISP_WIDTH;
    ...
    {
        /* cut string with \n,
           string width not contain \r\n
         */
        scratch_p = next_p;
        end_p = strstr(scratch_p, "\n");

        if (NULL != end_p)
        {
            next_p = end_p+1;
            str_len = end_p - scratch_p + 1;
            str_width = str_len - 1;
        ...
        {
            next_p = NULL;
            str_len = strlen(scratch_p);
            str_width = str_len;
            end_p = scratch_p + str_len - 1;
        }

        while (*end_p == '\r' && end_p >= scratch_p)
        {
            --str_width;
        ...
        if (str_width > screen_width)
        {
            do
            {
                strncpy(scratch, scratch_p, screen_width);
                scratch[screen_width] = '\0';
                scratch_p += screen_width;
                str_len = str_len - screen_width;
                str_width = str_width - screen_width;

                strncat((char *)ctrl_P->send_buf, scratch, STR_BUFF_UNUSED(ctrl_P->send_buf));
                ctrl_P->send_buf[sizeof(ctrl_P->send_buf) - 1] = '\0';
                strncat((char *)ctrl_P->send_buf, "\r\n", STR_BUFF_UNUSED(ctrl_P->send_buf));
                ctrl_P->send_buf[sizeof(ctrl_P->send_buf) - 1] = '\0';
                CLI_IO_PrintOut(ctrl_P);

                ++line_num;
                line_num = CLI_LIB_ProcessPrintInteractive(line_num);
            ...
            } while (str_width > screen_width);
        }

        strncpy(scratch, scratch_p, str_len);
        scratch[str_len] = '\0';
        strncat((char *)ctrl_P->send_buf, scratch, STR_BUFF_UNUSED(ctrl_P->send_buf));
        ctrl_P->send_buf[sizeof(ctrl_P->send_buf) - 1] = '\0';
        CLI_IO_PrintOut(ctrl_P);
...
void CLI_LIB_PrintStr(char *str)
{
    ...
    line_num = CLI_LIB_PrintStrToScreen(str, line_num);
}
...
{
    char buf[CLI_DEF_OUTPUT_BUFFER]={0};
    va_list   ap;
    UI32_T  retval;
    ...
    va_end(ap);
    if(retval >= CLI_DEF_OUTPUT_BUFFER)
    {
...
{
    char scratch[CLI_DEF_OUTPUT_BUFFER];

    assert(ctrl_P != NULL);

    SYSFUN_Snprintf(scratch, CLI_DEF_OUTPUT_BUFFER, "%s", str);
    strncat(ctrl_P->send_buf, scratch, CLI_DEF_OUTPUT_BUFFER);
...
{
#define CLI_DEF_OUTPUT_BUFFER_MINUS_ONE (CLI_DEF_OUTPUT_BUFFER-1)

    CLI_TASK_WorkingArea_T *ctrl_P = (CLI_TASK_WorkingArea_T *)CLI_TASK_GetMyWorkingArea();
    UI32_T print_time = 0;
    UI32_T i = 0;
    UI32_T print_len = 0;
    char  scratch[CLI_DEF_OUTPUT_BUFFER] = {0};/*pttch 2002.06.03 initial the string for jobs*/
    ...
    if(strlen(str)%CLI_DEF_OUTPUT_BUFFER_MINUS_ONE == 0)
    {
        print_time = strlen(str)/CLI_DEF_OUTPUT_BUFFER_MINUS_ONE;
    }
    else
    {
        print_time = strlen(str)/CLI_DEF_OUTPUT_BUFFER_MINUS_ONE + 1 ;
    }

    for(i=0;i<print_time;i++)
    {
        memset(scratch,0,sizeof(scratch));
        if(strlen((str+CLI_DEF_OUTPUT_BUFFER_MINUS_ONE*i))>=CLI_DEF_OUTPUT_BUFFER_MINUS_ONE)
        {
            print_len = CLI_DEF_OUTPUT_BUFFER_MINUS_ONE;
            memcpy(scratch,str+CLI_DEF_OUTPUT_BUFFER_MINUS_ONE*i,CLI_DEF_OUTPUT_BUFFER_MINUS_ONE);
            scratch[CLI_DEF_OUTPUT_BUFFER_MINUS_ONE] = '\0';
            strncat(ctrl_P->send_buf, scratch, CLI_DEF_OUTPUT_BUFFER);
        ...
        {
            print_len = strlen((str+CLI_DEF_OUTPUT_BUFFER_MINUS_ONE*i));
            memcpy(scratch,str+CLI_DEF_OUTPUT_BUFFER_MINUS_ONE*i,print_len);
            scratch[print_len] = '\0';
            strncat(ctrl_P->send_buf, scratch, CLI_DEF_OUTPUT_BUFFER);
...
void CLI_LIB_PrintNullStr(int line_num)
{
    ...
    {
        strncat((char *)ctrl_P->send_buf, (char *)scratch, CLI_DEF_OUTPUT_BUFFER);
    }
    CLI_IO_PrintOut(ctrl_P);
}
...
/* FUNCTION NAME: CLI_LIB_IsSameWithLastHistBuf
 * PURPOSE : check the input command is same with final input command or not
 * INPUT   : s      - input command
 *           ctrl_P - CLI_TASK_WorkingArea
 * OUTPUT  : None
 * RETURN  : 1    - same
 *           0    - not same
 * NOTES   : None
 */
UI16_T CLI_LIB_IsSameWithLastHistBuf(
    char *s,
...
)
{
    ...
    default:
        ...
        if (histsize >= ctrl_P->CMenu.HistTail)
        {
            lastIdx = (ctrl_P->CMenu.HistTail + histsize - 1) % histsize;
        }
        else
        {
            lastIdx = (histsize + histsize - 1) % histsize;
        }

        /* check input is same with last command or not */
        if (strcmp(s, (char *)ctrl_P->CMenu.HistBuf[lastIdx]) == 0)
        {
            return 1;
        }
   ...
   case PRIVILEGE_CFG_ROUTE_MAP_MODE:
    ...
    case PRIVILEGE_CFG_HASH_SELECTION_IPV6_MODE:
        ...
        /* find last history index */
        if (histsize >= ctrl_P->CMenu.CfgHistTail)
        {
            lastIdx = (ctrl_P->CMenu.CfgHistTail + histsize - 1) % histsize;
        }
        else
        {
            lastIdx = (histsize + histsize - 1) % histsize;
        ...
        /* check input is same with last command or not */
        if (strcmp(s, (char *)ctrl_P->CMenu.CfgHistBuf[lastIdx]) == 0)
        {
            return 1;
        }
...
UI16_T CLI_LIB_AddHistBuf(char *s, CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    /* check the command is same as final input command
     */
    if (CLI_LIB_IsSameWithLastHistBuf(s, ctrl_P))
    {
        return 0;
    }
...
UI16_T CLI_LIB_GetCommandDispWidth(CLI_TASK_WorkingArea_T *ctrl_P)
{
    UI32_T        screen_width;
    ...
    if (CLI_LIB_IsProvision(ctrl_P) == FALSE)
    {
        ...
        if (entry_p)
        {
            ...
            {
#if (SYS_CPNT_MSTP_SUPPORT_PVST==TRUE)
                disp_width = screen_width - (len + strlen( entry_p->prompt ) + 6);
#else
                disp_width = screen_width - (len + strlen( entry_p->prompt ));
#endif
            }
            else
            {
...
void CLI_LIB_ShowPromptMsg(CLI_TASK_WorkingArea_T *ctrl_P)
{
    enum
    {
        PLUS_SESSION_ID_STR_MAX_LEN = sizeof("-255") - 1
    };

    char plus_session_id_str[PLUS_SESSION_ID_STR_MAX_LEN+1] = {0};
    char prompt[PROMPT_LEN] = {0};
    SYS_PMGR_GetPromptString((UI8_T *)prompt);
    ...
    case CLI_TYPE_UART:
        ...
        if(strlen(prompt)==0)
        {
            strncpy(prompt, CLI_TASK_CONSOLE_STR, sizeof(prompt)-1);
    ...
    case CLI_TYPE_TELNET:
        if (strlen(prompt) == 0)
        {
            strncpy(prompt, CLI_TASK_VTY_STR, sizeof(prompt)-1);
            prompt[sizeof(prompt)-1] = '\0';
        }

        SYSFUN_Snprintf(plus_session_id_str, sizeof(plus_session_id_str), "-%d", CLI_TASK_GetMySessId());
        plus_session_id_str[sizeof(plus_session_id_str)-1] = '\0';
        SYSFUN_Strncat(prompt, plus_session_id_str, STR_BUFF_UNUSED(prompt));
        prompt[sizeof(prompt)-1] = '\0';
        break;
    ...
    if (CLI_LIB_IsProvision(ctrl_P) == FALSE)
    {
        ...
        if (NULL != entry_p)
        {
            if (PRIVILEGE_CFG_VLAN_DATABASE_MODE == ctrl_P->CMenu.AccMode)
            {
                ...
                SYSFUN_Sprintf(vlan_string_temp,"%s%lu)#", (unsigned long)entry_p->prompt,vlan_id);
...
UI16_T CLI_LIB_RedisplayCmd(char *buf, UI16_T str_idx, UI16_T disp_width, UI16_T type, CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    if (new_shift > 0)
    {
        /*clear the display and back to prompt*/
        for (i=0; i<=disp_width-1; i++)
...
void CLI_LIB_PrintFStr(char *str, UI16_T sidx, UI16_T eidx)
{
    ...
    if ((eidx-sidx)>=sizeof(scratch))
        eidx = sidx + sizeof(scratch) - 1;
...
void CLI_LIB_PrintLineChar(char ch, UI32_T len)
{
#if (SYS_CPNT_CLI_TERMINAL == TRUE)
    CLI_TASK_WorkingArea_T *ctrl_P = (CLI_TASK_WorkingArea_T *)CLI_TASK_GetMyWorkingArea();
#endif /*#if (SYS_CPNT_CLI_TERMINAL == TRUE)*/
    char  scratch[CLI_DEF_MAX_DISP_WIDTH+1];
    UI32_T i;
    UI32_T screen_width;
...
#define STRING_POOL_USED_SIZE(sp)   (sp.str - sp.pool)
#define STRING_POOL_FREE_SIZE(sp)   (sp.capacity - STRING_POOL_USED_SIZE(sp))
    ...
    while (idx != NULL_ARG)
    {
        ...
        switch(ui_arg_lst[idx].type)
        {
            ...
            case IPV6_ADDR_DATA:
                if (0 < STRING_POOL_FREE_SIZE(string_pool))
                {
                    SYSFUN_Snprintf(string_pool.str,
                                    STRING_POOL_FREE_SIZE(string_pool),
                                    "X:X:X:X::X");
                    string_pool.str[ STRING_POOL_FREE_SIZE(string_pool) -1 ] = '\0';
            ...
            case IPV6_PREFIX_DATA:
                if (0 < STRING_POOL_FREE_SIZE(string_pool))
                {
                    SYSFUN_Snprintf(string_pool.str,
                                    STRING_POOL_FREE_SIZE(string_pool),
                                    "X:X:X:X::X/<%lu-%lu>",
                                    (unsigned long)ui_arg_lst[idx].min_range,
                                    (unsigned long)ui_arg_lst[idx].max_range);
                    string_pool.str[ STRING_POOL_FREE_SIZE(string_pool) -1 ] = '\0';
            ...
            case RANGE_DATA:
             ...
             case MINUS_DATA:
                if (0 < STRING_POOL_FREE_SIZE(string_pool))
                {
                    I32_T min_range, max_range;

                    min_range = ui_arg_lst[idx].min_range;
                    max_range = ui_arg_lst[idx].max_range;
            ...
            case UNIT_COLON_DATA:/*pttch stacking*/
                if (0 < STRING_POOL_FREE_SIZE(string_pool))
                {
                    ...
                    if (STKTPLG_OM_GetStackMaxUnitID(&max_unit_id) == FALSE)
                        max_unit_id = SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK;

                    SYSFUN_Snprintf(string_pool.str,
                                    STRING_POOL_FREE_SIZE(string_pool),
                                    "<1-%lu>:",
                                    (unsigned long)max_unit_id);
                    string_pool.str[ STRING_POOL_FREE_SIZE(string_pool) -1 ] = '\0';
            ...
            case PVC_DATA:
                if (0 < STRING_POOL_FREE_SIZE(string_pool))
                {
                    ...
                    {
                        min_value = 1;
                        if (STKTPLG_OM_GetStackMaxUnitID(&max_value) == FALSE)
                            max_value = SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK;
                    }
#else
...
#undef STRING_POOL_USED_SIZE
#undef STRING_POOL_FREE_SIZE
}
...
{
#if (SYS_CPNT_CLI_TERMINAL == TRUE)
    char  disp_buf[CLI_DEF_MAX_DISP_WIDTH*2+1] = {0};
    char  buff[CLI_DEF_MAX_DISP_WIDTH+1] = {0};
#else
...
{
    char   buf[CLI_DEF_MAX_DISP_WIDTH+1];
...
#define MORE_PROMPT_STR "--- [Space] Next page, [Enter] Next line, [A] All, Others to exit ---"
   ...
   len = strlen(MORE_PROMPT_STR);
     ...
     */
    CLI_LIB_PrintLineChar(0x08, len); /* BACK */
    CLI_LIB_PrintLineChar(0x20, len); /* SPACE */
    CLI_LIB_PrintLineChar(0x08, len); /* BACK */
...
UI16_T CLI_LIB_PrintStrLine(char *str, UI16_T line_num)
{
    ...
    /* Display one string line, 2011.07.14 pttch, add ret == 4 need to print the this str
       because return to previous function it will not print current str
     */
    if ((ret == 1) || (ret == 2) || (ret == 4))
    {
        SYSFUN_Snprintf((char *)scratch, sizeof(scratch), "%s\r\n", str);
        scratch[sizeof(scratch)-1] = '\0';
        strncat((char *)ctrl_P->send_buf, (char *)scratch, STR_BUFF_UNUSED(ctrl_P->send_buf));
        ctrl_P->send_buf[sizeof(ctrl_P->send_buf) - 1] = '\0';
        CLI_IO_PrintOut(ctrl_P);
    }
    return ret_val;
...
#if (SYS_CPNT_MSTP_SUPPORT_PVST==TRUE)
            CLI_API_GET_VLAN_DATABASE_VLANID(&vlan_id);
            SYSFUN_Sprintf((char *)vlan_string_temp,"%s%lu)#", PRIVILEGE_CFG_VLAN_DATABASE_MODE_PROMPT,(unsigned long)vlan_id);
            strcat(prompt, vlan_string_temp);
#else
...
BOOL_T  CLI_LIB_EAtoStr(UI8_T *mac, char *Str)   //cjchu 2001/8/2
{
...
void HexAddrToStr(UI32_T HexAddr, char *OutStr, BOOL_T is_reverse)
{
    ...
    if (is_reverse)
        SYSFUN_Sprintf(OutStr, "%u.%u.%u.%u", Value[3], Value[2],Value[1],Value[0]);
    else
        SYSFUN_Sprintf(OutStr, "%u.%u.%u.%u", Value[0], Value[1],Value[2],Value[3]);
}
...
/* -------------------------------------------------------------------------
 * FUNCTION NAME - CLI_LIB_Bitmap_To_List_Ex
 * -------------------------------------------------------------------------
 * FUNCTION: Convert bitmap array to a list string (ex: 1-2,5-7).
 * INPUT   : bitmap           - pointer to bitmap array.
 *           buffer_len       - length of output list buffer
 *           bit_numbers      - bit numbers of bitmap
 *           nbr_1st_bit      - the number representing the 1st bit
 *                              ex:  2 for the 1st bit,
 *                                   3 for the 2nd bit, and so on
 *           is_first_bit_use - TRUE if the 1st bit is used for bitmap
 * OUTPUT  : list             - pointer to output list string
 * RETURN  : TRUE/FALSE
 * NOTE    : For PFC.
 * -------------------------------------------------------------------------
 */
BOOL_T CLI_LIB_Bitmap_To_List_Ex(
    char    *bitmap,
    char    *list,
    UI32_T  buffer_len,
    UI32_T  bit_numbers,
    UI8_T   nbr_1st_bit,
    BOOL_T  is_first_bit_use)
{
    UI32_T  beg_nbr, end_nbr, i, leading = 0xffffffff, len =0;
    UI8_T   case_val = 0, nbr_offset =0;
    char    gen_str[16] = {0};
    ...
    *list = 0;
    ...
    {
        beg_nbr = 1;
        end_nbr = bit_numbers+1;
    }

    /* should not occur when 1st bit is not used but nbr_1st_bit is 0
     */
    if (nbr_1st_bit < beg_nbr)
        return FALSE;

    nbr_offset = nbr_1st_bit - beg_nbr;

    for (i=beg_nbr; i<end_nbr; i++)
    {
        switch(case_val)
        {
        case 0:
            if ( bitmap[i/8] & ( 1 << (7-i%8)) )
            {
                ...
                {
                    SYSFUN_Sprintf((char *)gen_str, "%lu,", (unsigned long)(i + nbr_offset));
        ...
        case 1:
            if ( !(bitmap[i/8] & ( 1 << (7-i%8))) )
            {
                ...
                if (i == leading+1) /*individual*/
                    SYSFUN_Sprintf((char *)gen_str, "%lu,", (unsigned long)(leading + nbr_offset));
                else               /*range*/
                    SYSFUN_Sprintf((char *)gen_str, "%lu-%lu,", (unsigned long)(leading + nbr_offset), (unsigned long)(i + nbr_offset -1));
            ...
            else if(i == end_nbr-1) /*exist and last one*/
            {
                SYSFUN_Sprintf((char *)gen_str, "%lu-%lu,", (unsigned long)(leading + nbr_offset), (unsigned long)(i + nbr_offset));
                len = len + strlen((char *)gen_str);
                if (len >= buffer_len)
                    return FALSE;
                else
...
/* -------------------------------------------------------------------------
 * FUNCTION NAME - CLI_LIB_Bitmap_To_List
 * -------------------------------------------------------------------------
 * FUNCTION: Convert bitmap array to a list string (ex: 1-2,5-7).
 * INPUT   : bitmap           - pointer to bitmap array.
 *           buffer_len       - length of output list buffer
 ...
 * OUTPUT  : list             - pointer to output list string
 * RETURN  : TRUE/FALSE
 * NOTE    : For VLAN, SPT, etc.
 *           (the number representing the 1st bit is always 1)
 * -------------------------------------------------------------------------
 */
BOOL_T CLI_LIB_Bitmap_To_List(
    char    *bitmap,
    char    *list,
    UI32_T  buffer_len,
    UI32_T  bit_numbers,
    BOOL_T  is_first_bit_use)
{
    return CLI_LIB_Bitmap_To_List_Ex(
                bitmap,
                list,
                buffer_len,
                bit_numbers,
        ...
        for (i=0; i<bit_numbers; i++)
        {
            ...
            case 0:
                if( bitmap[i/8] & ( 1 << (7-i%8)) )
                {
                    ...
                    {
                        SYSFUN_Sprintf((char *)gen_str, "%lu,", (unsigned long)(i % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1);
            ...
            case 1:
                if( !(bitmap[i/8] & ( 1 << (7-i%8))) )
                {
                    ...
                    case_val = 0;
                    if(i == leading+1) /*individual*/
                        SYSFUN_Sprintf((char *)gen_str, "%lu,", (unsigned long)(leading % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1);
                    else               /*range*/
                        SYSFUN_Sprintf((char *)gen_str, "%lu-%lu,", (unsigned long)(leading % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1, (unsigned long)((i - 1) % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1);
                ...
                {
                    SYSFUN_Sprintf((char *)gen_str, "%lu-%lu,", (unsigned long)(leading % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1, (unsigned long)(i % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1);
                    *unit_id = (leading / SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1; /* talor 2004-08-31 */
        ...
        for (i=1; i<bit_numbers+1; i++)
        {
            ...
            case 0:
                if( bitmap[i/8] & ( 1 << (7-i%8)) )
                {
                    ...
                    {
                        SYSFUN_Sprintf((char *)gen_str, "%lu,", (unsigned long)((i - 1) % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1);
            ...
            case 1:
                if( !(bitmap[i/8] & ( 1 << (7-i%8))) )
                {
                    case_val = 0;
                    if(i == leading + 1) /*individual*/
                        SYSFUN_Sprintf((char *)gen_str, "%lu,", (unsigned long)((leading - 1) % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1);
                    else               /*range*/
                        SYSFUN_Sprintf((char *)gen_str, "%lu-%lu,", (unsigned long)((leading - 1) % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1, (unsigned long)((i - 2) % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1);
                ...
                {
                    SYSFUN_Sprintf((char *)gen_str, "%lu-%lu,", (unsigned long)((leading - 1) % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1, (unsigned long)((i - 1) % SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1);
                    *unit_id = (leading / SYS_ADPT_MAX_NBR_OF_PORT_PER_UNIT) + 1; /* talor 2004-08-31 */
                    ...
                    switch(case_val)
                    {
                        ...
                        if( bitmap[(ifindex-1)/8] & ( 1 << (7-(ifindex-1)%8)) )
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    SYSFUN_Sprintf(gen_str, " untagged ethernet %lu/%lu\r\n",(unsigned long)begin_unit,(unsigned long)begin_port);
                                }
                                else
                                {
                        ...
                        if( !(bitmap[(ifindex-1)/8] & ( 1 << (7-(ifindex-1)%8))) )
                        {
                            ...
                            else               /*range*/
                            {
                                ...
                                if(port_member_status==0)
                                {
                                    SYSFUN_Sprintf(gen_str, " untagged ethernet %lu/%lu to %lu/%lu\r\n",(unsigned long)begin_unit,(unsigned long)begin_port,(unsigned long)end_unit,(unsigned long)end_port);
                                }
                                else
                                {
...
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_LIB_ConvertToMonthString
 * ------------------------------------------------------------------------
 * PURPOSE  : This function is used to convert integer to date string
 * INPUT    : day
 * OUTPUT   : data
 * RETURN   : none
 * NOTE     : used in cli_runcfg.c
 * ------------------------------------------------------------------------
 */
void CLI_LIB_ConvertToMonthString(UI32_T month, char *month_str_p, UI32_T month_length)
{
    switch (month)
    {
        case 1:
            strncpy(month_str_p, "january", month_length);
            break;
        case 2:
            strncpy(month_str_p, "february", month_length);
            break;
        case 3:
            strncpy(month_str_p, "march", month_length);
            break;
        case 4:
            strncpy(month_str_p, "april", month_length);
            break;
        case 5:
            strncpy(month_str_p, "may", month_length);
            break;
        case 6:
            strncpy(month_str_p, "june", month_length);
            break;
        case 7:
            strncpy(month_str_p, "july", month_length);
            break;
        case 8:
            strncpy(month_str_p, "august", month_length);
            break;
        case 9:
            strncpy(month_str_p, "september", month_length);
            break;
        case 10:
            strncpy(month_str_p, "october", month_length);
            break;
        case 11:
            strncpy(month_str_p, "november", month_length);
            break;
        case 12:
            strncpy(month_str_p, "december", month_length);
            break;
        default:
            break;
    }

    month_str_p[month_length] = '\0';
}

/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_LIB_ConvertToDateString
 * ------------------------------------------------------------------------
 * PURPOSE  : This function is used to convert integer to date string
 * INPUT    : day
 * OUTPUT   : data
 ...
 * ------------------------------------------------------------------------
 */
void CLI_LIB_ConvertToDateString(UI32_T day, char *data_p, UI32_T data_length)
{
    switch(day)
    {
        case 1:
            strncpy(data_p, "monday", data_length);
            break;
        case 2:
            strncpy(data_p, "tuesday", data_length);
            break;
        case 3:
            strncpy(data_p, "wednesday", data_length);
            break;
        case 4:
            strncpy(data_p, "thursday", data_length);
            break;
        case 5:
            strncpy(data_p, "friday", data_length);
            break;
        case 6:
            strncpy(data_p, "saturday", data_length);
            break;
        case 0:
            strncpy(data_p, "sunday", data_length);
            break;
        default:
            break;
    }

    data_p[data_length] = '\0';
}
#endif /* #if (SYS_CPNT_SYS_TIME_SUMMERTIME == TRUE) */

/*------------------------------------------------------------------------
 * ROUTINE NAME - CLI_LIB_SendLoginLogoutTrap
 *------------------------------------------------------------------------
 * FUNCTION: Send login/logout trap
 * INPUT   : trap_type         -- trap type is login or logout
 *           session type      -- session type
 *           user_name_p       -- user name
 *           rem_ip_addr_p     -- remote user ip address
 * OUTPUT  : None
 * RETURN  : TRUE/FALSE
 * NOTE    : None
 *------------------------------------------------------------------------
 */
BOOL_T
CLI_LIB_SendLoginLogoutTrap(
    UI32_T trap_type,
    UI8_T session_type,
    char *user_name_p,
    L_INET_AddrIp_T *rem_ip_addr_p)
{
    ...
    {
        case CLI_TYPE_UART:
            data.u.user_info.session_type = VAL_trapVarSessionType_console;
            break;

        case CLI_TYPE_TELNET:
            data.u.user_info.session_type = VAL_trapVarSessionType_telnet;
            break;

        case CLI_TYPE_SSH:
            data.u.user_info.session_type = VAL_trapVarSessionType_ssh;
            break;
     ...
     */
    if (0 != rem_ip_addr_p->addrlen)
    {
        if (NETCFG_TYPE_FAIL == NETCFG_PMGR_ROUTE_GetReversePathIpMac(rem_ip_addr_p,
    ...
    memcpy(&data.u.user_info.user_ip, rem_ip_addr_p, sizeof(data.u.user_info.user_ip));

    SNMP_PMGR_ReqSendTrapOptional(&data, TRAP_EVENT_SEND_TRAP_OPTION_LOG_AND_TRAP);
    return TRUE;
}/* End of CLI_LIB_SendLoginLogoutTrap */

#if (SYS_CPNT_WEB_MULTI_PRIVILEGE_LEVEL == TRUE)
static BOOL_T
CLI_LIB_Stub_GetMaxCommandPrivilege(const CLI_TrackEntry_T argv[],
                                    UI32_T argc,
                                    struct GetMaxCommandPrivilegeParam *param_p)
{
    int i;
    ...
    for (i = 1; i <= argc; ++i)
    {
        CLI_TrackEntry_T *node_p = (CLI_TrackEntry_T *) &argv[argc - i];

        if (node_p->idx >= param_p->end_index)
        {
            break;
        }
        else
        {
            if (param_p->max_privilege < node_p->cmd_list[ node_p->idx ].current_privilege)
            {
                param_p->max_privilege = node_p->cmd_list[ node_p->idx ].current_privilege;
            }
        }
    }
...
/* -------------------------------------------------------------------------
 * ROUTINE NAME - CLI_LIB_GetMaxCommandPrivilege
 * -------------------------------------------------------------------------
 * FUNCTION: Get max command privilege in the subtree
 * INPUT   : cmd_index    - command index
 *           cmd_mode     - which mode
 * OUTPUT  : None
 * RETURN  : The max command privilege in the subtree.
 * NOTE    : None
 * -------------------------------------------------------------------------
 */
UI32_T
CLI_LIB_GetMaxCommandPrivilege(const UI32_T cmd_index,
                               enum CLI_ACCESS_MODE_E cmd_mode)
{
    CLI_TrackEntry_T node;
    struct GetMaxCommandPrivilegeParam param;
    ...
    memset(&param, 0, sizeof(param));
    param.max_privilege = 0;
    param.end_index = node.cmd_list[cmd_index].neibor_idx;

    CLI_LIB_Inorder(&node, WALK_COMMAND_ONLY, 100, (VisitorFuncPtr)CLI_LIB_Stub_GetMaxCommandPrivilege, &param);

    return param.max_privilege;
}
#endif  /* #if (SYS_CPNT_WEB_MULTI_PRIVILEGE_LEVEL == TRUE) */

#if (SYS_CPNT_CHANGE_LOADER_BACKDOOR_PASSWORD == TRUE)
/* -------------------------------------------------------------------------
 * ROUTINE NAME - CLI_LIB_Encrypt
 * -------------------------------------------------------------------------
 * FUNCTION:   encrypt data for loader password
 * INPUT   : data    - data which needs to encrypt
 * OUTPUT  : data
 * RETURN  : None
 * NOTE    : None
 * -------------------------------------------------------------------------
 */
void CLI_LIB_Encrypt(char * data)
{
