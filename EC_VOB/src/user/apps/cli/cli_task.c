...
static SYSFUN_MsgQ_T    cli_ipc_msgq_handle;
 ...
 */
/*
05/05/2004 11:25�W�� modify pttch add for hot-swap module provision porting from hagrid
...
BOOL_T CLI_TASK_GET_IPC_Handle(SYSFUN_MsgQ_T * handler_p)
{
...
*/
    ...
    for (i = 0; i < CLI_TASK_MAX_SESSION_NUM; i++)
    {
        ...
        cli_working_buffer[i].cmd_info.unit_cmd_flag = FALSE;
        cli_working_buffer[i].cmd_info.eth_mode_flag = FALSE;
        cli_working_buffer[i].cmd_info.unit_id = 0;
...
/*------------------------------------------------------------------------
 * ROUTINE NAME - CLI_TASK_InitConsoleWorkingArea
 *------------------------------------------------------------------------
 * PURPOSE : initial working area of console session
 * INPUT   : none
 * OUTPUT  : none
 * RETURN  : none
 * NOTE    :
 *------------------------------------------------------------------------*/
void CLI_TASK_InitConsoleWorkingArea()
{
    CLI_TASK_WorkingArea_T *console_ctrl_p = CLI_TASK_GetWorkingAreaByIndex(CLI_TASK_UART_SESSION_ID);

    if ( (console_ctrl_p != NULL )
        && (console_ctrl_p->is_empty == FALSE)
       )
    {
        CLI_MAIN_Init_Menu_Table(console_ctrl_p);
    }
}
...
BOOL_T CLI_TASK_CreateTask(UI32_T cli_session_type, I32_T  socket_id, UI32_T unit,char *prov_buffer, UI32_T prov_buffer_len)
{
    ...
    /* Specify task name
     */
    sprintf(taskname, "CLITASK%lu", (unsigned long)sess_id);
...
void CLI_TASK_DumpCliWorkingBuffer(void)
{
    ...
    for (i = 1; i <= 4; i++)
    {
        SYSFUN_Debug_Printf("\r\n[%2d] is_empty = %s, cli_tid = %ld, socket = %ld",
               ...
               (long)cli_working_buffer[i].cli_tid,
               (long)cli_working_buffer[i].socket);
    }
    SYSFUN_Debug_Printf("\r\n");
    CLI_TASK_LeaveCriticalSection(cli_task_semid,orig_priority);
...
CLI_TASK_WorkingArea_T *CLI_TASK_GetMyWorkingArea(void)
{
    ...
    SYSFUN_Debug_Printf("\r\n%s(): cannot find CLI working space, current task id = %ld", __FUNCTION__, (long)tid);
                        ...
                        while(is_Go_To_Provision == TRUE)
                        {
                            ...
                            if(CLI_MGR_Get_RuntimeProvisionFlag() != TRUE)
                            {
                                ...
                                else
                                {
									...
									{
                                        CLI_LIB_PrintStr("No startup configuration file\r\n");
                                        
                                        /* added the following 4 statements because AOS ONIE Installing has no any config file (no Factory default config file either)
                                         */
                                        CLI_LIB_PrintStr("Use Facotry Default Configuration\r\n");
                                        is_need_save = TRUE;
                                        is_startup = TRUE;
                                        CLI_OM_EXP_SetStartupFactoryDefaultFlag(TRUE); /* provision by factory default config file */
									}
									if (strcmp( src_filename_ar, SYS_DFLT_restartConfigFile)==0)
									{
										/* provision by factory default config file */
										CLI_OM_EXP_SetStartupFactoryDefaultFlag(TRUE);
									}
                                    ...
                                    if (is_need_save == TRUE || is_factory_default_exist == FALSE)
                                    {
                                        ...
                                        */
                                        ...
                                        if (!XFER_PMGR_StreamToLocal(
                                            &user_info,
                                            (UI8_T *)SYS_DFLT_restartConfigFile,
                                            FS_FILE_TYPE_CONFIG,
                                            (void *)working_area,
                                            SYS_BLD_CLI_GROUP_IPCMSGQ_KEY,CLI_MGR_XferCopy_Callback))
                                        {
                            ...
                            else
                            {
                                ...
                                CLI_OM_EXP_SetStartupFactoryDefaultFlag(FALSE);
                            ...
                            if (is_config_file_ok == FALSE)
                            {
                                ...
                                {
                                    CLI_LIB_PrintStr("Set Factory Default Configuration as startup-config\r\n");
                                    is_Go_To_Provision = TRUE;
                                    CLI_MGR_Set_Is_Go_To_Provision(is_Go_To_Provision);
                                }
                                else
                                {
                                    CLI_LIB_PrintStr("Cannot set Factory Default Configuration as startup-config\r\n");
                            ...
                            else
                            {
...

                                {
                                    ...
                                    if(CLI_TASK_Reproduce_Config(file_name))
                                    {
                                        ...
                                        if (FALSE == XFER_PMGR_AutoConfigToUnit((UI8_T*)file_name,(void *)working_area,SYS_BLD_CLI_GROUP_IPCMSGQ_KEY, NULL))
                                        {
                                            /* This error message will show up when auto upgrade is enabled.
                                             * Because auto upgrade will call XFER_PMGR to begin a transaction and
                                             * the transaction is done after the auto upgrade operation is done.
                                             * The function call to XFER_PMGR_AutoConfigToUnit will fail because
                                             * the transaction of auto upgrade is in process. This error does
                                             * not cause any problem in non-stacking project. So a quick workaround is
                                             * to comment out the error message on projects that do not support stacking.
                                             */
  ...
  when hotswap add ,save the topo info in flash*/
          ...
          */
            ...
            case CLI_MGR_OPMODE_TRANSITION_MODE:
                ...
                CLI_TASK_InitConsoleWorkingArea();
...
#if (SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG == TRUE)
    case CLI_TYPE_PARTIAL_PROV:
        if (cli_working_buffer[CLI_TASK_PARTIAL_PROV_SESSION_ID].is_empty)
        {
            *sess_id = CLI_TASK_PARTIAL_PROV_SESSION_ID;
            CLI_MGR_Set_Provision_Flag(TRUE);
        }
        break;
#endif  /* #if (SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG == TRUE) */
                                   ...
                                   UI32_T prov_buffer_len)
{
    ...
    if(sess_type == CLI_TYPE_TELNET)
    {
        ...
        socklen_t    nlen = (socklen_t)sizeof(nam);
...
BOOL_T CLI_TASK_GetOnLineUserInfoByIndex(UI32_T index, CLI_TASK_OnlineUserInfo_T *user_info)
{
      ...
      cli_working_buffer[index].CMenu.AuthenState != CLI_DEF_AUTHEN_STATE_PASS)
   {
    ...
    switch (cli_working_buffer[index].sess_type)
    {
        ...
        {
            strncpy(user_info->line, CLI_TASK_TELNET_STR, sizeof(user_info->line)-1);
            user_info->line[sizeof(user_info->line) -1] = '\0';
        }
        else
        {
            strncpy(user_info->line, CLI_TASK_SSH_STR, sizeof(user_info->line)-1);
            user_info->line[sizeof(user_info->line) -1] = '\0';
        }

        if (L_INET_RETURN_SUCCESS != L_INET_InaddrToString((L_INET_Addr_T *)&cli_working_buffer[index].CMenu.TelnetIPAddress,
                                                           user_info->location,
                                                           sizeof(user_info->location)))
        {
            return FALSE;
        }
 ...
 BOOL_T CLI_TASK_Reproduce_Config(char *file_name)
{
	...
	else
    {
		...
		for(i=0;i<SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK;i++)
		{
		    ...
		    UI32_T      unit_id = 0;
		    char        sub_str[40] = {0};
		    CLI_LIB_EAtoStr(stacking_db_old[i].mac_addr, mac_str);
		    device_type = stacking_db_old[i].device_type;
		    unit_id = stacking_db_old[i].unit_id;
		    SYSFUN_Sprintf((char *)sub_str,"%02lu_%s_%02lu",(unsigned long)unit_id,mac_str,(unsigned long)device_type);
        ...
        for(i=0;i<SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK;i++)
		{
		    ...
		    CLI_LIB_EAtoStr(stacking_db_new[i].mac_addr, mac_str_new);
		    ...
		    for(j=0;j<SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK;j++)
		    {
		        ...
		        CLI_LIB_EAtoStr(stacking_db_old[j].mac_addr, mac_str_old);
		        ...
		        if(strcmp(mac_str_new,mac_str_old) == 0)
		        {
		            ...
		            {
		                SYSFUN_Sprintf(sub_str,"%02lu_%s_%02lu",(unsigned long)unit_id_new,mac_str_new,(unsigned long)device_type_new);
        ...
        if (CLI_TASK_ReadLine_From_XferBuf(cmd_buf, &tmp_xbuf))
        {
        	...
        	if (strcmp((char *)type_str, "stackingMac") == 0)
        	{
                ...
                if(content_str[0] != '0'||content_str[1] != '0')
            	{
                    ...
                    /* Add for later to check the validity.
                     */
                    CLI_MGR_AddDevice(unit_id);
    		    }
            }
        }
        else
...
/*------------------------------------------------------------------------
 * ROUTINE NAME - CLI_TASK_CloseTelnetSessions
 *------------------------------------------------------------------------
 * FUNCTION: This function will close telnet/ssh sessions by ip address
 * INPUT   : ip_addr_p  -- the local ip address which need to close
 *                         if ip_addr_p==NULL, means all telnet/ssh sessions
 * OUTPUT  : none
 * RETURN  : none
 * NOTE    :
 *-----------------------------------------------------------------------*/
void CLI_TASK_CloseTelnetSessions(L_INET_AddrIp_T *ip_addr_p)
{
    int i;
    struct  sockaddr_in6    sock_addr;
    socklen_t               sock_addr_size = sizeof(sock_addr);
    L_INET_AddrIp_T         local_addr;

    for (i = CLI_TASK_MIN_TELNET_SESSION_ID; i <= CLI_TASK_MAX_TELNET_SESSION_ID; i++)
    {
        if (cli_working_buffer[i].is_empty == FALSE &&
            cli_working_buffer[i].sess_type == CLI_TYPE_TELNET)
        {
            if (ip_addr_p)
            {
                /* get socket's local ip
                 */
                if( getsockname(cli_working_buffer[i].socket, (struct sockaddr*)&sock_addr, &sock_addr_size) != 0 )
                {
                    continue;
                }
                ...
                if ((local_addr.addrlen == ip_addr_p->addrlen) &&
                    (memcmp(local_addr.addr, ip_addr_p->addr, ip_addr_p->addrlen)==0))
                {
