...
void display_ethernet_msg_for_cfg(CLI_API_EthStatus_T status, UI32_T unit, UI32_T port)
{
   char buff[CLI_DEF_MAX_BUFSIZE] = {0};
   ...
   {        
      ...
      {
      case CLI_API_ETH_TRUNK_MEMBER:
         sprintf(buff, "Trunk member, ethernet %s, could not be configured.\r\n", name);
         break;
      
      default:
         display_ethernet_msg(status, unit, port);
      }
   }
#else
   switch(status)
   {
   case CLI_API_ETH_TRUNK_MEMBER:
      sprintf(buff, "Trunk member, ethernet %lu/%lu, could not be configured.\r\n", (unsigned long)unit, (unsigned long)port);
      break;
...
BOOL_T GetIfCounter(UI32_T ifindex, CLI_API_ShowIfCounter_T* IfCounter)
{
   ...
   if(!NMTR_PMGR_GetSystemwideIfTableStats(ifindex, &IfCounter->if_table_stats))
   {
#if (SYS_CPNT_EH == TRUE)
   ...
   if(!NMTR_PMGR_GetSystemwideIfXTableStats(ifindex, &IfCounter->if_xtable_stats))
   {
#if (SYS_CPNT_EH == TRUE)
      CLI_API_Show_Exception_Handeler_Msg();
#else
   ...
   if(!NMTR_PMGR_GetSystemwideEtherLikeStats(ifindex, &IfCounter->ether_like_stats))
   {
#if (SYS_CPNT_EH == TRUE)
      CLI_API_Show_Exception_Handeler_Msg();
#else
   ...
   if(!NMTR_PMGR_GetSystemwideEtherLikePause(ifindex, &IfCounter->ether_like_pause))
   {
#if (SYS_CPNT_EH == TRUE)
      CLI_API_Show_Exception_Handeler_Msg();
#else
   ...
   if(!NMTR_PMGR_GetSystemwideRmonStats(ifindex, &IfCounter->rmon_stats))
   {
#if (SYS_CPNT_EH == TRUE)
      CLI_API_Show_Exception_Handeler_Msg();
#else
