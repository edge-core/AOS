 ...
 */
#include "sys_pmgr.h"
#include "telnetd.h"    /* from Telnet */
#include "nudefs.h"     /* from Telnet */
...
#if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_DHCP == TRUE || SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_DHCP == TRUE)
...
static BOOL_T CLI_IO_LogDynamicProvisionInvalidOption(UI32_T option);
static BOOL_T CLI_IO_LogDynamicProvisionServerNameError();
static BOOL_T CLI_IO_LogDynamicProvisionDownloadError(char *cfg_name_p);
static BOOL_T CLI_IO_LogDynamicProvisionStartProvision(char *cfg_name_p);
static BOOL_T CLI_IO_ProcessDynamicProvision(CLI_TASK_WorkingArea_T *ctrl_P);
static BOOL_T CLI_IO_IsFileExist(const char *filename_p, FS_File_Type_T file_type);
#endif
...
BOOL_T CLI_IPCIO_HandleIPCReqMsg(SYSFUN_Msg_T *msg_p, UI32_T *need_wait_key_p)
{
    ...
    switch (l_ipcio_msg_p->ipcio_cmd)
    {
        ...
        case L_IPCIO_CMD_PRINT:
            printf("%s", l_ipcio_msg_p->ipcio_arg_buffer);
            fflush(stdout);
            break;
...
void CLI_IPCIO_ParseKey(SYSFUN_MsgQ_T msgq_handle, char ch, UI32_T *need_wait_key_p, char *buff)
{
...
static UI8_T CLI_IO_ReadFromConsole(CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    CLI_OM_EXP_GetStartupFactoryDefaultFlag(&is_provision_factory_default);
#endif
    ...
    while(1)
    {
        ...
        if ( (is_first_copy == TRUE) && (is_provision_factory_default == TRUE) )
        {
            ...
            if( (count_time%100) ==0)
            {
                if (TRUE == CLI_IO_ProcessDynamicProvision(ctrl_P))
                {
#if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_UDHCPC_CRAFT_PORT == TRUE)
                    CLI_MGR_StopUdhcpc();
#endif
...
#endif /* #if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_DHCP == TRUE || SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_UDHCPC_CRAFT_PORT == TRUE) */
      ...
      if(ch == 0)
      {
         ...
         */
         ...
         switch(ctrl_P->CMenu.AuthenState)
         {
            ...
            if (consoleCfg.exec_timeout != 0 &&     /*0: disable*/
            current_ticks - start_ticks >= consoleCfg.exec_timeout*100)
            {
#if (SYS_CPNT_EH == TRUE)
               CLI_LIB_PrintNullStr(2);
      ...
      else if (rc == 0)  /*no key in*/
      {
         ...
         */
         ...
         switch(ctrl_P->CMenu.AuthenState)
         {
            ...
            case CLI_DEF_AUTHEN_STATE_PASS:
            if (telnetCfg.exec_timeout != 0   &&   /*0: disable*/
            current_ticks - start_ticks >= telnetCfg.exec_timeout*100)
            {
...
void CLI_IO_PrintOut(CLI_TASK_WorkingArea_T *ctrl_P)
{
   ...
   case CLI_TYPE_TELNET:
   {
        ...
        {
            if (len == send(ctrl_P->socket, (char *)ctrl_P->send_buf, (I32_T)len, 0 | MSG_DONTWAIT))
            {
                break;
            }
        ...
        /* Check socket's state if timeout.
         * Set kill_itself flag if peer disconnected.
         */
        if (5 == retries)
        {
            ...
            if (getpeername(ctrl_P->socket, (struct sockaddr*)&peer_sock_addr, &peer_sock_addr_size) != 0 )
            {
                ctrl_P->kill_itself = TRUE;
            }
        }
...
#if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_DHCP == TRUE || SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_UDHCPC_CRAFT_PORT == TRUE)
/*------------------------------------------------------------
 * ROUTINE NAME - CLI_IO_LogDynamicProvisionInvalidOption
 *------------------------------------------------------------
 * PURPOSE : Log dynamic provision error: invalid option.
...
static BOOL_T CLI_IO_LogDynamicProvisionInvalidOption(UI32_T option)
{
    ...
    return SYSLOG_PMGR_AddFormatMsgEntry(&owner_info,
        DYNAMIC_PRIVISION_INVALID_OPTION_MESSAGE_INDEX,
        &option,
        0,
        0);
}
...
/*------------------------------------------------------------
 * ROUTINE NAME - CLI_IO_LogDynamicProvisionServerNameError
 *------------------------------------------------------------
 * PURPOSE : Log dynamic provision error: server name cannot
 *           be resolved.
 * INPUT   : cfg_name_p -- Config file name.
 * OUTPUT  : None.
 * RETURN  : TRUE/FALSE.
 *------------------------------------------------------------
 */
static BOOL_T CLI_IO_LogDynamicProvisionServerNameError()
{
    ...
    return SYSLOG_PMGR_AddFormatMsgEntry(&owner_info,
        DYNAMIC_PRIVISION_SERVER_NAME_CANNOT_RESOLVED_MESSAGE_INDEX,
        0,
        0,
        0);
}
...
/*------------------------------------------------------------
 * ROUTINE NAME - CLI_IO_LogDynamicProvisionDownloadError
 *------------------------------------------------------------
 * PURPOSE : Log dynamic provision error: download config.
 * INPUT   : cfg_name_p -- Config file name.
 * OUTPUT  : None.
 * RETURN  : TRUE/FALSE.
 *------------------------------------------------------------
...
static BOOL_T CLI_IO_LogDynamicProvisionDownloadError(char *cfg_name_p)
{
    ...
    return SYSLOG_PMGR_AddFormatMsgEntry(&owner_info,
        DYNAMIC_PRIVISION_DOWNLOAD_CFG_ERROR_MESSAGE_INDEX,
        cfg_name_p,
        0,
        0);
}
...
/*------------------------------------------------------------
 * ROUTINE NAME - CLI_IO_LogDynamicProvisionStartProvision
 *------------------------------------------------------------
 * PURPOSE : Log dynamic provision starts to provision.
 * INPUT   : cfg_name_p -- Config file name.
 * OUTPUT  : None.
 * RETURN  : TRUE/FALSE.
 *------------------------------------------------------------
...
static BOOL_T CLI_IO_LogDynamicProvisionStartProvision(char *cfg_name_p)
{
    ...
    return SYSLOG_PMGR_AddFormatMsgEntry(&owner_info,
        DYNAMIC_PRIVISION_START_PROVISION_MESSAGE_INDEX,
        cfg_name_p,
        0,
        0);
}
...
static BOOL_T CLI_IO_ProcessDynamicProvision(CLI_TASK_WorkingArea_T *ctrl_P)
{
    ...
    char server_ip_buf[SYS_ADPT_DHCP_MAX_DOMAIN_NAME_LEN+1] = {0};
    char src_filename[MAXSIZE_tftpSrcFile+1] = {0};
    char dst_filename[SYS_ADPT_FILE_SYSTEM_NAME_LEN + 1] = {0};
    ...
    UI32_T server_ip_len = 0;
    UI32_T src_filename_len = 0;
    BOOL_T dhcp_config_staus, xfer_result;
    BOOL_T is_dst_filename_exist = FALSE;

    memset(&server_ip, 0, sizeof(server_ip));

    if (FALSE == CLI_MGR_Get_DhcpRxOptionConfig(server_ip_buf, &server_ip_len,
    ...
    if (0 == server_ip_len)
    {
        CLI_LIB_PrintStr("Invalid TFTP server name in DHCP option 66.\r\n");
        CLI_IO_LogDynamicProvisionInvalidOption(66);
        return TRUE;
    }
    else if (0 == src_filename_len)
    {
        CLI_LIB_PrintStr("Invalid bootfile name in DHCP option 67.\r\n");
        CLI_IO_LogDynamicProvisionInvalidOption(67);
        return TRUE;
    }
    ...
    {
        CLI_LIB_PrintStr_1("\nShould not download Factory default file %s.\r\n", SYS_DFLT_restartConfigFile);
        CLI_IO_LogDynamicProvisionInvalidOption(67);
        return TRUE;
    }

    /* Trim the filename if remote filename is exceed the max. length of the
     * local filesystem.
     */
    strncpy(dst_filename, src_filename, sizeof(dst_filename) - 1);
    dst_filename[sizeof(dst_filename) - 1] = '\0';

    if (L_INET_RETURN_SUCCESS != L_INET_StringToInaddr(L_INET_FORMAT_IP_UNSPEC,
                                                       server_ip_buf,
                                                       (L_INET_Addr_T *) &server_ip,
                                                       sizeof(server_ip)))
    {
        if(FALSE == CLI_LIB_HostStringToInaddr((UI8_T *)server_ip_buf, AF_INET, &server_ip))
        {
            CLI_LIB_PrintStr_1("\nFailed to convert %s to IP address.\r\n", server_ip_buf);
            CLI_IO_LogDynamicProvisionServerNameError();
            return TRUE;
        }
    }

    is_dst_filename_exist = CLI_IO_IsFileExist(dst_filename, FS_FILE_TYPE_CONFIG);
    ...
    if (FALSE == XFER_PMGR_CopyFile(&user_info,
                                    publickey_username,
                                    0,
                                    &server_ip,
                                    (UI8_T*)dst_filename,
                                    ...
                                    ))
    {
        ...
        CLI_IO_LogDynamicProvisionDownloadError(dst_filename);
        return TRUE;
    }
    ...
    if (TRUE == xfer_result)
    {
        ...
        if (TRUE == is_dst_filename_exist)
        {
            ...
            SYSLOG_PMGR_AddFormatMsgEntry(&owner_info,
                DYNAMIC_PRIVISION_CFG_OVERWRITE_MESSAGE_INDEX,
                dst_filename,
                0,
                0);
        }

        if( FS_SetStartupFilename(DUMMY_DRIVE,FS_FILE_TYPE_CONFIG, (UI8_T*)dst_filename)!=FS_RETURN_OK )
        {
        ...
        strcpy((char *)xfer_buf, CLI_MGR_RUNTIME_PROVISION_MAGIC_WORD);
         ...
         if (FS_RETURN_OK != XFER_PMGR_LocalToStream(&user_info,
                                                     (UI8_T *)dst_filename,
        ...
        CLI_IO_LogDynamicProvisionStartProvision(dst_filename);
...
static BOOL_T CLI_IO_IsFileExist(const char *filename_p, FS_File_Type_T file_type)
{
    ...
    strncpy((char *)file_attr.file_name, filename_p, sizeof(file_attr.file_name) - 1);
    file_attr.file_name[sizeof(file_attr.file_name) - 1] = '\0';

    return (FS_RETURN_OK == FS_GetFileInfo(unit_id, &file_attr));
}
#endif /* #if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_DHCP == TRUE || SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_UDHCPC_CRAFT_PORT == TRUE) */
...
int CLI_IO_DebugMode_GetChar(void *cookie)
{
    ...
    UI32_T sess_id = (uintptr_t)cookie;
    int ch;
