   ...
   char name[MAXSIZE_ifName+1] = {0};
    ...
    if (arg[0] == NULL)
    {
        ...
        while (SWCTRL_POM_GetNextMirrorEntry(&mirroring_entry))
        {
            ...
            sprintf(name, "Eth%2lu/%2lu", (unsigned long)unit, (unsigned long)port);
#endif
        ...
        while (SWCTRL_PMGR_GetNextVlanMirrorEntry(&vlan_mirror_entry))
        {
            ...
            {
#if (CLI_SUPPORT_PORT_NAME == 1) 
                CLI_LIB_Ifindex_To_Name(vlan_mirror_entry.mirror_dest_port, name);
#else
...
#if (CLI_SUPPORT_PORT_NAME == 1) 
                CLI_LIB_Ifindex_To_Name(dest_port, name);
#else
...
#if (SYS_CPNT_ACL_MIRROR == TRUE)
        {
            UI32_T  acl_dest_port, tmp_dest_port;
            UI32_T  acl_index=0;
            ...
            acl_dest_port = 0;
            tmp_dest_port = acl_dest_port;
            while (RULE_TYPE_OK == L4_PMGR_ACL_GetNextAclMirrorEntry(&acl_dest_port, &acl_index, &mirror_type, &acl_entry) )
            {
                if (tmp_dest_port != acl_dest_port)
                {
                    tmp_dest_port = acl_dest_port;
                    SWCTRL_POM_LogicalPortToUserPort(acl_dest_port, &unit, &port, &trunk_id);
                    sprintf(buff, "\r\n Destination Port (listen port)  : Eth%lu/%2lu\r\n",(unsigned long)unit,(unsigned long)port);
                    PROCESS_MORE(buff);
                }
...
#else
                SWCTRL_POM_LogicalPortToUserPort(vlan_mirror_entry.mirror_dest_port, &unit, &port, &trunk_id);
                sprintf(name, "Eth%2lu/%2lu", unit, port);
#endif
    ...
    if (*arg[0] == 'a' || *arg[0] == 'A')
    {
        ...
        if(arg[2]!= NULL)
        {
            switch(*arg[2])
            {
                ...
                case 't':
                case 'T':
                    mode = VAL_aclMirrorType_tx;
                    break;
          
                case 'b':
                case 'B':
                default:
                    mode = VAL_aclMirrorType_both;     
                    break;
            }
        }
#endif
        ...
        {
            case PRIVILEGE_CFG_INTERFACE_ETH_CMD_W2_PORT_MONITOR:
                if (RULE_TYPE_OK != L4_PMGR_ACL_AddAclMirrorEntry(dest_ifindex, arg[1], mode))
                {
 ...
 *--------------------------------------------------------*/
    ...
    if (*arg[0] == 'e' || *arg[0] == 'E')
    {
        ...
        if (ctrl_P->sess_type == CLI_TYPE_PROVISION)
        {
            ...
            {
                sprintf((char*)cmd_buff,"!\ninterface ethernet %lu/%lu\n", (unsigned long)ctrl_P->CMenu.unit_id, (unsigned long)verify_dest_port);
