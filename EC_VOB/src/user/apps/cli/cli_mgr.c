...
#define CLI_USE_CSC_CHECK_OPER_MODE(RET_VAL, function_no)                             \
    if (CLI_OM_GetOperatingMode() != SYS_TYPE_STACKING_MASTER_MODE)  {   \
     return (RET_VAL);                                                      \
  }

#define CLI_USE_CSC_CHECK_OPER_MODE_WITHOUT_RETURN_VALUE(function_no)               \
    if (CLI_OM_GetOperatingMode() == SYS_TYPE_STACKING_SLAVE_MODE)  {   \
    return;                                                                 \
  }
...
#if ( SYS_CPNT_UNIT_HOT_SWAP == TRUE )
static BOOL_T CLI_MGR_ClearDeviceConfig(UI32_T device_id);
#else
static BOOL_T CLI_MGR_ClearModuleConfig(UI32_T module_id);
#endif
...
static void CLI_MGR_RifDestroyedCallbackHandler(UI32_T ifindex, L_INET_AddrIp_T *ipaddr_p);
...
static UI32_T  device_cfg_cursor[ CLI_MGR_NUMBER_OF_TOTAL_DEVICE ];
static BOOL_T  device_valid[CLI_MGR_NUMBER_OF_TOTAL_DEVICE];
#else
...
static char     stacking_buf [CLI_DEF_MAX_HEADER*CLI_DEF_MAX_DISP_WIDTH];
static UI32_T    cli_provision_dirtybit[ SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK ];

#if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_UDHCPC_CRAFT_PORT == TRUE) /* to support DHCP Cllient on CRAFT port while using Busybox's udhcpc */
    static BOOL_T cli_mgr_is_done_udhcpc = FALSE;
#endif
...
static char  cli_mgr_dhcp_rx_option_config_address[SYS_ADPT_DHCP_MAX_DOMAIN_NAME_LEN + 1]={0};
static UI32_T cli_mgr_dhcp_rx_option_config_address_len = 0;
static char  cli_mgr_dhcp_rx_option_config_filename_ar[MAXSIZE_tftpSrcFile+1]={0};
static UI32_T cli_mgr_dhcp_rx_option_config_filename_len = 0;
#endif  /* #if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_DHCP == TRUE) */

#if (SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG == TRUE)
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_ADDRUNCONFIG_Notify
 * ------------------------------------------------------------------------
 * PURPOSE  : This API is used to notify CLI addrunconfig task to do partial provision
 * INPUT    : None
 * OUTPUT   : None
 * RETURN   : None
 * NOTE     : None
 * ------------------------------------------------------------------------
 */
static void CLI_MGR_ADDRUNCONFIG_Notify();

/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_ADDRUNCONFIG_DeleteExpiredEntry
 * ------------------------------------------------------------------------
 * PURPOSE  : This API is used to delete the expired done entry
 * INPUT    : None
 * OUTPUT   : None
 * RETURN   : None
 * NOTE     : None
 * ------------------------------------------------------------------------
 */
static void CLI_MGR_ADDRUNCONFIG_DeleteExpiredEntry();
...
/*   if(SYSFUN_CreateSem (1, SYSFUN_SEM_FIFO, &cli_mgr_semaphore_id) != SYSFUN_OK) */
   ...
   STKCTRL_TASK_Register_SavingConfig_CallBack(&CLI_MGR_Set_StkctrlSavingConfigStatus);
    ...
    memset( device_cfg_cursor, 0, sizeof( device_cfg_cursor) );
    memset(device_valid, 0, sizeof(device_valid));
#else
    memset(module_cfg, 0, sizeof(module_cfg));
    memset(module_cfg_cursor, 0, sizeof(module_cfg_cursor));
#endif
...
void CLI_MGR_Create_InterCSC_Relation(void)
{
    ...
    */
    BACKDOOR_MGR_Register_SubsysBackdoorFunc_CallBack("CLI", SYS_BLD_CLI_GROUP_IPCMSGQ_KEY, CLI_BACKDOOR_Main);
...
{
    CLI_OM_EnterTransitionMode();
    /* cli can not process IPC when cli stay in transition mode,
     * so need to reset CLI_MGR_current_session_count here.
     */
    CLI_MGR_current_session_count = 0;
...
{
    return CLI_OM_GetOperatingMode();
}
...
void CLI_MGR_Set_XferProgressStatusByWorkArea(CLI_TASK_WorkingArea_T *ctrl_p, BOOL_T Is_Xfer_In_Progress)
{
    if (NULL != ctrl_p)
        ctrl_p->xfer_in_prog = Is_Xfer_In_Progress;
}

void CLI_MGR_Set_XferProgressStatus(BOOL_T Is_Xfer_In_Progress)
{
    CLI_MGR_Set_XferProgressStatusByWorkArea(
        CLI_TASK_GetMyWorkingArea(), Is_Xfer_In_Progress);
}

void CLI_MGR_Get_XferProgressStatusByWorkArea(CLI_TASK_WorkingArea_T *ctrl_p, BOOL_T *result)
{
    if (NULL != ctrl_p)
        *result = ctrl_p->xfer_in_prog;
    else
        *result = FALSE;
}
...
{
    CLI_MGR_Get_XferProgressStatusByWorkArea(
        CLI_TASK_GetMyWorkingArea(), result);
}

void CLI_MGR_Set_XferProgressResultByWorkArea(CLI_TASK_WorkingArea_T *ctrl_p, BOOL_T result)
{
    if (NULL != ctrl_p)
        ctrl_p->xfer_prog_result = result;
}

void CLI_MGR_Set_XferProgressResult(BOOL_T result)
{
    CLI_MGR_Set_XferProgressResultByWorkArea(
        CLI_TASK_GetMyWorkingArea(), result);
}

void CLI_MGR_Get_XferProgressResultByWorkArea(CLI_TASK_WorkingArea_T *ctrl_p, BOOL_T *result)
{
    if (NULL != ctrl_p)
        *result = ctrl_p->xfer_prog_result;
    else
        *result = FALSE;
}
...
{
    CLI_MGR_Get_XferProgressResultByWorkArea(
        CLI_TASK_GetMyWorkingArea(), result);
}
...
#if (SYS_CPNT_3COM_CLI == TRUE)
void CLI_MGR_XferCopy_Callback(void *cookie, UI32_T status)
{
...
void CLI_MGR_XferCopy_Callback(void *cookie, UI32_T status)
{
    BOOL_T xfer_is_progressing;

    CLI_MGR_Get_XferProgressStatusByWorkArea((void *)cookie, &xfer_is_progressing);

    if (FALSE == xfer_is_progressing)
    {
        return;
    }
    ...
    {
        case XFER_MGR_FILE_COPY_COMPLETED: /*The final message*/
            CLI_MGR_Set_XferProgressStatusByWorkArea((void *)cookie, FALSE);
            CLI_MGR_xfer_callback_status = XFER_MGR_FILE_COPY_COMPLETED;
            break;

        case XFER_MGR_FILE_COPY_SUCCESS:
            CLI_MGR_Set_XferProgressResultByWorkArea((void *)cookie, TRUE);
            CLI_MGR_xfer_callback_status = XFER_MGR_FILE_COPY_SUCCESS;
            break;

        case XFER_MGR_FILE_COPY_ERROR:
            CLI_MGR_Set_XferProgressResultByWorkArea((void *)cookie, FALSE);
            CLI_MGR_xfer_callback_status = XFER_MGR_FILE_COPY_ERROR;
            break;
...
BOOL_T CLI_MGR_SaveRunningCfg(char *filename)
{
   ...
   time_ticks_b = SYS_TIME_GetSystemTicksBy10ms();
    ...
    if (!XFER_PMGR_StreamToLocal(&user_info,
                                 (UI8_T *)filename,
                                 FS_FILE_TYPE_CONFIG,
                                 0,
...
void CLI_MGR_HandleHotInsertion( UI32_T starting_port_ifindex, UI32_T number_of_port, BOOL_T use_default )
{
    ...
    else
    {
        if ( STKTPLG_POM_IsModulePort( unit, port ) == TRUE )
        {
            CLI_MGR_ClearDeviceConfig(unit + SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK);
            /* add by fen.wang,sometimes stktplg cannot assign right unit id,
             * because the unit id from cli is not correct for when do hot add ,
             * cli will not save the topo when hotswap add ,save the topo info
             * in flash
             */
#if 0
...
#else
            {
                ...
                if(is_provision != TRUE)
                {
                   ...
                   if(CLI_TASK_Reproduce_Config(file_name))
                   {
                      ...
                      CLI_OM_EXP_SetIsReproduceDone(TRUE);
#endif
        ...
        {
            CLI_MGR_ClearDeviceConfig(unit);
            /* add by fen.wang,sometimes stktplg cannot assign right unit id,
             * because the unit id from cli is not correct for when do hot add ,
             * cli will not save the topo when hotswap add ,save the topo info
             * in flash
    ...
    if (FALSE == use_default)
    {
        ...
        CLI_TASK_CreateTask( CLI_TYPE_MODULE_PROVISION,
                             ...
                             unit,
                             module_cfg[unit - 1],
                             module_cfg_cursor[unit -1]);
    }
    else
    {
        CLI_MGR_ClearModuleConfig(unit);
        CLI_MGR_Notify_ModuleProvisionComplete(unit);
    }
    return;
...
void CLI_MGR_HandleHotRemoval(UI32_T starting_port_ifindex, UI32_T number_of_port)
{
    ...
    if ( STKTPLG_POM_IsModulePort( unit, port ) == TRUE )
    {
        ...
        device_cfg_cursor[unit + SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK - 1] = strlen(device_cfg[unit + SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK - 1]);
    }
    else if ( master_id != unit )
    {
        ...
        device_cfg_cursor[unit - 1] = strlen(device_cfg[unit - 1]);
    }
    else
    {
        ;
    ...
    module_cfg_cursor[unit - 1] = strlen(module_cfg[unit - 1]);
...
void CLI_MGR_AppendModuleCfg(UI32_T cmd_index, CLI_STRLB_CMD_T *cmd, UI8_T *pre_attr, UI8_T *post_attr)
{
    ...
    {
        cmd_string_len = strlen(cmd[unit].unit_port_string);
        /* Check whether the left buffer space is large enough
         * Need to reserve 1 byte for '\0'
         */
        if ( (cmd_string_len > 0) &&
            (module_cfg_cursor[unit] + cmd_string_len) < CLI_MGR_RUNTIME_PROVISION_UC_MEM_SIZE)
        {
#if 0
            memset(cmd_string, 0, sizeof(cmd_string));
            CLI_RUNCFG_ConstructCmd(cmd_index, cmd[unit].unit_port_string, pre_attr, post_attr, cmd_string);
#endif
            sprintf( module_cfg[unit] + module_cfg_cursor[unit],
                "%s\n", cmd[unit].unit_port_string);
            module_cfg_cursor[unit] += cmd_string_len + 1; /* +1 for adding '\n' */
        }
    }
}
#endif
...
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_AddDevice
 * ------------------------------------------------------------------------
 * PURPOSE  : This function add the specified unit to array which is used
 *            for later to determine the unit is valid or not.
 * INPUT    : unit_id       -- unit ID
 * OUTPUT   : None.
 * RETURN   : TRUE/FALSE
 * NOTE     : If hot swap feature is not enabled, this function do nothing
 *            and always returns TRUE.
 * ------------------------------------------------------------------------
 */
BOOL_T CLI_MGR_AddDevice(UI32_T unit_id)
{
#if (SYS_CPNT_UNIT_HOT_SWAP == TRUE)
    if (   (unit_id == 0)
        || (unit_id > CLI_MGR_NUMBER_OF_TOTAL_DEVICE)
        )
    {
        return FALSE;
    }

    device_valid[unit_id - 1] = TRUE;
...
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_SaveCmdInfo
 * ------------------------------------------------------------------------
 * PURPOSE  : This function saves command info (set related flags)
 *            according to current flag, session type and the unit ID
 ...
 * ------------------------------------------------------------------------
 */
void CLI_MGR_SaveCmdInfo(UI32_T unit_id)
{
#if (SYS_CPNT_UNIT_HOT_SWAP == TRUE)
    ...
    /* Save the command if this unit is valid when in provision.
     */
    if (   (   (ctrl_p->sess_type == CLI_TYPE_PROVISION)
            || (ctrl_p->sess_type == CLI_TYPE_MODULE_PROVISION))
        && (ctrl_p->cmd_info.eth_mode_flag == FALSE)
        && (device_valid[unit_id - 1] == TRUE)
        )
    {
        ctrl_p->cmd_info.unit_cmd_flag = TRUE;
        ctrl_p->cmd_info.unit_id = unit_id;
    }
#endif /* #if (SYS_CPNT_UNIT_HOT_SWAP == TRUE) */
}

/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_SaveCfgToDeviceCfg
 * ------------------------------------------------------------------------
 * PURPOSE  : This function saves command into device cfg if the commands
 *            are unit-specific commands.
 * INPUT    : cmd_eidx  -- the ID of last command argument in the command
 *            cmd_buf_p -- command
 * OUTPUT   : None.
 * RETURN   : None.
 * NOTE     : If hot swap feature is not enabled, this function do nothing.
 * ------------------------------------------------------------------------
 */
void CLI_MGR_SaveCfgToDeviceCfg(UI16_T cmd_eidx, char *cmd_buf_p)
{
#if (SYS_CPNT_UNIT_HOT_SWAP == TRUE)
    CLI_TASK_WorkingArea_T *ctrl_p = (CLI_TASK_WorkingArea_T *)CLI_TASK_GetMyWorkingArea();
    ...
    /* Set the flag to indicate that the following commands are necessary
     * to be saved until to leave from Ethernet mode.
     */
    if (   (ctrl_p->cmd_info.unit_cmd_flag == TRUE)
        && (cmd_eidx == PRIVILEGE_CFG_GLOBAL_CMD_W2_INTERFACE_ETHERNET)
        )
    {
        ctrl_p->cmd_info.eth_mode_flag = TRUE;
    }

    /* The commands with valid unit or the commands under Ethernet modes with
     * valid unit are necessary to be saved.
     */
    if (   (ctrl_p->cmd_info.unit_cmd_flag == TRUE)
        || (ctrl_p->cmd_info.eth_mode_flag == TRUE)
        )
    {
        CLI_MGR_AddDeviceCfg(ctrl_p->cmd_info.unit_id, (UI8_T *)cmd_buf_p);
    }
#endif /* #if (SYS_CPNT_UNIT_HOT_SWAP == TRUE) */
}
...
UI8_T CLI_MGR_GetStackingDB(STKTPLG_OM_StackingDB_T stacking_db[SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK])
{
    ...
    fs_ret = FS_ReadFile( DUMMY_DRIVE, file_attr.file_name ,  (UI8_T *)tmp_xbuf, CLI_DEF_MAX_HEADER*CLI_DEF_MAX_DISP_WIDTH, &xfer_buf_length);
...
void CLI_MGR_AddDeviceCfg( UI32_T device_id, UI8_T * cmd_buffer )
{
    ...
    cmd_buffer_len = strlen((char*)cmd_buffer);
    ...
    {
        /* Check whether the left space of device_cfg is large enough
         * Need to reserve 1 byte for '\0'
         */
        if ((device_cfg_cursor[device_id-1] + cmd_buffer_len) < CLI_MGR_RUNTIME_PROVISION_UC_MEM_SIZE)
        {
            sprintf( device_cfg[device_id-1] + device_cfg_cursor[device_id-1],
                "%s\n", cmd_buffer);
            device_cfg_cursor[device_id-1] += cmd_buffer_len + 1; /* +1 for adding '\n' */
        }
        else
        {
            #if 0 /* for debug */
            printf("%s(%d):device_cfg overflow. device_id=%lu, cursor=%lu, add_cmd_len=%d\r\n",
                __FUNCTION__, __LINE__, device_id, device_cfg_cursor[device_id-1], (int)cmd_buffer_len);
            #endif
        }
    }
    return;
...
void CLI_MGR_AddModuleCfg(UI32_T unit, UI8_T *cmd_buffer)
{
    ...
    else
    {
         ...
         * Need to reserve 1 byte for '\0'
         */
        if ((module_cfg_cursor[unit-1] + cmd_buffer_len) < CLI_MGR_RUNTIME_PROVISION_UC_MEM_SIZE)
        {
            sprintf( module_cfg[unit-1] + module_cfg_cursor[unit-1],
                "%s\n", cmd_buffer);
            module_cfg_cursor[unit-1] += cmd_buffer_len + 1; /* +1 for adding '\n' */
        }
        else
        {
            #if 0 /* for debug */
            printf("%s(%d):module_cfg overflow. unit=%lu, cursor=%lu, add_cmd_len=%d\r\n",
                __FUNCTION__, __LINE__, unit, module_cfg_cursor[unit-1], (int)cmd_buffer_len);
            #endif
        }
...
BOOL_T CLI_MGR_GetModuleCfg(UI32_T unit, UI8_T *module_buffer, UI32_T *module_buffer_len)
{
    if(unit > SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK || unit < 1)
        return FALSE;
    module_buffer = (UI8_T*)module_cfg[unit - 1];
    *module_buffer_len = module_cfg_cursor[unit - 1];
    return TRUE;
}
#endif
...
#if (SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG == TRUE)
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_ADDRUNCONFIG_CreateEntry
 * ------------------------------------------------------------------------
  * PURPOSE  : This API is used to create addrunconfig entry
 * INPUT    : commands -- commands for creating running-config on the device
 * OUTPUT   : id       -- entry id
 ...
 * ------------------------------------------------------------------------
 */
CLI_OM_EXP_ADDRUNCONFIG_RETURN_T CLI_MGR_ADDRUNCONFIG_CreateEntry(const char *commands, UI32_T *id)
{
    ...
    *id = abs(SYSFUN_TaskIdSelf() + SYSFUN_GetSysTick() + rand());

    ret = CLI_OM_EXP_ADDRUNCONFIG_CreateEntry(commands, id);
    if (ret == CLI_OM_EXP_ADDRUNCONFIG_SUCCESS)
    {
        CLI_MGR_ADDRUNCONFIG_Notify();
        return CLI_OM_EXP_ADDRUNCONFIG_SUCCESS;
    }
    else if (ret == CLI_OM_EXP_ADDRUNCONFIG_NO_MORE_EMPTY_ENTRY)
    {
        CLI_MGR_ADDRUNCONFIG_DeleteExpiredEntry();
...
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_ADDRUNCONFIG_DeleteEntryById
 * ------------------------------------------------------------------------
 ...
 * ------------------------------------------------------------------------
 */
CLI_OM_EXP_ADDRUNCONFIG_RETURN_T CLI_MGR_ADDRUNCONFIG_DeleteEntryById(UI32_T id)
{
    CLI_OM_EXP_ADDRUNCONFIG_RETURN_T ret;
    CLI_OM_EXP_ADDRUNCONFIG_ENTRY_T entry;

    ret = CLI_OM_EXP_ADDRUNCONFIG_GetEntryById(id, &entry);
    if (ret == CLI_OM_EXP_ADDRUNCONFIG_SUCCESS)
    {
        if (entry.status == CLI_OM_EXP_ADDRUNCONFIG_ENTRY_RUNNING)
        {
            return CLI_OM_EXP_ADDRUNCONFIG_NOT_ALLOW;
        }
        else
        {
            return CLI_OM_EXP_ADDRUNCONFIG_DeleteEntryById(id);
        }
    }
    else
    {
...
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_ADDRUNCONFIG_GetEntryById
 * ------------------------------------------------------------------------
 ...
 *            CLI_OM_EXP_ADDRUNCONFIG_SUCCESS
 * NOTE     : If the entry status is done, will be deleted
 * ------------------------------------------------------------------------
 */
CLI_OM_EXP_ADDRUNCONFIG_RETURN_T CLI_MGR_ADDRUNCONFIG_GetEntryById(UI32_T id, CLI_OM_EXP_ADDRUNCONFIG_ENTRY_T *entry)
{
    CLI_OM_EXP_ADDRUNCONFIG_RETURN_T ret;
    ...
    if (ret == CLI_OM_EXP_ADDRUNCONFIG_SUCCESS)
    {
        if (entry->status == CLI_OM_EXP_ADDRUNCONFIG_ENTRY_INVALID)
        {
            return CLI_OM_EXP_ADDRUNCONFIG_NOT_FOUND;
        }
        else if (entry->status == CLI_OM_EXP_ADDRUNCONFIG_ENTRY_DONE)
        {
            CLI_OM_EXP_ADDRUNCONFIG_DeleteEntryById(entry->id);
            return CLI_OM_EXP_ADDRUNCONFIG_SUCCESS;
        }
        else
        {
...
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_ADDRUNCONFIG_IsTriggerByAddRunconfigTask
 * ------------------------------------------------------------------------
 * PURPOSE  : This API is used to check if provision is triggered by addrunconfig task
 * INPUT    : None
 * OUTPUT   : None
 * RETURN   : TRUE/FALSE
 * NOTE     : None
 * ------------------------------------------------------------------------
 */
BOOL_T CLI_MGR_ADDRUNCONFIG_IsTriggerByAddRunconfigTask()
{
    return CLI_OM_EXP_ADDRUNCONFIG_HasRunningEntry();
}

/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_ADDRUNCONFIG_Done
 * ------------------------------------------------------------------------
 * PURPOSE  : This API is used to update the status and finish time
 * INPUT    : buf_p -- buffer pointer that allocated by BUFFER_MGR
 * OUTPUT   : None
 * RETURN   : None
 * NOTE     : None
 * ------------------------------------------------------------------------
 */
void CLI_MGR_ADDRUNCONFIG_Done(void *buf_p)
{
    CLI_OM_EXP_ADDRUNCONFIG_Done(SYSFUN_GetSysTick());
    BUFFER_MGR_Free(buf_p);
}
#endif /* SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG */
...
void CLI_MGR_HandleSysCallbackIPCMsg(SYSFUN_Msg_T* msgbuf_p)
{
    ...
    switch(sys_cbmsg_p->callback_event_id)
    {
        ...
        case SYS_CALLBACK_MGR_CALLBACKEVENTID_RIF_DESTROYED:
		    SYS_CALLBACK_MGR_HandleIPCMsgAndCallback(msgbuf_p, CLI_MGR_RifDestroyedCallbackHandler);
            break;
...
BOOL_T CLI_MGR_HandleIPCReqMsg(SYSFUN_Msg_T* ipcmsg_p)
{
     ...
     */
    if (CLI_OM_GetOperatingMode() == SYS_TYPE_STACKING_TRANSITION_MODE)
    {
...
#if (SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG == TRUE)
    case CLI_MGR_IPC_PARTIAL_PROVISION:
        CLI_MGR_Set_BuffermgmtOffset(mgr_msg_p->data.running_config_data.buffer);
        CLI_MGR_PartialProvision();
        return FALSE;

    case CLI_MGR_IPC_ADDRUNCONFIG_CREATE_ENTRY:
        ipcmsg_p->msg_size=sizeof(mgr_msg_p->data.create_addrunconfig_entry_data)+CLI_MGR_MSGBUF_TYPE_SIZE;
        mgr_msg_p->type.result = CLI_MGR_ADDRUNCONFIG_CreateEntry(mgr_msg_p->data.create_addrunconfig_entry_data.commands, &mgr_msg_p->data.create_addrunconfig_entry_data.id);
        break;

    case CLI_MGR_IPC_ADDRUNCONFIG_DELETE_ENTRY_BY_ID:
        ipcmsg_p->msg_size=CLI_MGR_MSGBUF_TYPE_SIZE;
        mgr_msg_p->type.result = CLI_MGR_ADDRUNCONFIG_DeleteEntryById(mgr_msg_p->data.ui32_data.ui32);
        break;

    case CLI_MGR_IPC_ADDRUNCONFIG_GET_ENTRY_BY_ID:
        ipcmsg_p->msg_size=sizeof(mgr_msg_p->data.get_addrunconfig_entry_data)+CLI_MGR_MSGBUF_TYPE_SIZE;
        mgr_msg_p->type.result = CLI_MGR_ADDRUNCONFIG_GetEntryById(mgr_msg_p->data.get_addrunconfig_entry_data.id, &mgr_msg_p->data.get_addrunconfig_entry_data.entry);
        break;
#endif  /* SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG */

    case CLI_MGR_IPC_GET_MAX_COMMAND_PRIVILEGE:
        ipcmsg_p->msg_size = CLI_MGR_MSGBUF_TYPE_SIZE;
        mgr_msg_p->type.result = CLI_MGR_Get_Max_Command_Privilege(mgr_msg_p->data.max_command_privilege_data.cmd_idx,
                                                                    mgr_msg_p->data.max_command_privilege_data.privilege_mode);
        break;
...
/* -------------------------------------------------------------------------
 * ROUTINE NAME - CLI_MGR_Get_Max_Command_Privilege
 * -------------------------------------------------------------------------
 ...
 * -------------------------------------------------------------------------
 */
UI16_T CLI_MGR_Get_Max_Command_Privilege(
    UI16_T  cmd_idx,
    UI8_T  privilege_mode)
{
    ...
    max_pri = CLI_LIB_GetMaxCommandPrivilege(cmd_idx, privilege_mode);
#else
    max_pri = SYS_ADPT_MAX_LOGIN_PRIVILEGE;
...
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_RifDestroyedCallbackHandler
 * ------------------------------------------------------------------------
 * PURPOSE  : Handler function of CLI when a rif is destroyed
 * INPUT    : ifindex    -- interface where the rif is located
...
static void CLI_MGR_RifDestroyedCallbackHandler(UI32_T ifindex, L_INET_AddrIp_T *ipaddr_p)
{
    CLI_TASK_CloseTelnetSessions(ipaddr_p);
}

#if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_UDHCPC_CRAFT_PORT == TRUE)
/*-----------------------------------------------------------
 * ROUTINE NAME -  CLI_MGR_StopUdhcpc
 *-----------------------------------------------------------
 * FUNCTION: Stop udhcpc for zero touch config.
 * INPUT   : None
 * OUTPUT  : None
 * RETURN  : None
 * NOTE    : None
 *----------------------------------------------------------*/
void CLI_MGR_StopUdhcpc(void)
{
    #define UDHCPC_PID_FILE "/var/run/udhcpc.pid"
    char  buf[100];
    /* if udhcpc is still running -> terminate it
     * since front port has got the config file to do dynamic provision
     */
    FILE *fd=fopen(UDHCPC_PID_FILE, "r");
    if (fd != NULL) /* it's still not terminate yet */
    {
        fclose(fd);
        /* release IP doesn't work. maybe because we doesn't set the IP to kernel */
        sprintf(buf, "kill -15 `cat %s`", UDHCPC_PID_FILE);
        system(buf); /* to terminate udhcpc process */
    }
}

/*-----------------------------------------------------------
 * ROUTINE NAME - CLI_MGR_SetDoneFlagUdhcpc
 *-----------------------------------------------------------
 * FUNCTION: Set done flag for udhcpc.
 * INPUT   : is_done - TRUE/FALSE
 * OUTPUT  : None
 * RETURN  : None
 * NOTE    : None
 *----------------------------------------------------------*/
void CLI_MGR_SetDoneFlagUdhcpc(BOOL_T is_done)
{
    cli_mgr_is_done_udhcpc = is_done;
}

/*-----------------------------------------------------------
 * ROUTINE NAME - CLI_MGR_RunUdhcpc
 *-----------------------------------------------------------
 * FUNCTION: Run udhcpc for zero touch config.
 * INPUT   : count_time - count_time from cli_io
 * OUTPUT  : None
 * RETURN  : None
 * NOTE    : None
 *----------------------------------------------------------*/
void CLI_MGR_RunUdhcpc(UI32_T count_time)
{
    //            #define DHCLIENT_CRAFT_ZTP_PID_FILE "/var/run/dhclient_craft_ZTP.pid"
     ...
     */
    if ((cli_mgr_is_done_udhcpc == FALSE) && (count_time%100==0))
    {
        /* should not use -R option because using it will have the following error
         * udhcpc: bind(UDP): Cannot assign requested address
         * Entering released state
         */
        #define UDHCPC_CMD_TMP "udhcpc -i CRAFT -V %s -r %s -b -O tftp -O bootfile -p /var/run/udhcpc.pid -S -s /etc/udhcpc.sh -A 60 > /dev/null 2>&1 &"

        UI32_T id_len = 0;
        UI8_T  model_name[SYS_ADPT_MAX_MODEL_NAME_SIZE+1] = {0};
        char   vendor_buf[DHCP_MGR_CLASSID_BUF_MAX_SIZE+1] = {0};
        char   ipv4_addr[16] = {0};
        char   dhcpc_cmd[300];
        int    rc = 0;
        BOOL_T ip_addr_ready = FALSE;

        /* make sure IP is ready for next tftp get config file first
         */
        {
            ...
            {
                sprintf(ipv4_addr, "%d.%d.%d.%d", craft_addr.addr.addr[0], craft_addr.addr.addr[1], craft_addr.addr.addr[2], craft_addr.addr.addr[3]);
                CLI_LIB_PrintStr("\r\nAuto Provision: try to get config file name and tftp server IP address from DHCP server... \r\n");
                ip_addr_ready = TRUE;
            }
        }
        /* stop user's config on networking
         */
        if (ip_addr_ready == TRUE)
        {
            ...
            if (TRUE == SYS_PMGR_GetModelName(0, model_name))
            {
                ...
                memcpy(vendor_buf, model_name, id_len);
            }
            else
            {
                CLI_LIB_PrintStr("\r\nError: Can't get Model name for dynamic config file provision!\r\n");
            }
            sprintf(dhcpc_cmd, UDHCPC_CMD_TMP, model_name, ipv4_addr);
            rc=system(dhcpc_cmd);
            if (rc != 0)
            {
                CLI_LIB_PrintStr("\r\nError: can't run udhcpc on CRAFT port for dynamic config gile provision!\r\n");
                CLI_LIB_PrintStr_1("r\nsystem call failed:%s\r\n", dhcpc_cmd);
            }

            cli_mgr_is_done_udhcpc = TRUE; /* stop further udchpc try */
        }
    }
}
#endif /* #if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_UDHCPC_CRAFT_PORT == TRUE) */

#if (SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_DHCP == TRUE || SYS_CPNT_CLI_DYNAMIC_PROVISION_VIA_UDHCPC_CRAFT_PORT == TRUE)
/* ------------------------------------------------------------------------
...
{
    /* Accept first option 66 and 67 data only (even it is miss or invalid).
     */
    if (TRUE == cli_mgr_dhcp_rx_option_config_status)
    {
        return;
    }

    /* Length is used later to check invalid or not and value is used only when
     * length is valid, so that we must save the length at this time.
     */
    if (   (0 == option66_length)
        || (option66_length > SYS_ADPT_DHCP_MAX_DOMAIN_NAME_LEN)
        )
    {
        cli_mgr_dhcp_rx_option_config_address_len = 0;
        cli_mgr_dhcp_rx_option_config_address[0] = '\0';
    }
    else
    {
        cli_mgr_dhcp_rx_option_config_address_len = option66_length;

        strncpy(cli_mgr_dhcp_rx_option_config_address, (char *)option66_data_p, cli_mgr_dhcp_rx_option_config_address_len);
        cli_mgr_dhcp_rx_option_config_address[cli_mgr_dhcp_rx_option_config_address_len] = '\0';
    }

    if (   (0 == option67_length)
        || (option67_length > MAXSIZE_tftpSrcFile)
        )
    {
        cli_mgr_dhcp_rx_option_config_filename_len = 0;
        cli_mgr_dhcp_rx_option_config_filename_ar[0] = '\0';
    }
    else
    {
        cli_mgr_dhcp_rx_option_config_filename_len = option67_length;

        strncpy(cli_mgr_dhcp_rx_option_config_filename_ar, option67_data_p, cli_mgr_dhcp_rx_option_config_filename_len);
        cli_mgr_dhcp_rx_option_config_filename_ar[cli_mgr_dhcp_rx_option_config_filename_len] = '\0';
    }
 ...
 *            config file.
 * INPUT    : None.
 * OUTPUT   : tftp_address_p        -- TFTP server address/name.
 *            tftp_address_len_p    -- Length of TFTP server address/name.
 *            file_name_p           -- Config file name
 *            file_name_len_p       -- Length of config file name.
 * RETURN   : TRUE                  -- Remote config is ready.
 *            FALSE                 -- Remote config is not ready.
 * NOTE     : None.
 * ------------------------------------------------------------------------
 */
BOOL_T CLI_MGR_Get_DhcpRxOptionConfig(
    char *tftp_address_p, UI32_T *tftp_address_len_p,
    char *file_name_p, UI32_T *file_name_len_p)
{
    if (   (NULL == tftp_address_p)
        || (NULL == tftp_address_len_p)
        || (NULL == file_name_p)
        || (NULL == file_name_len_p)
        )
    {
        return FALSE;
    }
...
#if (SYS_CPNT_DNS == TRUE)
    {
        #define DNS_FILE "/var/run/udhcpc.dns_filename" /* must be the same as in /etc/udhcpc.sh */
        FILE            *dns_fd;
        I32_T           tmp_ret;
        L_INET_AddrIp_T dns_addr;
        BOOL_T          is_enable_dns = FALSE;
        ...
        if (dns_fd != NULL)
        {
            ...
            do
            {
                tmp_ret = fscanf(dns_fd, "%hhu.%hhu.%hhu.%hhu\n",
                            &dns_addr.addr[0], &dns_addr.addr[1],
                            &dns_addr.addr[2], &dns_addr.addr[3]);
            ...
            if (TRUE == is_enable_dns)
            {
                DNS_PMGR_EnableDomainLookup();
            }
        }
    }
#endif /* #if (SYS_CPNT_DNS == TRUE) */

    #define LEASE_FILE "/var/run/udhcpc.tftp_filename" /* must be the same as in /etc/udhcpc.sh */
    FILE *fd;
    char  c1, c2;
    fd=fopen(LEASE_FILE, "r");
    if (fd != NULL) /* got tftp server IP and config file stored */
    {
        fscanf(fd,"%c", &c1);
        if (c1=='!')  /* string length overflows (>= 128) */
        {
            char cmd[80];
            static BOOL_T print_once=TRUE;
            fclose(fd);
            if (print_once == TRUE)
            {
                CLI_LIB_PrintStr("\r\nError for ZTP from CRAFT port: The length of tftp server's IP address or config file name is too long!\r\n");
                print_once = FALSE;
            }
//            sprintf(cmd, "cat %s", LEASE_FILE);
//            system(cmd);  /* display which one(s) gets overflow */

            return FALSE;
        }
        else
        {
            fscanf(fd,"%s %s %c", tftp_address_p, file_name_p, &c2);
            fclose(fd);
            /* printf("\r\n--debug info: tftp_address_p=%s, file_name_p=%s.\r\n", tftp_address_p, file_name_p); */

            *tftp_address_len_p = strlen(tftp_address_p);
            *file_name_len_p = strlen(file_name_p);

            if (   (0 == *tftp_address_len_p)
                || (0 == *file_name_len_p)
                || (c2 != '.')
                )
            {
            ...
            {
                return TRUE;  /* CRAFT port gets a higher priority for ZTP */
            }
        }
    }
#endif
    ...
    {
        *tftp_address_len_p = cli_mgr_dhcp_rx_option_config_address_len;
        strncpy(tftp_address_p, cli_mgr_dhcp_rx_option_config_address, *tftp_address_len_p);
            tftp_address_p[*tftp_address_len_p] = '\0';

        *file_name_len_p = cli_mgr_dhcp_rx_option_config_filename_len;
        strncpy(file_name_p, cli_mgr_dhcp_rx_option_config_filename_ar, *file_name_len_p);
        file_name_p[*file_name_len_p] = '\0';
...
#if ( SYS_CPNT_UNIT_HOT_SWAP == TRUE )
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_ClearDeviceConfig
 * ------------------------------------------------------------------------
 * PURPOSE  : Clear the buffer which stores config commands for the
 *            specified device id.
 * INPUT    : device_id - 1 ~ SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK -> unit id(main board)
 *                        (SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK+1) ~ (SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK+CLI_MGR_NUMBER_OF_DEVICE_PER_UNIT)
 *                        -> module id(expansion module with CPU)
 * OUTPUT   : None
 * RETURN   : TRUE  --  Success
 *            FALSE --  Failed
 * NOTE     : None
...
static BOOL_T CLI_MGR_ClearDeviceConfig(UI32_T device_id)
{
    if ( (device_id == 0) || (device_id > CLI_MGR_NUMBER_OF_TOTAL_DEVICE))
    {
        printf("%s(%d): invalid device_id=%lu\r\n", __FUNCTION__, __LINE__, (unsigned long)device_id);
        return FALSE;
    }

    memset( device_cfg[device_id - 1], 0, CLI_MGR_RUNTIME_PROVISION_UC_MEM_SIZE);
    device_cfg_cursor[device_id-1]=0;
...
#else /* #if ( SYS_CPNT_UNIT_HOT_SWAP == TRUE ) */
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_ClearModuleConfig
 * ------------------------------------------------------------------------
 * PURPOSE  : Clear the buffer which stores config commands for the
 *            specified module id.
 * INPUT    : module_id - 1 ~ SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK -> module id(expansion module with CPU)
 * OUTPUT   : None
 * RETURN   : TRUE  --  Success
 *            FALSE --  Failed
 * NOTE     : None
...
static BOOL_T CLI_MGR_ClearModuleConfig(UI32_T module_id)
{
    if ( (module_id == 0) || (module_id > SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK))
    {
        printf("%s(%d): invalid module_id=%lu\r\n", __FUNCTION__, __LINE__, module_id);
        return FALSE;
    }

    memset( module_cfg[module_id-1], 0, CLI_MGR_RUNTIME_PROVISION_UC_MEM_SIZE);
    module_cfg_cursor[module_id-1]=0;
...
#if (SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG == TRUE)
/* ------------------------------------------------------------------------
 * FUNCTION NAME - CLI_MGR_PartialProvision
 * ------------------------------------------------------------------------
...
void CLI_MGR_PartialProvision()
{
    ...
    if (NULL != provision_p)
    {
        if (TRUE != CLI_TASK_CreateTask(CLI_TYPE_PARTIAL_PROV, 0, 0, provision_p, strlen(provision_p)))
        {
            XFER_PMGR_SetPartialProvisionStatus(XFER_MGR_FILE_COPY_ERROR);
            CLI_MGR_Set_BuffermgmtOffset(0);
            BUFFER_MGR_Free(provision_p);
            provision_p = NULL;
        }
...
static void CLI_MGR_ADDRUNCONFIG_DeleteExpiredEntry()
{
    ...
    {
        if (entry.status == CLI_OM_EXP_ADDRUNCONFIG_ENTRY_DONE)
        {
            UI32_T duration = (SYSFUN_GetSysTick() - entry.finish_time_by_ticks);

            if (CLI_OM_EXP_ADDRUNCONFIG_EXPIRE_TIME_BY_TICKS < duration)
            {
                CLI_OM_EXP_ADDRUNCONFIG_DeleteEntryById(entry.id);
            }
        }
...
static void CLI_MGR_ADDRUNCONFIG_Notify()
{
    ...
    CLI_OM_EXP_ADDRUNCONFIG_GetTaskID(&tid);
    SYSFUN_SendEvent(tid, CLI_OM_EXP_ADDRUNCONFIG_EVENT_EXECUTE_PARTIAL_PROVISION);
}
#endif  /* SYS_CPNT_CLI_ADD_TO_RUNNING_CONFIG */
