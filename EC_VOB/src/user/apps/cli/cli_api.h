...
#else
/* Temporarily workaround for the compiler warning shown below:
 *   argument to 'sizeof' in 'memset' call is the same expression as the destination; did you mean to provide an explicit length?
 *
 * This warning occurs when the variable "buff" is the name of a function argument
 */
#define PROCESS_MORE_FUNC(STR){                          \
    line_num = CLI_LIB_PrintStrToScreen(STR, line_num);  \
                                                         \
    ...
    {                                                    \
        return EXIT_SESSION_MORE;                        \
                                                         ...
                                                         \
    SYSFUN_GCC_DIAG_OFF(sizeof-pointer-memaccess)        \
    memset(buff, 0, sizeof(buff)); }                     \
    SYSFUN_GCC_DIAG_ON(sizeof-pointer-memaccess)
#endif

#define PROCESS_3COM_MORE(STR){                 \
   CLI_LIB_PrintStr(STR);                  \
   if(++line_num  >= CLI_LIB_GetMaxTerminalLineNum()-1)\
   {  switch(CLI_STRLB_Proc3ComMoreFeature())    \
      {  case 1:                           \
            line_num = 0;                  \
            break;                         \
         case 2:                           \
...
#define PROCESS_MORE_F(fmt_str, args...) \
    do{ \
        sprintf(buff, fmt_str, ##args); \
        PROCESS_MORE(buff); \
    } while (0)
...
#define PROCESS_MORE_1(fmt, arg1)  {    \
   char message[CLI_DEF_MAX_BUFSIZE];   \
...
#define EXIT_SESSION_MORE    0xfffffffe
