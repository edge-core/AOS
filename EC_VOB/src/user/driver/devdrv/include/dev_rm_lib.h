...
BOOL_T
DEVRM_LIB_RulePatternQualifyIpv6TrafficClass(
    DEVRM_AceEntry_T *rule_pattern_p,
    UI8_T data,
    UI8_T mask);
...
BOOL_T
DEVRM_LIB_RulePatternQualifyIpv6FlowLabel(
    DEVRM_AceEntry_T *rule_pattern_p,
    UI32_T data,
    UI32_T mask);
...
static BOOL_T
DEVRM_LIB_ActionRedirectRoute(
    DEVRM_ACTION_T *action_p,
    UI32_T index)
{
    ASSERT(NULL != action_p);

    action_p->redirect_l3_egress.is_redirect = TRUE;
    action_p->redirect_l3_egress.index = index;
    ...
    UI32_T ebs_bytes)
{
    ...
    action_p->policer.meter.bytes_burst_size = cbs_bytes;
    action_p->policer.meter.peak_bytes_burst_size = ebs_bytes;
    ...
    UI32_T pbs_bytes)
{
    ...
    action_p->policer.meter.peak_bytes_burst_size = pbs_bytes;
...
DEVRM_LIB_ActionRedirectDump(
    ...
    const char *prefix)
{
    if (TRUE == redirect_p->is_redirect)
    {
        printf_fn("%*s" "+-o %s\r\n", indent, "", prefix);
...
#undef METER_MODE
        ...
        DEVRM_POLICER_METER_MODE_TRTCM_COLOR_AWARE == meter_p->mode)
    {
        ...
        printf_fn("%*s"
                  "peak_bytes_burst_size = %lu\r\n",
                  indent, "",
                  meter_p->peak_bytes_burst_size);
    }
...
#undef PKT_CMD
    ...
    DEVRM_LIB_ActionRedirectDump(&action_p->redirect_l3_egress, printf_fn, indent, "redirect");
