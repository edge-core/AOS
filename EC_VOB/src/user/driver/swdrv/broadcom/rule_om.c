...
static UI32_T RULE_OM_LocalCreateWorkingAcl(RULE_TYPE_AclType_T acl_type, UI32_T *acl_index_p);
#endif
...
static void
RULE_OM_LocalCountTimeRangeRefCounter(
    UI16_T time_range_index,
    RULE_TYPE_TIME_RANGE_REF_COUNT_T count_type
);
...
#define MAX_UI32_STR_LEN        10  /* 0xffffffff */
    ...
    sprintf(buf, "%lu", (unsigned long)policy_map_index);
...
#define MAX_UI32_STR_LEN        10  /* 0xffffffff */
    ...
    /* default name == index */
    sprintf(buf, "%lu", (unsigned long)class_map_index);
...
#if (SYS_CPNT_ACL_AUTO_COMPRESS_ACE == TRUE)
    /* reserve aggregate ACL first */
    acl_p->flag_aggregate_done = FALSE;
    ret = RULE_OM_LocalCreateWorkingAcl(acl_p->acl_type, &acl_p->aggregate_acl_index);
    if (RULE_TYPE_OK != ret)
    {
        RULE_OM_LeaveCriticalSectionReturnState(ret);
    }
...
#define MAX_UI32_STR_LEN        10  /* 0xffffffff */
    ...
    /* default name == index */
    sprintf(buf, "%lu", (unsigned long)acl_index);
...
UI32_T RULE_OM_BindElement2Acl(UI32_T acl_index, RULE_TYPE_AclType_T ace_type, UI32_T ace_index)
{
    ...
    if (RULE_TYPE_OK == RULE_OM_LocalGetAclIndexByAce(ace_index, &found_index, NULL))
    {
        ...
        {
            LOG("ace <%ld> is owned by another acl <%ld>.", (unsigned long)ace_index, (unsigned long)found_index);
    ...
    UI32_T ace_index)
{
    ...
    {
        LOG("ace <%ld> is not owned by acl <%ld>.", (unsigned long)ace_index, (unsigned long)acl_index);
 ...
 * OUTPUT:   acl_index, time_range_index
 * RETURN:   Error code.
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
    *time_range_index = bind_acl_p->time_range_index;
#endif /* SYS_CPNT_TIME_BASED_ACL */
 ...
 * PURPOSE:  Get next binding ACL by port
 * INPUT:    uport_ifindex, acl_index, precedence
 * OUTPUT:   acl_index, precedence, acl_entry, time_range_index.
 * RETURN:   Error code.
 * NOTE:     none
 *------------------------------------------------------------------------------
 */
 ...
 *------------------------------------------------------------------------------
 */
UI32_T RULE_OM_GetNextUIAclByPortAndBindingType(UI32_T precedence, UI32_T *ifindex_p, RULE_TYPE_UI_AclEntry_T *acl_entry_p)
{
    RULE_TYPE_RETURN_TYPE_T result;
    ...
    RULE_TYPE_AclType_T ace_type)
{
    ...
    switch (ace_type)
    {
        ...
        case RULE_TYPE_IP_STD_ACL:
        {
            ...
            ip_ace_p->aceControlCode  = RULE_TYPE_UNDEF_IP_CONTROL_CODE;
 ...
 *           acl_index      - Source ACL index
 *           mirror_type    - VAL_aclMirrorType_rx/VAL_aclMirrorType_tx/VAL_aclMirrorType_both
 * OUTPUT:   NONE
 * RETURN:   Error code.
 * NOTE:     NONE
 *------------------------------------------------------------------------------
    ...
    UI32_T mirror_type)
{
    ...
    if (mirror_type == VAL_aclMirrorType_rx ||
        mirror_type == VAL_aclMirrorType_both)
    {
    ...
    if (mirror_type == VAL_aclMirrorType_tx ||
        mirror_type == VAL_aclMirrorType_both)
    {
        acl_p->mirror_egress.enable = RULE_TYPE_MIRROR_ENABLE;
        acl_p->mirror_egress.dest_ifindex = ifindex_dest;
    }
    ...
    if (NULL != agg_acl_p)
    {
        ...
        ASSERT(sizeof(agg_acl_p->mirror) == sizeof(acl_p->mirror));
        memcpy(&agg_acl_p->mirror, &acl_p->mirror, sizeof(agg_acl_p->mirror));
#if (SYS_CPNT_ACL_MIRROR_EGRESS == TRUE)
        memcpy(&agg_acl_p->mirror_egress, &acl_p->mirror_egress, sizeof(agg_acl_p->mirror_egress));
#endif
    }
#endif /* SYS_CPNT_ACL_AUTO_COMPRESS_ACE */
    ...
    UI32_T acl_index)
{
    ...
    if (acl_p->mirror.dest_ifindex == ifindex_dest)
    {
        acl_p->mirror.enable = RULE_TYPE_MIRROR_DISABLE;
        acl_p->mirror.dest_ifindex = 0;
    }
#if (SYS_CPNT_ACL_MIRROR_EGRESS == TRUE)
    if (acl_p->mirror_egress.dest_ifindex == ifindex_dest)
    {
        acl_p->mirror_egress.enable = RULE_TYPE_MIRROR_DISABLE;
        acl_p->mirror_egress.dest_ifindex = 0;
    }
#endif /*#if (SYS_CPNT_ACL_MIRROR_EGRESS == TRUE)*/
    ...
    RULE_TYPE_UI_AclEntry_T *acl_entry)
{
    ...
    *mirror_type = 0;
                                                                ...
                                                                &tmp_acl_entry))
    {
        ...
        {
            *mirror_type = VAL_aclMirrorType_rx;
        }
#if (SYS_CPNT_ACL_MIRROR_EGRESS == TRUE)
        if (tmp_acl_entry.mirror_egress.enable == RULE_TYPE_MIRROR_ENABLE &&
            ifindex_dest == tmp_acl_entry.mirror_egress.dest_ifindex)
        {
            if (*mirror_type == VAL_aclMirrorType_rx)
            {
                *mirror_type = VAL_aclMirrorType_both;
            ...
            {
                *mirror_type = VAL_aclMirrorType_tx;
            }
        }
#endif
        if (*mirror_type != 0)
    ...
    BOOL_T *b_acl_drop)
{
    ...
    *b_acl_drop = FALSE;
         ...
         RULE_TYPE_OK == RULE_OM_LocalGetNextAceByAcl(acl_p, &ace_index, &ace_entry_p); )
    {
        ...
        if ( RULE_TYPE_ARP_REPLY == arp_pkt_p->opcode ) /* only for arp reply */
        {
            /* 4. check Dst IP
             */
    ...
    UI32_T *acl_index_p)
{
    ...
    do
    {
        ...
        sprintf(acl_name, "~w%05lu", (unsigned long)working_acl_index++);
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
    if (src_ace_p->time_range_index != agg_ace_p->time_range_index)
    {
        return RULE_TYPE_FAIL;
    }
#endif /* SYS_CPNT_TIME_BASED_ACL */
    ...
    RULE_TYPE_AclType_T acl_type)
{
    ...
    LOG("user config acl index in precedence[%lu] = %lu", (unsigned long)precedence, (unsigned long)acl_index);
    RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_OK);
}
    ...
    const RULE_TYPE_UI_BundleGroup_T *ui_bundle_group_p)
{
    ...
    {
        LOG("Invalid bundle group index %lu", (unsigned long)ui_bundle_group_p->index);
        return RULE_TYPE_INVALID_PARAMETER;
    }
    ...
    {
        LOG("Invalid class-map count %lu", (unsigned long)ui_bundle_group_p->class_map_count);
        return RULE_TYPE_INVALID_PARAMETER;
    }
    ...
    for (i = 0; i < ui_bundle_group_p->class_map_count; ++i)
    {
         ...
         */
        {
            ...
            if (NULL == policy_map_p)
            {
                LOG("Invalid policy-map index %lu", (unsigned long)policy_map_index);
                return RULE_TYPE_INVALID_PARAMETER;
            }
            ...
            for (bi = 0; bi < _countof(policy_map_p->bundle_group); ++bi)
            {
                ...
                for (ci = 0; ci < test_bundle_group_p->class_map_count; ++ci)
                {
                    ...
                    {
                        LOG("Class-map %s exist on bundle-group %lu already", class_map_name, (unsigned long)bi+1);
                        return RULE_TYPE_INVALID_PARAMETER;
                    }
                }
            }
    ...
    UI32_T *burst_size_p)
{
    ...
    {
        LOG("Invalid meter index(%lu)", (unsigned long)policy_element_p->meter_index);
        return RULE_TYPE_INVALID_PARAMETER;
    }
    ...
    if (NULL == action_p)
    {
        LOG("Invalid action index(%lu)", (unsigned long)policy_element_p->action_index);
        return RULE_TYPE_INVALID_PARAMETER;
    }

    if (RULE_TYPE_METER_MODE_FLOW != meter_entry_p->meter_model)
    {
        LOG("Invalid meter mode(%lu)", (unsigned long)meter_entry_p->meter_model);
        return RULE_TYPE_E_INVALID_METER_MODE;
    }

    if (0 == (RULE_TYPE_ACTION_RED_DROP & action_p->action_bitmap))
    {
        LOG("Invalid action(%lu)", (unsigned long)action_p->action_bitmap);
    ...
    UI32_T burst_size)
{
    ...
    if (NULL == child_bundle)
    {
        ...
        {
            LOG("Not available entry for new child bundle for priority %lu", (unsigned long)priority);
            return RULE_TYPE_FAIL;
        }
    ...
    if (NULL == class_p)
    {
        ...
        {
            LOG("Not available entry for new class-map %lu", (unsigned long)class_map_index);
            return RULE_TYPE_FAIL;
        }
    ...
    RULE_TYPE_BundleGroup_T *bundle_group_p)
{
    ...
    for (i = 0; i < ui_bundle_group_p->class_map_count; ++i)
    {
        ...
        if (RULE_TYPE_OK != ret)
        {
            LOG("Class-map index %lu is not exist", (unsigned long)class_map_index);
            return ret;
        }
        ...
        if (RULE_TYPE_OK != ret)
        {
            LOG("Failed to get the bandwidth of policy-map index %lu, class-map index %lu",
                (unsigned long)policy_map_index, (unsigned long)class_map_index);
            return ret;
        }
...
/*------------------------------------------------------------------------------
 * FUNCTION NAME - RULE_OM_CreateTimeRangeEntry
 *------------------------------------------------------------------------------
 * PURPOSE:  Create a time range.
 * INPUT:    time_range_name  - time range name
 * OUTPUT:   time_range_index - time range index
 * RETURN:   Error code.
 * NOTE:     NONE
 *------------------------------------------------------------------------------
 */
RULE_TYPE_RETURN_TYPE_T RULE_OM_CreateTimeRangeEntry (const char *time_range_name, UI16_T *time_range_index)
{
    UI32_T index, unused_pos = RULE_TYPE_UNDEF_TIME_RANGE ;

    RULE_OM_EnterCriticalSection();

    /* check if exist and find out new space for new entry */
    for (index =0 ; index < SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE ; index++)
    {
        if (TRUE == shmem_data_p->time_range_table[index].valid)
        {
            if (0 == strcmp(shmem_data_p->time_range_table[index].name, time_range_name))
            {
                LOG("acl_p is null or RULE_OM_LocalClassifyAcl failed. \r\n");
                RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
            }
        }
        else if (RULE_TYPE_UNDEF_TIME_RANGE == unused_pos)
        {
            unused_pos = index;
        }
    }

    if (RULE_TYPE_UNDEF_TIME_RANGE == unused_pos) /* no free space */
    {
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
    }
    else
    {
        /* install new entry
         */
        memset(&shmem_data_p->time_range_table[unused_pos], 0, sizeof(RULE_TYPE_TIME_ACL_TIME_RANGE_T));
        /*valid*/
        shmem_data_p->time_range_table[unused_pos].valid = TRUE;
        /*name*/
        strncpy(shmem_data_p->time_range_table[unused_pos].name, time_range_name,
                                         SYS_ADPT_MAX_LENGTH_OF_TIME_ACL_TABLE_NAME);

        /*in active, trigger to be active by timer */
        shmem_data_p->time_range_table[unused_pos].active = FALSE;

        /*reference counter */
        shmem_data_p->time_range_table[unused_pos].ref_counter = 0;

        /* output, return */
        *time_range_index = unused_pos;
...
/*------------------------------------------------------------------------------
 * FUNCTION NAME - RULE_OM_DeleteTimeRangeEntry
 *------------------------------------------------------------------------------
 * PURPOSE:  Delete a time range.
 * INPUT:    time_range_index
 * OUTPUT:   NONE
 * RETURN:   Error code.
 * NOTE:     NONE
 *------------------------------------------------------------------------------
 */
RULE_TYPE_RETURN_TYPE_T RULE_OM_DeleteTimeRangeEntry (UI16_T time_range_index)
{
    /* check if out of range */
    if (time_range_index >= SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE)
    {
        LOG("out of range \r\n");
        return RULE_TYPE_FAIL /*RULE_TYPE_INVALID_PARAMETER*/;
    }

    RULE_OM_EnterCriticalSection();
    /* check if reference counter */
    if (shmem_data_p->time_range_table[time_range_index].ref_counter > 0)
    {
        LOG("ref_couter not zero \r\n");
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
    }

    /* clear */
    shmem_data_p->time_range_table[time_range_index].valid = FALSE;
    RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_OK);
}

/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_SetTimeRangeAbsolute
 *------------------------------------------------------------------------------
 * PURPOSE:  Set a time range absolute time and date.
 * INPUT:    time_range_index           - time range index
 *           p_start_absolute_time_date - start absolute time and date
 *           p_end_absolute_time_date   - end absolute time and date
 * OUTPUT:   NONE.
 * RETURN:   Error code.
 * NOTE:     NONE
 *------------------------------------------------------------------------------
 */
RULE_TYPE_RETURN_TYPE_T RULE_OM_SetTimeRangeAbsolute(UI16_T time_range_index,
                            RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T *p_start_absolute_time_date,
                            RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T *p_end_absolute_time_date)
{
    if ( (time_range_index >= SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE) ||
         ((NULL == p_start_absolute_time_date) && (NULL == p_end_absolute_time_date)) )
    {
        return RULE_TYPE_FAIL /*RULE_TYPE_INVALID_PARAMETER*/;
    }

    RULE_OM_EnterCriticalSection();
    if (FALSE == shmem_data_p->time_range_table[time_range_index].valid)
    {
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
    }

    if ( TRUE == shmem_data_p->time_range_table[time_range_index].check_start_absolute ||
         TRUE == shmem_data_p->time_range_table[time_range_index].check_end_absolute )
    {
        LOG("absolute exist \r\n");
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
    }

    if (NULL != p_start_absolute_time_date)
    {
        memcpy(&shmem_data_p->time_range_table[time_range_index].start_time, p_start_absolute_time_date, sizeof(RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T));
        shmem_data_p->time_range_table[time_range_index].check_start_absolute = TRUE;
    }

    if (NULL != p_end_absolute_time_date)
    {
        memcpy(&shmem_data_p->time_range_table[time_range_index].end_time, p_end_absolute_time_date, sizeof(RULE_TYPE_TIME_ACL_ABSOLUTE_TIME_DATE_T));
        shmem_data_p->time_range_table[time_range_index].check_end_absolute = TRUE;
    }
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_DestoryTimeRangeAbsolute
 *------------------------------------------------------------------------------
 * PURPOSE:  Destory an time range absolute time and date.
 * INPUT:    time_range_index
 * OUTPUT:   NONE.
 * RETURN:   Error code.
 * NOTE:     NONE
 *------------------------------------------------------------------------------
 */
RULE_TYPE_RETURN_TYPE_T RULE_OM_DestoryTimeRangeAbsolute(UI16_T time_range_index)
{
    if (time_range_index >= SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE)
    {
        return RULE_TYPE_FAIL /*RULE_TYPE_INVALID_PARAMETER*/;
    ...
    RULE_OM_EnterCriticalSection();
    shmem_data_p->time_range_table[time_range_index].check_start_absolute = FALSE;
    shmem_data_p->time_range_table[time_range_index].check_end_absolute   = FALSE;
    RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_OK);
}

/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_SetTimeRangePeriodic
 *------------------------------------------------------------------------------
 * PURPOSE:  Set a time range periodic time.
 * INPUT:    time_range_index  - time range index
 *           p_periodic_time   - periodic time
 * OUTPUT:   NONE.
 * RETURN:   Error code.
 * NOTE:     NONE
 *------------------------------------------------------------------------------
 */
RULE_TYPE_RETURN_TYPE_T RULE_OM_SetTimeRangePeriodic(UI16_T time_range_index, RULE_TYPE_TIME_ACL_PERIODIC_TIME_T *p_periodic_time)
{
    UI32_T index, unused_pos = SYS_ADPT_MAX_NBRS_OF_TIME_ACL_PERIODIC ;
    ...
    RULE_OM_EnterCriticalSection();
    /* check if exist and find out new space for new entry */
    for (index =0 ; index < SYS_ADPT_MAX_NBRS_OF_TIME_ACL_PERIODIC ; index++)
    {
        if (TRUE == shmem_data_p->time_range_table[time_range_index].check_periodic[index])
        {
            if (0 == memcmp(&shmem_data_p->time_range_table[time_range_index].periodic_daily_time[index], p_periodic_time, sizeof(RULE_TYPE_TIME_ACL_PERIODIC_TIME_T)))
            {
                LOG("acl_p is null or RULE_OM_LocalClassifyAcl failed.");
                RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
            }
        }
        else if (SYS_ADPT_MAX_NBRS_OF_TIME_ACL_PERIODIC == unused_pos)
        {
            unused_pos = index;
        }
    }

    if (SYS_ADPT_MAX_NBRS_OF_TIME_ACL_PERIODIC == unused_pos) /* no free space */
    {
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
    }
    else
    {
        ...
        memcpy(&shmem_data_p->time_range_table[time_range_index].periodic_daily_time[unused_pos], p_periodic_time, sizeof(RULE_TYPE_TIME_ACL_PERIODIC_TIME_T));

        /*in active, trigger to be active by timer */
        shmem_data_p->time_range_table[time_range_index].check_periodic[unused_pos] = TRUE;
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_DestoryTimeRangePeriodic
 *------------------------------------------------------------------------------
 * PURPOSE:  Destory a time range periodic time.
 * INPUT:    time_range_index  - time range index
 *           p_periodic_time   - periodic time
 * OUTPUT:   NONE.
 * RETURN:   Error code.
 ...
 *------------------------------------------------------------------------------
 */
RULE_TYPE_RETURN_TYPE_T RULE_OM_DestoryTimeRangePeriodic(UI16_T time_range_index, RULE_TYPE_TIME_ACL_PERIODIC_TIME_T *p_periodic_time)
{
    UI32_T index;
    ...
    if (SYS_ADPT_MAX_NBRS_OF_TIME_ACL_PERIODIC == index) /* no find */
    {
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
    }
    else
    {
        /* clear */
        shmem_data_p->time_range_table[time_range_index].check_periodic[index] = FALSE;
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_GetNextTimeRange
 *------------------------------------------------------------------------------
 * PURPOSE:  Get next time range entry by time range index
 * INPUT:    time_range_index - time range index
 *           time_range_entry - time range entry
 * OUTPUT:   time_range_index - time range index
 *           time_range_entry - time range entry
 * RETURN:   Error code.
 * NOTE:     Let *time_range_index = RULE_TYPE_UNDEF_TIME_RANGE to get first enty
 *------------------------------------------------------------------------------
 */
RULE_TYPE_RETURN_TYPE_T RULE_OM_GetNextTimeRange(UI16_T *time_range_index, RULE_TYPE_TIME_ACL_TIME_RANGE_T *time_range_entry)
{
    UI16_T index;

    /* the decision of time range index*/
    ...
    {
        /* get next */
        index = *time_range_index + 1;
    }

    RULE_OM_EnterCriticalSection();
    for ( ; index<SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE; index++)
    {
        if (TRUE == shmem_data_p->time_range_table[index].valid)
        {
            break;
    ...
    if (index >= SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE)
    {
        RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_FAIL);
    }

    memcpy(time_range_entry, &shmem_data_p->time_range_table[index], sizeof(RULE_TYPE_TIME_ACL_TIME_RANGE_T));
    *time_range_index = index;
    RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_OK);
}

/*------------------------------------------------------------------------------
 * FUNCTION NAME - RULE_OM_CheckTimeRangeEntryIsActiveByIndex
 *------------------------------------------------------------------------------
 * PURPOSE : Get the field "active" of Time Range by index
 *   INPUT : time_range_index
 *  OUTPUT : p_is_active
 *  RETURN : TRUE / FALSE
 *    NOTE : "active" mean time range entry is in active in current time
 *------------------------------------------------------------------------------
 */
BOOL_T RULE_OM_CheckTimeRangeEntryIsActiveByIndex (UI16_T time_range_index, BOOL_T *p_is_active)
{
    /* check if out of range */
    if (time_range_index >= SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE)
    {
    ...
    {
        RULE_OM_EnterCriticalSection();
        *p_is_active = shmem_data_p->time_range_table[time_range_index].active;
        RULE_OM_LeaveCriticalSection();
    }
    return TRUE;
}
...
/*------------------------------------------------------------------------------
 * FUNCTION NAME - RULE_OM_UpdateTimeRangeTableByTimer
 *------------------------------------------------------------------------------
 * PURPOSE : Update the field "active" of Time Range Table by timer
 *   INPUT : p_current_time - current time
 *  OUTPUT : p_state_list   - the state list "active" of time range table
 *  RETURN : TRUE / FALSE
 *    NOTE : "active" mean time range entry is in active in current time
 *------------------------------------------------------------------------------
 */
BOOL_T RULE_OM_UpdateTimeRangeTableByTimer (RULE_TYPE_TIME_ACL_CONVERT_TIME_T *p_current_time , UI8_T *p_state_list)
{
    UI32_T time_range_index, periodic_index;
    UI32_T day_of_week;
    UI32_T time_hour_plus_min, start_time_hour_plus_min, end_time_hour_plus_min;
    UI8_T  start_type_of_wk, end_type_of_wk;
    BOOL_T in_active, compare_time_hour_plus_min, in_coming_check_periodic, in_coming_check_absolute;
    RULE_TYPE_TIME_ACL_TIME_RANGE_T    *time_range_entry;
    RULE_TYPE_TIME_ACL_PERIODIC_TIME_T *p_periodic_daily_time;

    RULE_OM_EnterCriticalSection();

    /* initial */
    memset(p_state_list, 0, SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE);
    time_hour_plus_min  = 60 * p_current_time->hour + p_current_time->minute;

    for (time_range_index=0, in_active=FALSE; time_range_index <= SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE; time_range_index++)
    {
        /***********************
         * update om and state list (record previous the result of comparsion)
         ***********************/
        if (time_range_index>0)
        {
            if (TRUE == in_active)
            {
                /* update and record this event */
                shmem_data_p->time_range_table[time_range_index-1].active = TRUE;
                p_state_list[time_range_index-1]            = RULE_TYPE_TIME_RANGE_IN_ACTIVE;
            }
            else
            {
                /* update and record this event */
                shmem_data_p->time_range_table[time_range_index-1].active = FALSE;
                p_state_list[time_range_index-1]            = RULE_TYPE_TIME_RANGE_UN_ACTIVE;
            }

            /* finish to compare all time range entry, and end for */
            if (SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE == time_range_index)
            {
                break;
            }
        }
        ...
        /****************************************************
         * check if time_range entry is in-active at current time
         * A. check absolute time
         * B. check periodic time
         ****************************************************/

        /* initial */
        in_active            =  FALSE;
        time_range_entry     = &shmem_data_p->time_range_table[time_range_index];
        day_of_week          =  p_current_time->day_of_week ;
        in_coming_check_absolute    =  FALSE;

        /* check the time range entry if be valid */
        if (FALSE == time_range_entry->valid )
        {
            continue; /* no pass, check next time range etnry */
        }

        /* A.1. check start absolute time
         */
        if (TRUE == time_range_entry->check_start_absolute)
        {
            in_coming_check_absolute = TRUE;

            if (p_current_time->time_by_sec < time_range_entry->start_time.time_by_sec)
            {
                continue; /* no pass, check next time range etnry */
            }
        }

        /* A.2. check end absolute time
         */
        if (TRUE == time_range_entry->check_end_absolute)
        {
            in_coming_check_absolute = TRUE;

            if (p_current_time->time_by_sec > time_range_entry->end_time.time_by_sec)
            {
                continue; /* no pass, check next time range etnry */
            }
        }

        /* initial for in_coming_check_periodic */
        in_coming_check_periodic = FALSE;

        /* B. check periodic time
         */
        for (periodic_index=0; periodic_index < SYS_ADPT_MAX_NBRS_OF_TIME_ACL_PERIODIC; periodic_index++)
        {
            /* initial for compare_time_hour_plus_min */
            compare_time_hour_plus_min = FALSE;

            /* any one periodic time is matched,
               then the time range entry is passed.*/
            if (TRUE == in_active)
            {
                break;
            }
            ...
            {
                in_coming_check_periodic =  TRUE;
                p_periodic_daily_time    = &time_range_entry->periodic_daily_time[periodic_index];
                start_type_of_wk         =  p_periodic_daily_time->start_type_of_wk;
                end_type_of_wk           =  p_periodic_daily_time->end_type_of_wk;
                start_time_hour_plus_min =  60 * p_periodic_daily_time->start_hour +
                                            p_periodic_daily_time->start_minute;
                end_time_hour_plus_min   =  60 * p_periodic_daily_time->end_hour +
                                            p_periodic_daily_time->end_minute;
                switch (start_type_of_wk)
                {
                    /* Daily : Monday through Sunday */
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_DAILY:
                        compare_time_hour_plus_min = TRUE;
                        break;
                    /* Weekdays : Monday through Friday */
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_WEEKDAYS:
                        if ( (day_of_week != RULE_TYPE_TIME_RANGE_TYPE_OF_WK_SUNDAY) &&
                             (day_of_week != RULE_TYPE_TIME_RANGE_TYPE_OF_WK_SATURDAY) )
                        {
                            compare_time_hour_plus_min = TRUE;
                        }
                        break;
                    /* WEEKEND : Saturday and Sunday */
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_WEEKEND:
                        if ( (day_of_week == RULE_TYPE_TIME_RANGE_TYPE_OF_WK_SUNDAY) ||
                             (day_of_week == RULE_TYPE_TIME_RANGE_TYPE_OF_WK_SATURDAY) )
                        {
                            compare_time_hour_plus_min = TRUE;
                    ...
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_SUNDAY:
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_MONDAY:
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_TUESDAY:
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_WEDNESDAY:
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_THURSDAY:
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_FRIDAY:
                    case RULE_TYPE_TIME_RANGE_TYPE_OF_WK_SATURDAY:

                        /*
                         Condition 1 : start and end are the same day
                         1.1 Normal Case - monday 5:00 ~ monday 7:00
                         1.2 A Whole Day - monday 5:00 ~ monday 5:00
                         1.3 Other       - monday 5:00 ~ monday 2:00 (mean: only monday 2:01 ~ 4:59 is invalid)
                         */
                        if (start_type_of_wk == end_type_of_wk)
                        {
                            /* 1.1 Normal Case */
                            if (end_time_hour_plus_min > start_time_hour_plus_min)
                            {
                            ...
                            /* 1.2 A Whole Day */
                            else if (end_time_hour_plus_min == start_time_hour_plus_min)
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    if ( (time_hour_plus_min >= start_time_hour_plus_min) ||
                                         (time_hour_plus_min <= end_time_hour_plus_min) )
                                    {
                                        in_active = TRUE;
                                    }
                                }
                        ...
                        else
                        {
                            ...
                            /* check if end is cross week comparsion with start,
                               yes -> plus 7 */
                            if (start_type_of_wk > end_type_of_wk)
                            {
                                end_type_of_wk = end_type_of_wk + 7;
                            }

                            /* check if day of week is cross week comparsion with start,
                               yes -> plus 7 */
                            if (day_of_week /* today */ < start_type_of_wk)
                            {
                            ...
                            {
                                if (time_hour_plus_min /* now */ >= start_time_hour_plus_min)
                                {
                                    in_active = TRUE;
                                }
                            }
                            ...
                            {
                                if (time_hour_plus_min <= end_time_hour_plus_min)
                                {
                                    in_active = TRUE;
                                }
                            }
                ...
                /* check hour and min */
                if (TRUE == compare_time_hour_plus_min)
                {
                    if ( (time_hour_plus_min /* now */ >= start_time_hour_plus_min) &&
                         (time_hour_plus_min /* now */ <= end_time_hour_plus_min) )
                    {
                        in_active = TRUE;
                    }
                }
            } /* end of if (TRUE == time_range_entry->check_periodic[periodic_index])*/
        }
        /* no check periodic - pass */
        if ((FALSE == in_coming_check_periodic) && (TRUE == in_coming_check_absolute))
        {
            in_active = TRUE;
        }
    } /* End of RULE_OM_UpdateTimeRangeTableByTimer*/
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_GetTimeRangeIdByName
 *------------------------------------------------------------------------------
 * PURPOSE:  Get index of time range by it's name.
 * INPUT:    time_range_name
 * OUTPUT:   time_range_index
 * RETURN:   Error code (OK / FAIL)
 * NOTE:     NONE
 *------------------------------------------------------------------------------
 */

UI32_T RULE_OM_GetTimeRangeIdByName(const char *time_range_name, UI16_T *time_range_index)
{
    UI32_T  index;

    if ((NULL == time_range_name) || (NULL == time_range_index))
    {
        LOG("NULL pointer ");
        return RULE_TYPE_FAIL;
    }
    ...
    RULE_OM_EnterCriticalSection();
    /* sequential search */
    for (index = 0; SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE > index; ++index)
    {
        if (FALSE == shmem_data_p->time_range_table[index].valid)
            continue;

        if (0 == strcmp(time_range_name, shmem_data_p->time_range_table[index].name))
        {
            *time_range_index = index;
            RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_OK);
        }
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_GetTimeRangeNameByID
 *------------------------------------------------------------------------------
 * PURPOSE:  Get name of time range by it's ID.
 * INPUT:    time_range_index
 * OUTPUT:   time_range_name
 * RETURN:   Error code (OK / FAIL)
 * NOTE:     NONE
 *------------------------------------------------------------------------------
 */
RULE_TYPE_RETURN_TYPE_T RULE_OM_GetTimeRangeNameByID(UI16_T time_range_index, char *time_range_name)
{
    /* check if out of range */
    if (time_range_index >= SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE)
    {
    ...
    RULE_OM_EnterCriticalSection();
    strncpy(time_range_name, shmem_data_p->time_range_table[time_range_index].name, SYS_ADPT_MAX_LENGTH_OF_TIME_ACL_TABLE_NAME);
    RULE_OM_LeaveCriticalSectionReturnState(RULE_TYPE_OK);
}


/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_CountTimeRangeRefCounter
 *------------------------------------------------------------------------------
 * PURPOSE:  Update reference counter by time range ID.
 * INPUT:    time_range_index, count_type
 * OUTPUT:   NONE
 * RETURN:   NONE
 * NOTE:     NONE
 *------------------------------------------------------------------------------
 */
void RULE_OM_CountTimeRangeRefCounter(UI16_T time_range_index, RULE_TYPE_TIME_RANGE_REF_COUNT_T count_type)
{

    RULE_OM_EnterCriticalSection();
    RULE_OM_LocalCountTimeRangeRefCounter(time_range_index, count_type);
    RULE_OM_LeaveCriticalSection();
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_LocalCountTimeRangeRefCounter
 *------------------------------------------------------------------------------
 * PURPOSE:  Update reference counter by time range ID.
 * INPUT:    time_range_index, count_type
 * OUTPUT:   NONE
 ...
 *------------------------------------------------------------------------------
 */
static void RULE_OM_LocalCountTimeRangeRefCounter(UI16_T time_range_index, RULE_TYPE_TIME_RANGE_REF_COUNT_T count_type)
{
    /* check if out of range */
    if (time_range_index >= SYS_ADPT_MAX_NBRS_OF_TIME_ACL_TABLE)
    {
    ...
    ASSERT(TRUE == shmem_data_p->time_range_table[time_range_index].valid);

    switch (count_type)
    {
        case RULE_TYPE_TIME_RANGE_REF_COUNT_PLUS:
            shmem_data_p->time_range_table[time_range_index].ref_counter++;
            break;

        case RULE_TYPE_TIME_RANGE_REF_COUNT_MINUS:
            shmem_data_p->time_range_table[time_range_index].ref_counter--;
            break;
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
    /* record the reference counter of time_range */
    RULE_OM_LocalCountTimeRangeRefCounter(shmem_data_p->ace_table[ace_index-1].time_range_index, RULE_TYPE_TIME_RANGE_REF_COUNT_PLUS);
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
    RULE_OM_LocalCountTimeRangeRefCounter(ace_entry_p->time_range_index, RULE_TYPE_TIME_RANGE_REF_COUNT_MINUS);
#endif /* SYS_CPNT_TIME_BASED_ACL */
    ...
    UI32_T *precedence_p)
{
    ...
    for (++(*precedence_p); *precedence_p <= RULE_TYPE_PRECEDENCE_LAST; ++ (*precedence_p))
    {
        ...
        if (RULE_TYPE_OK == result)
        {
            RULE_TYPE_InOutDirection_T direction = RULE_TYPE_UNSPEC_DIRECTION;
            RULE_TYPE_ACL_MEMENTO_PTR_T bind_acl_p;
    ...
    UI32_T *precedence_p)
{
    ...
    for (++(*precedence_p); *precedence_p <= RULE_TYPE_PRECEDENCE_LAST; ++ (*precedence_p))
    {
        ...
        if (RULE_TYPE_OK == result)
        {
            RULE_TYPE_AclType_T port_acl_type = RULE_TYPE_MAX_NBR_OF_ACL_TYPE;
            RULE_TYPE_ACL_MEMENTO_PTR_T bind_acl_p;
...
#define FIELD(val)  {#val, val}
    ...
    for (i=0; i<sizeof(fields)/sizeof(*fields); ++i)
    {
        ...
        if (RULE_TYPE_OK != res)
        {
            LOG("Field %s(%lu) failed", fields[i].strval, (unsigned long)fields[i].val);
            return res;
        }
    }
    ...
    {
        if((ip_ace_p->aceControlCode != dflt_ip_ace_p->aceControlCode) ||
            (ip_ace_p->aceControlCodeBitmask != dflt_ip_ace_p->aceControlCodeBitmask))
        {
...
#define SUPPORTED_ACTION    \
    ...
    if (1 < cnt)
    {
        LOG("in profile action(0x%08lx) conflicted", (unsigned long)action_bitmap & RULE_TYPE_IN_PROFILE_ACTION);
        return RULE_TYPE_FAIL;
    }
    ...
    if (1 < cnt)
    {
        LOG("green action(0x%08lx) conflicted", (unsigned long)action_bitmap & RULE_TYPE_GREEN_ACTION);
        return RULE_TYPE_FAIL;
    }
    ...
    {
        LOG("yellow action(0x%08lx) conflicted", (unsigned long)action_bitmap & RULE_TYPE_YELLOW_ACTION);
        return RULE_TYPE_FAIL;
    }
    ...
    {
        LOG("green action(0x%08lx) conflicted", (unsigned long)action_bitmap & RULE_TYPE_RED_ACTION);
        return RULE_TYPE_FAIL;
    }
    ...
    if (0 < action_bitmap)
    {
        ...
        LOG("action(0x%08lx) contains unsupported bit", (unsigned long)action_bitmap & SUPPORTED_ACTION);
        return RULE_TYPE_FAIL;
    }
...
static UI32_T RULE_OM_LocalValidateMeter(const RULE_TYPE_TBParamEntry_T *meter_entry)
{
    ...
    switch (meter_entry->meter_model)
    {
        ...
        case RULE_TYPE_METER_MODE_SRTCM_COLOR_BLIND:
            ...
            if ((SYS_ADPT_DIFFSERV_MIN_POLICE_BURST > meter_entry->burst_size) || (SYS_ADPT_DIFFSERV_MAX_POLICE_BURST < meter_entry->burst_size))
                return RULE_TYPE_FAIL; /* out of range */
            if ((SYS_ADPT_DIFFSERV_MIN_POLICE_BURST > meter_entry->peak_burst_size) || (SYS_ADPT_DIFFSERV_MAX_POLICE_BURST < meter_entry->peak_burst_size))
                return RULE_TYPE_FAIL; /* out of range */
            break;
    ...
    if ((SYS_ADPT_DIFFSERV_MIN_POLICE_RATE > meter_entry->rate) || (SYS_ADPT_DIFFSERV_MAX_POLICE_RATE < meter_entry->rate))
        return RULE_TYPE_FAIL; /* out of range */
            if ((SYS_ADPT_DIFFSERV_MIN_POLICE_BURST > meter_entry->burst_size) || (SYS_ADPT_DIFFSERV_MAX_POLICE_BURST < meter_entry->burst_size))
                return RULE_TYPE_FAIL; /* out of range */
            if ((SYS_ADPT_DIFFSERV_MIN_POLICE_RATE > meter_entry->peak_rate) || (SYS_ADPT_DIFFSERV_MAX_POLICE_RATE < meter_entry->peak_rate))
                return RULE_TYPE_FAIL; /* out of range */
            if ((SYS_ADPT_DIFFSERV_MIN_POLICE_BURST > meter_entry->peak_burst_size) || (SYS_ADPT_DIFFSERV_MAX_POLICE_BURST < meter_entry->peak_burst_size))
                return RULE_TYPE_FAIL; /* out of range */
            break;
...
static UI32_T RULE_OM_LocalValidateIpAceFieldForACL(const RULE_TYPE_IpAce_Entry_T *ace_entry, RULE_TYPE_AclType_T ace_type, UI32_T field_index)
{
    ...
    case LEAF_diffServIpAceIcmpType:
        if (ace_entry->aceIcmpType == dflt_ip_ace_entry->aceIcmpType)
        {
            break;
        }
        ...
        if (ace_entry->aceIcmpType > MAX_diffServIpAceIcmpType)
            return RULE_TYPE_FAIL;
        break;
    ...
    void *param1)
{
    switch (message)
    {
        ...
        default:
            LOG("Unknow message %lu", (unsigned long)message);
            break;
    }
...
#define IPV6_QUALIFY 0x04
    ...
    if (ui_ace_p->ace_type == RULE_TYPE_ARP_ACL)
    {
        ...
        if (FALSE == RULE_OM_IS_ZERO_MAC(ui_ace_p->arp.sender.mac.mask))
        {
        ...
        if (FALSE == RULE_OM_IS_ZERO_MAC(ui_ace_p->arp.target.mac.mask))
        {
        ...
        if (FALSE == RULE_OM_IS_ZERO_IPV4_ADDR(ui_ace_p->arp.sender.ip.mask))
        {
        ...
        if (FALSE == RULE_OM_IS_ZERO_IPV4_ADDR(ui_ace_p->arp.target.ip.mask))
        {
    ...
    if (VAL_diffServMacAceEtherTypeOp_equal == ui_ace_p->ether.ethertype.op)
    {
        ...
        ace_p->u.mac.aceMinEtherType = ui_ace_p->ether.ethertype.u.s.data;
        ...
        if (ui_ace_p->ether.ethertype.u.s.mask == MAX_diffServMacAceEtherTypeBitmask)
        {
            ...
            /* L3 init,
             */
            switch (ui_ace_p->ether.ethertype.u.s.data)
            {
                case RULE_TYPE_ETHERTYPE_IP:
    ...
    if (FALSE == RULE_OM_IS_ZERO_IPV4_ADDR(ui_ace_p->ipv4.sip.mask))
    {
        ...
        ace_p->u.ip.aceSourceIpAddr = L_STDLIB_Ntoh32(*ip_addr);
        ace_p->u.ip.aceSourceIpAddrBitmask = L_STDLIB_Ntoh32(*ip_mask);
    }
    ...
    if (FALSE == RULE_OM_IS_ZERO_IPV4_ADDR(ui_ace_p->ipv4.dip.mask))
    {
        ...
        ace_p->u.ip.aceDestIpAddr = L_STDLIB_Ntoh32(*ip_addr);
        ace_p->u.ip.aceDestIpAddrBitmask = L_STDLIB_Ntoh32(*ip_mask);
    }
    ...
    /* IPv4 + vid */
    if (VAL_diffServMacAceVidOp_equal == ui_ace_p->ipv4.vid.op)
    {
        ace_p->u.ip.aceVidOp = VAL_diffServMacAceVidOp_equal;
        ace_p->u.ip.aceMinVid = ui_ace_p->ipv4.vid.u.s.data;
        ace_p->u.ip.aceVidBitmask = ui_ace_p->ipv4.vid.u.s.mask;
    }
    ...
    {
        ace_p->u.ip.aceIcmpType = RULE_TYPE_UNDEF_ICMP_TYPE;
    }
    ...
    {
        ace_p->u.ip.aceSourcePortOp = ui_ace_p->l4_common.sport.op;
        ace_p->u.ip.aceDestPortOp = ui_ace_p->l4_common.dport.op;
    ...
    {
        ace_p->u.ipv6.aceSourcePortOp = ui_ace_p->l4_common.sport.op;
        ace_p->u.ipv6.aceDestPortOp = ui_ace_p->l4_common.dport.op;
        if (VAL_diffServIpAceSourcePortOp_equal == ui_ace_p->l4_common.sport.op)
        {
...
#define IPV6_QUALIFY 0x04
    ...
    switch (ace_p->ace_type)
    {
        ...
        case RULE_TYPE_ARP_ACL:
            ...
            if (FALSE == RULE_OM_IS_ZERO_MAC(ace_p->u.arp.aceSenderMacAddrBitmask))
            {
            ...
            if (FALSE == RULE_OM_IS_ZERO_MAC(ace_p->u.arp.aceTargetMacAddrBitmask))
            {
            ...
            if (0 != ace_p->u.arp.aceSenderIpAddr)
            {
            ...
            if (0 != ace_p->u.arp.aceTargetIpAddr)
            {
                UI8_T *ip_addr;
                UI8_T *ip_mask;

                UI32_T addr = L_STDLIB_Hton32(ace_p->u.arp.aceTargetIpAddr);
                UI32_T mask = L_STDLIB_Hton32(ace_p->u.arp.aceTargetIpAddrBitmask);

                ip_addr = (UI8_T *)&addr;
                ip_mask = (UI8_T *)&mask;

                memcpy(ui_ace_p->arp.target.ip.addr, ip_addr,
                       sizeof(ui_ace_p->arp.target.ip.addr));
                memcpy(ui_ace_p->arp.target.ip.mask, ip_mask,
    ...
    if (ace_qualify & IPV4_QUALIFY)
    {
        if (0 != ace_p->u.ip.aceSourceIpAddrBitmask)
        {
            ...
            UI32_T addr = L_STDLIB_Hton32(ace_p->u.ip.aceSourceIpAddr);
            UI32_T mask = L_STDLIB_Hton32(ace_p->u.ip.aceSourceIpAddrBitmask);

            ip_addr = (UI8_T *)&addr;
            ip_mask = (UI8_T *)&mask;

            memcpy(ui_ace_p->ipv4.sip.addr, ip_addr,
                   sizeof(ui_ace_p->ipv4.sip.addr));
            memcpy(ui_ace_p->ipv4.sip.mask, ip_mask,
        ...
        if (0 != ace_p->u.ip.aceDestIpAddrBitmask)
        {
            ...
            UI32_T addr = L_STDLIB_Hton32(ace_p->u.ip.aceDestIpAddr);
            UI32_T mask = L_STDLIB_Hton32(ace_p->u.ip.aceDestIpAddrBitmask);

            ip_addr = (UI8_T *)&addr;
            ip_mask = (UI8_T *)&mask;


            memcpy(ui_ace_p->ipv4.dip.addr, ip_addr,
                   sizeof(ui_ace_p->ipv4.dip.addr));
            memcpy(ui_ace_p->ipv4.dip.mask, ip_mask,
        ...
        if (RULE_TYPE_UNDEF_IP_CONTROL_CODE != ace_p->u.ip.aceControlCode)
        {
...
#if (SYS_CPNT_ACL_IP_PLUS_VID == TRUE)
        /* IPv4 + vid */
        if (VAL_diffServMacAceVidOp_equal == ace_p->u.ip.aceVidOp)
        {
            ui_ace_p->ipv4.vid.op = VAL_diffServMacAceVidOp_equal;
            ui_ace_p->ipv4.vid.u.s.data = ace_p->u.ip.aceMinVid;
            ui_ace_p->ipv4.vid.u.s.mask = ace_p->u.ip.aceVidBitmask;
        }
#endif /* #if (SYS_CPNT_ACL_IP_PLUS_VID == TRUE) */
    }
...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - RULE_OM_IsQosEnabledPort
 *------------------------------------------------------------------------------
    ...
    UI32_T ifindex)
{
    ...
    for (direction = RULE_TYPE_INBOUND; direction <= RULE_TYPE_OUTBOUND; ++direction)
    {
        int i;
        RULE_TYPE_PortEntry_T *port_entry_p;
        RULE_TYPE_AclType_T acl_type_check_list[] =
        {
            RULE_TYPE_MAC_ACL,
            RULE_TYPE_IP_ACL,
            RULE_TYPE_IPV6_ACL
        ...
        for (i = 0; i < _countof(acl_type_check_list); ++i)
        {
            RULE_TYPE_AclMemento_T *local_bind_acl_p;
            RULE_TYPE_AclType_T acl_type;

            acl_type = acl_type_check_list[i];
            local_bind_acl_p = &port_entry_p->bind_acl[acl_type];

            if (TRUE == RULE_OM_IS_ENTRY_ACTIVE(local_bind_acl_p->row_status))
            {
                RULE_OM_LeaveCriticalSectionReturnState(TRUE);
            }
        }
