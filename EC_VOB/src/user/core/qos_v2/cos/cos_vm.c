...
/*------------------------------------------------------------------------------
 * ROUTINE NAME - COS_VM_SetPortEgressHybridQueueMode
 *------------------------------------------------------------------------------
 * PURPOSE: This function sets the queue mode by port
 * INPUT:   l_port   - logic port
 *          priority - the priority of COS config, ex: COS_TYPE_PRIORITY_XX
 ...
 */
COS_TYPE_ERROR_CODE_E
COS_VM_SetPortEgressHybridQueueMode(
    UI32_T l_port,
    UI16_T priority,
    UI32_T mode,
...
{
#if (SYS_CPNT_WRR_Q_MODE_PER_PORT_CTRL == TRUE)
    COS_TYPE_ERROR_CODE_E        result = COS_TYPE_E_NONE;
    COS_VM_QUEUE_STRICT_STATUS_T status = COS_VM_QUEUE_STRICT_STATUS_DISABLED;
    UI32_T                       queue = 0;
    UI32_T                       first_strict_queue = 0, last_wrr_queue = 0;
    BOOL_T                       strict_queue_found = FALSE, wrr_queue_found = FALSE;
    UI32_T                       old_mode = COS_VM_SCHEDULING_WEIGHT_ROUND_ROBIN;
    UI32_T                       old_strict_queue_bitmap = 0;
    COS_VM_QUEUE_STRICT_STATUS_T old_strict_status = COS_VM_QUEUE_STRICT_STATUS_DISABLED;

    if (   mode != COS_VM_SCHEDULING_METHOD_SP_DRR 
        && mode != COS_VM_SCHEDULING_METHOD_SP_WRR)
    {
        return COS_TYPE_E_PARAMETER;
    }
    
    /* Backup for original configuration, will be used for recovery if the scheduling and queue mode
     * cannot be successfully changed.
     */
    if (COS_VM_GetPortEgressQueueMode(l_port, priority, &old_mode) != COS_TYPE_E_NONE)
    {
        return COS_TYPE_E_UNKNOWN;
    }
    old_strict_queue_bitmap = 0;
    for (queue = SYS_ADPT_MAX_NBR_OF_PRIORITY_QUEUE; queue > 0; queue --)
    {
        if (COS_VM_GetPortEgressQueueStrictStatus(l_port, priority, queue - 1, &old_strict_status) == COS_TYPE_E_NONE)
        {
            if (old_strict_status == COS_VM_QUEUE_STRICT_STATUS_ENABLED)
            {
                old_strict_queue_bitmap = old_strict_queue_bitmap | (1 << (queue - 1));
            }
        }
        else
        {
    ...
    /* Change queue mode to Strict-WRR or Strict-DRR.
     * Notes: return to WRR scheduling first, let all queues become WRR queue, 
     * ensure new strict queue configuration can succeed, avoid the impact
     * by old queue mode configuration.
     */
    if (   (result = COS_VM_Priv_SetEgressQueueMode(l_port, priority, COS_VM_SCHEDULING_WEIGHT_ROUND_ROBIN)) != COS_TYPE_E_NONE
        || (result = COS_VM_Priv_SetEgressQueueMode(l_port, priority, mode)) != COS_TYPE_E_NONE)
    {
        /* Recover the old settings if scheduling method change failed..
         */
        COS_VM_Priv_SetEgressQueueMode(l_port, priority, old_mode);
        return COS_TYPE_E_SCHEDULING_METHOD;
    }

    /* Specify which queue to be strict priority.
     */
    for (queue = SYS_ADPT_MAX_NBR_OF_PRIORITY_QUEUE; queue > 0; queue --)
    {
        status = (strict_queue_bitmap & (1 << (queue - 1))) ? 
            COS_VM_QUEUE_STRICT_STATUS_ENABLED : COS_VM_QUEUE_STRICT_STATUS_DISABLED;
 
        if ((result = COS_VM_SetPortEgressQueueStrictStatus(l_port, priority, queue - 1, status)) != COS_TYPE_E_NONE)
        {
            break;
        }
    }
    ...
    if (result != COS_TYPE_E_NONE)
    {
        /* Recover the old settings if strict queue config failed.
         */
        if (   old_mode == COS_VM_SCHEDULING_METHOD_SP_DRR
            || old_mode == COS_VM_SCHEDULING_METHOD_SP_WRR)
        {
            COS_VM_Priv_SetEgressQueueMode(l_port, priority, COS_VM_SCHEDULING_WEIGHT_ROUND_ROBIN);
            COS_VM_Priv_SetEgressQueueMode(l_port, priority, old_mode);
            for (queue = SYS_ADPT_MAX_NBR_OF_PRIORITY_QUEUE; queue > 0; queue --)
            {
                status = (old_strict_queue_bitmap & (1 << (queue - 1))) ? 
                    COS_VM_QUEUE_STRICT_STATUS_ENABLED : COS_VM_QUEUE_STRICT_STATUS_DISABLED;
                COS_VM_SetPortEgressQueueStrictStatus(l_port, priority, queue - 1, status);
            }
        }
        else
        {
    ...
    COS_VM_QUEUE_STRICT_STATUS_T status)
{
    ...
    UI16_T id;
    COS_TYPE_ERROR_CODE_E result;
    UI32_T queue_id = 0, strict_queue_bitmap = 0;
    UI32_T first_strict_queue = 0, last_wrr_queue = 0;
    BOOL_T strict_queue_found = FALSE, wrr_queue_found = FALSE;
    ...
    /* Check whether all strict queues priority are higher than WRR queues.
     */
    strict_queue_bitmap = 0;
    for (queue_id = 0; queue_id < SYS_ADPT_MAX_NBR_OF_PRIORITY_QUEUE; queue_id ++)
    {
        if (cos_vm_port_entry[ifindex - 1].queue_weight.config_p[id]->is_strict[queue_id] == TRUE)
        {
            strict_queue_bitmap = strict_queue_bitmap | (1 << queue_id);
        }
    }
    if (status == COS_VM_QUEUE_STRICT_STATUS_ENABLED)
    {
        strict_queue_bitmap = strict_queue_bitmap | (1 << queue);
    }
    else
    {
        strict_queue_bitmap = strict_queue_bitmap & !(1 << queue);
    ...
    for (queue_id = 0; queue_id < SYS_ADPT_MAX_NBR_OF_PRIORITY_QUEUE; queue_id ++)
    {
        if (strict_queue_bitmap & (1 << queue_id))
        {
            if (FALSE == strict_queue_found)
            {
                first_strict_queue = queue_id;
                strict_queue_found = TRUE;
            }
        }
        else
        {
            wrr_queue_found = TRUE;
            last_wrr_queue = queue_id;
        }
    }
    if (   TRUE == strict_queue_found
        && TRUE == wrr_queue_found
        && first_strict_queue < last_wrr_queue)
    {
        return COS_TYPE_E_PARAMETER_STRICT_QUEUE_PRIORITY;
    }
