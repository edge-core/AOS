...
#define _OPT_HELP_COMMAND                       0x00000020
#define _OPT_HELP_COMMAND_AND_ITS_PARAMETERS    0x00000040
#define _OPT_DFLT_VAL_AS_MISS_REQ_PARAMETER     0x00000080
#define _OPT_DFLT_VAL_AS_ALL_MISS_PARAMETER     0x00000100
...
static void L4_Backdoor_DiffServ_ShowTimeRangeTable(int argc, char** argv);
...
#if (SYS_CPNT_TIME_BASED_ACL == TRUE)
    {"show_time_range_table",           L4_Backdoor_DiffServ_ShowTimeRangeTable},
#endif /*#if (SYS_CPNT_TIME_BASED_ACL == TRUE)*/
...
static BOOL_T L4_Backdoor_Str2MacAddr(const char *str, UI8_T *addr)
{
    ...
    if (strlen(str) != MAC_ADDR_STR_LEN)
    {
        return FALSE;
    }
...
static void L4_Backdoor_CpuIf_TrapPacket2Cpu(int argc, char** argv)
{
   ...
   // UI8_T                       da[SYS_ADPT_MAC_ADDR_LEN];
   // UI32_T                      dip = 0;
    ...
    if ((2 > argc) || (4 < argc))
    {
        ...
        BACKDOOR_MGR_Printf("\r\n\t\t%2d (ospfv3)", RULE_TYPE_PacketType_OSPF6);       /* added by steven.gao for OSPFv3 */
#endif
        ...
        BACKDOOR_MGR_Printf("\r\n\t\t%2d (igmp, param1(to_cpu), param2(flood))", RULE_TYPE_PacketType_IGMP);
    ...
    switch (atoi(argv[0]))
    {
        case 0: enable_flag = FALSE; break;
        case 1: enable_flag = TRUE; break;
        default: BACKDOOR_MGR_Printf("\r\nbad enable_flag value"); return;
    }
    ...
    if (3 <= argc)
    {
        if (FALSE == L4_Backdoor_Str2MacAddr(argv[2], da))
            return;
    }
    ...
    if (4 <= argc)
    {
        if (FALSE == L4_Backdoor_Str2IpAddr(argv[3], (UI8_T*)&dip))
            return;
    }
#else
    if(RULE_TYPE_PacketType_UNKNOWN_IPMC == packet_type ||
       RULE_TYPE_PacketType_IGMP == packet_type)
    {
        if(4 != argc)
            return;
    ...
    )
{
    ...
    UI32_T unit;
    UI32_T port;
    UI8_T  mac[SYS_ADPT_MAC_ADDR_LEN] = {0};
    int res = _S_OK;
    ...
    if (options & _OPT_INSTALL || options & _OPT_UNINSTALL)
    {
        ...
        L4_Backdoor_Str2MacAddr(parameters[PARAM_DA].value, mac);

        if (FALSE == RULE_CTRL_1X_TrapDot1xUCToCPU(enable, unit, port, mac))
        {
            res = _E_FAILED;
        }
    }
    ...
    )
{
    ...
    if (options & _OPT_INSTALL || options & _OPT_UNINSTALL)
    {
        ...
        unit = atoi(parameters[PARAM_UNIT].value);
        port = atoi(parameters[PARAM_PORT].value);
        vid  = atoi(parameters[PARAM_VID].value);

        if (parameters[PARAM_SA].valid)
        {
            L4_Backdoor_Str2MacAddr(parameters[PARAM_SA].value, sa);
        }

        L4_Backdoor_Str2IpAddr(parameters[PARAM_SRC_IP].value, sip);

        if (FALSE == RULE_CTRL_IPSG_PermitIpPacket(enable, unit, port, vid, sip, (parameters[PARAM_SA].valid) ? sa : NULL))
        {
            res = _E_FAILED;
        }
    }
    ...
    {
        if (parameters[PARAM_UNIT].valid &&
            parameters[PARAM_PORT].valid &&
            parameters[PARAM_VID].valid &&
            parameters[PARAM_SRC_IP].valid)
        {
            UI32_T rule_id;
            ...
            if (FALSE == RULE_CTRL_IPSG_GetPermitIpPacketRule(unit,
                                                              port,
                                                              NULL,
                                                              vid,
                                                              sip,
                                                              (parameters[PARAM_SA].valid) ? sa : NULL,
                                                              &rule_id
                                                              ))
            {
                res = _E_FAILED;
            ...
            {
                BACKDOOR_MGR_Printf("Rule ID : %lu\n", rule_id);
        ...
        else
        {
            ...
            {
                BACKDOOR_MGR_Printf("vid %s is ignored\n", parameters[PARAM_VID].value);
            }

            if (parameters[PARAM_SRC_IP].valid)
            {
                BACKDOOR_MGR_Printf("sip %s is ignored\n", parameters[PARAM_SRC_IP].value);
            }

            if (parameters[PARAM_SA].valid)
            {
                BACKDOOR_MGR_Printf("sa %s is ignored\n", parameters[PARAM_SA].value);
            }
    ...
    )
{
    ...
    if (options & _OPT_INSTALL || options & _OPT_UNINSTALL)
    {
        ...
        if (FALSE == RULE_CTRL_IPSG_DenyAnyIpPacket(enable, unit, port))
        {
            res = _E_FAILED;
        }
    }
    ...
    )
{
    ...
    if (options & _OPT_INSTALL || options & _OPT_UNINSTALL)
    {
        ...
        if (FALSE == RULE_CTRL_WA_TrapHttpToCPU(enable, unit, port))
        {
            res = _E_FAILED;
        }
    }
    ...
    )
{
    ...
    if (options & _OPT_INSTALL || options & _OPT_UNINSTALL)
    {
        ...
        if (FALSE == RULE_CTRL_WA_DenyAnyIpPacket(enable, unit, port))
        {
            res = _E_FAILED;
        }
    }
    ...
    )
{
    ...
    if (options & _OPT_INSTALL || options & _OPT_UNINSTALL)
    {
        ...
        if (FALSE == RULE_CTRL_WA_PermitIpPacket(enable, unit, port, sip))
        {
            res = _E_FAILED;
        }
    }
    ...
    if (options & _OPT_SHOW)
    {
        ...
        if (parameters[PARAM_UNIT].valid &&
            ...
            parameters[PARAM_SRC_IP].valid)
        {
            unit = (UI32_T)atoi(parameters[PARAM_UNIT].value);
            port = (UI32_T)atoi(parameters[PARAM_PORT].value);
            L4_Backdoor_Str2IpAddr(parameters[PARAM_SRC_IP].value, sip);

            if (FALSE == RULE_CTRL_WA_GetPermitIpPacketRule(unit, port, sip, NULL, &enable, &rule_id))
            {
                res = _E_FAILED;
            }
            else
            {
                BACKDOOR_MGR_Printf("Status  : %s\n", enable? "Enabled" : "Disabled");

                if (enable)
                {
                    BACKDOOR_MGR_Printf("Rule ID : %lu\n", rule_id);

                    if (FALSE == RULE_CTRL_WA_GetPermitIpPacketRule(unit, port, sip, BACKDOOR_MGR_Printf, &enable, &rule_id))
                    {
                        res = _E_FAILED;
                    }
                }
    ...
    )
{
    ...
    if (options & _OPT_INSTALL || options & _OPT_UNINSTALL)
    {
        ...
        if (FALSE == RULE_CTRL_WA_PermitDhcp(enable, unit, port))
        {
            res = _E_FAILED;
        }
    }
    ...
    )
{
    ...
    if (options & _OPT_INSTALL || options & _OPT_UNINSTALL)
    {
        ...
        if (FALSE == RULE_CTRL_WA_PermitDns(enable, unit, port))
        {
            res = _E_FAILED;
        }
    }
    ...
    )
{
    int res = _S_OK;
    UI32_T vid;
    UI32_T rule_id_ar[SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK]
    ...
    {
        enable = (options & _OPT_INSTALL) ? TRUE : FALSE;
        vid = (UI32_T)atoi(parameters[PARAM_VID].value);
        L4_Backdoor_Str2Ip6Addr(parameters[PARAM_DST_IP6].value, ip6);

        if (FALSE == RULE_CTRL_IP6_TrapLinkLocalToCPU(enable, ip6, vid))
        {
            res = _E_FAILED;
        }
    }
    ...
    {
        if (parameters[PARAM_VID].valid &&
            parameters[PARAM_DST_IP6].valid)
        {
            vid = (UI32_T)atoi(parameters[PARAM_VID].value);
            L4_Backdoor_Str2Ip6Addr(parameters[PARAM_DST_IP6].value, ip6);

            if (RULE_CTRL_GetTrapLinkLocalRule(NULL, ip6, vid, rule_id_ar))
            {
                UI32_T unit;
                UI32_T device;

                BACKDOOR_MGR_Printf("Unit Device Rule ID\n");
                for (unit=0; unit<SYS_ADPT_MAX_NBR_OF_UNIT_PER_STACK; unit++)
                {
                    for (device=0; device<SYS_ADPT_MAX_NBR_OF_CHIP_PER_UNIT; device++)
                    {
                       BACKDOOR_MGR_Printf("%4lu %6lu %6lu\n", unit+1, device, rule_id_ar[unit][device]);
                    }
                }
            }

            RULE_CTRL_GetTrapLinkLocalRule(BACKDOOR_MGR_Printf, ip6, vid, rule_id_ar);
        }
        else
        {
            RULE_CTRL_Filter_T  filter;
            ...
            if (parameters[PARAM_VID].valid)
            {
                BACKDOOR_MGR_Printf("vid %lu is ignored\n", parameters[PARAM_VID].value);
            }

            if (parameters[PARAM_DST_IP6].valid)
            {
                BACKDOOR_MGR_Printf("dip6 %lu is ignored\n", parameters[PARAM_DST_IP6].value);
            }
...
static void L4_Backdoor_DiffServ_DumpIpAce(const RULE_TYPE_Ace_Entry_T *ace_p)
{
    ...
    {
        BACKDOOR_MGR_Printf ("/ icmp-type %021x icmp-type-mask %21x", ip_ace->aceIcmpType, 0xff);
    }
...
static void L4_Backdoor_DiffServ_DumpIpv6Ace(const RULE_TYPE_Ace_Entry_T *ace_p)
{
    ...
    if (FALSE == L4_Backdoor_IsNullIpv6Addr(ipv6_ace->aceSourceIpAddr))
    {
        ...
        L4_Backdoor_ShowIpv6Addr ((UI8_T *) ipv6_ace->aceSourceIpAddr, ipv6_ace->aceSourceIpAddrBitmask);
    }

    if (FALSE == L4_Backdoor_IsNullIpv6Addr(ipv6_ace->aceDestIpAddr))
    {
        BACKDOOR_MGR_Printf ("/ dipv6:");
        L4_Backdoor_ShowIpv6Addr ((UI8_T *) ipv6_ace->aceDestIpAddr, ipv6_ace->aceDestIpAddrBitmask);
    }
...
static void L4_Backdoor_DiffServ_ShowTimeRangeTable(int argc, char** argv)
{
    ...
    switch (argc)
    {
        case 1:
            if ('*' == argv[0][0])
            {
                while (RULE_TYPE_OK == L4_MGR_GetNextTimeRange(&time_range_index, &time_range_entry))
                {
                    BACKDOOR_MGR_Printf("\r\n-------------------------------------------------");
                    BACKDOOR_MGR_Printf("\r\n time range name(index=%u)(active=%u)(ref_count=%lu): %s", time_range_index, time_range_entry.active, time_range_entry.ref_counter, time_range_entry.name);

                    if (TRUE == time_range_entry.check_start_absolute)
                        BACKDOOR_MGR_Printf("\r\n start absolute : %4u.%s.%02u %2u:%02u ", time_range_entry.start_time.year,
                            month[time_range_entry.start_time.month], time_range_entry.start_time.day, time_range_entry.start_time.hour,
                            time_range_entry.start_time.minute);

                    if (TRUE == time_range_entry.check_end_absolute)
                        BACKDOOR_MGR_Printf("\r\n end absolute   : %4u.%s.%02u %2u:%02u", time_range_entry.end_time.year,
                            month[time_range_entry.end_time.month], time_range_entry.end_time.day, time_range_entry.end_time.hour,
                            time_range_entry.end_time.minute);
                    ...
                    for (periodic_index = 0; periodic_index <SYS_ADPT_MAX_NBRS_OF_TIME_ACL_PERIODIC; periodic_index++)
                    {
                        if (TRUE == time_range_entry.check_periodic[periodic_index])
                            BACKDOOR_MGR_Printf("\r\n periodic[%lu] : %8s %2u:%02u -- %8s %2u:%02u", periodic_index,
...
static void L4_Backdoor_DiffServ_ShowResourceInfo(int argc, char** argv)
{
    ...
    else if (3 == argc)
    {
        if (FALSE == RULE_CTRL_DumpResourceInfo(atoi(argv[0]), atoi(argv[1]), atoi(argv[2])))
            BACKDOOR_MGR_Printf("\r\nFailed to call RULE_CTRL_DumpResourceInfo()");
    }
}
...
static void L4_Backdoor_DiffServ_AddAclMirror(int argc, char** argv)
{
    ...
    ret = L4_MGR_ACL_AddAclMirrorEntry(ifindex_dest, acl_name, VAL_mirrorType_rx);
