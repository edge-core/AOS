...
get_sock_port(int sock, int local)
{
    ...
    if (local)
    {
        res = getsockname(sock, (struct sockaddr *)&sock_addr,(socklen_t *)&sock_addr_size);
    }
    else
    {
        res = getpeername(sock, (struct sockaddr *)&sock_addr, (socklen_t *)&sock_addr_size);
    }
    ...
    if (res == 0)
    {
        ...
        {
            struct  sockaddr_in *sock_in = (struct sockaddr_in*) &sock_addr;

            return (sock_in->sin_port);
        }
        else
        {
            struct  sockaddr_in6 *sock_in6 = (struct sockaddr_in6*) &sock_addr;

            return (sock_in6->sin6_port);
        }
    }
