...
/* FUNCTION NAME:  SSHD_INIT_InitiateSystemResources
 * PURPOSE:
...
void SSHD_INIT_Create_InterCSC_Relation(void)
{
    BACKDOOR_MGR_Register_SubsysBackdoorFunc_CallBack(
        "SSH", SYS_BLD_SSH_GROUP_IPCMSGQ_KEY, SSHD_MGR_BackdoorFunction);
...
/* FUNCTION	NAME : SSHD_INIT_RegisterCallBackFunction
 * PURPOSE:
 *          This call will set callback function.
 *
 *
 * INPUT:
 *		    None.
 ...
 *		    None.
 */
void SSHD_INIT_RegisterCallBackFunction(void)
{
    /* LOCAL CONSTANT DECLARATIONS
    */
