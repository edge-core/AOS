...
static void L_IPCMEM_Backdoor_VerifyTmz(void)
{
    ...
    /*  BODY
     */
    if(FALSE==(BOOL_T)SYSFUN_Syscall(SYSFUN_SYSCALL_L_IPCMEM, (void*)L_IPCMEM_CMD_BACKDOOR, (void*)L_IPCMEM_BACKDOOR_CMD_GETTMZINFO, (void*)TRUE, (void*)tmz, NULL))
    {
        BACKDOOR_MGR_Printf("\r\nsys_L_IPCMEM fails");
        return;
    }
    ...
    if(FALSE==(BOOL_T)SYSFUN_Syscall(SYSFUN_SYSCALL_L_IPCMEM, (void*)L_IPCMEM_CMD_BACKDOOR, (void*)L_IPCMEM_BACKDOOR_CMD_GETTMZINFO, (void*)FALSE, (void*)tmz, NULL))
    {
        BACKDOOR_MGR_Printf("\r\nsys_L_IPCMEM fails");
        return;
    }
...
static void L_IPCMEM_Backdoor_DisplayLMEMCounter(void)
{
    ...
    if(FALSE == (BOOL_T)(SYSFUN_Syscall(SYSFUN_SYSCALL_L_IPCMEM, (void*)L_IPCMEM_CMD_BACKDOOR, (void*)L_IPCMEM_BACKDOOR_CMD_GETBACKDOORINFO, backdoor_info, &allocate_fail_counter, NULL)))
    {
        BACKDOOR_MGR_Printf("\r\nsys_L_IPCMEM fails");
        return;
    }
    ...
    for (buf_type=0; buf_type<L_IPCMEM_NUMBER_OF_BUF_TYPE; buf_type++)
    {
        BACKDOOR_MGR_Printf ("       (%4d/%4d;%8lu/%8lu;%4ld/%4ld/%4ld)\r\n",
                    (int)backdoor_info[buf_type].partition_len,
                    (int)backdoor_info[buf_type].num_of_block,
                    (unsigned long)backdoor_info[buf_type].alloc_counter,
                    (unsigned long)backdoor_info[buf_type].free_counter,
                    (long)backdoor_info[buf_type].used_count,
                    (long)backdoor_info[buf_type].peak_count,
                    (long)backdoor_info[buf_type].fail_count);
    }
