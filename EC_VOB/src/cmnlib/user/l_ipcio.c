...
static UI32_T L_IPCIO_PrintBuffer(SYSFUN_MsgQ_T msgq_handle, const L_IPCIO_UserInfo_T *user_info_p, UI32_T wait_time, UI32_T buf_len, const void *buf);
 ...
 *           user_info_p - see desctipon of L_IPCIO_UserInfo_T
 ...
 * NOTE    : None.
 *----------------------------------------------------------------*/
UI32_T L_IPCIO_GetChar(SYSFUN_MsgQ_T msgq_handle, const L_IPCIO_UserInfo_T *user_info_p, int *ch_p)
{
    ...
    ipcio_msg_p->ipcio_user_info = *user_info_p;
 ...
 *----------------------------------------------------------------*/
UI32_T L_IPCIO_GetLine(SYSFUN_MsgQ_T msgq_handle, const L_IPCIO_UserInfo_T *user_info_p, UI32_T buf_size, char *buf_p)
{
    SYSFUN_Msg_T *msg_p;
    L_IPCIO_Msg_T *ipcio_msg_p;

    if (user_info_p == NULL || buf_size == 0 || buf_p == NULL)
        return L_IPCIO_ERR_ARG;
 ...
 * NOTE    : None.
 *----------------------------------------------------------------*/
UI32_T L_IPCIO_Print(SYSFUN_MsgQ_T msgq_handle, const L_IPCIO_UserInfo_T *user_info_p, UI32_T wait_time, char *str_p)
{
    UI32_T str_len;

    if (user_info_p == NULL || str_p == NULL)
        return L_IPCIO_ERR_ARG;
    ...
    return L_IPCIO_PrintBuffer(msgq_handle, user_info_p, wait_time, str_len, str_p);
}
 ...
 * NOTE    : None.
 *----------------------------------------------------------------*/
UI32_T L_IPCIO_Printf(SYSFUN_MsgQ_T msgq_handle, const L_IPCIO_UserInfo_T *user_info_p, UI32_T wait_time, char *fmt_p, ...)
{
    UI32_T ret;
    char *buf;
    int len;
    ...
    if (user_info_p == NULL || fmt_p == NULL)
        return L_IPCIO_ERR_ARG;
    if (SYSFUN_GetMsgQOwner(msgq_handle) == 0)
        return L_IPCIO_ERR_IPC;
    ...
    ret = L_IPCIO_PrintBuffer(msgq_handle, user_info_p, wait_time, len, buf);
 ...
 * NOTE    : None.
 *----------------------------------------------------------------*/
UI32_T L_IPCIO_VPrintf(SYSFUN_MsgQ_T msgq_handle, const L_IPCIO_UserInfo_T *user_info_p, UI32_T wait_time, char *fmt_p, va_list arg_p)
{
    UI32_T ret;
    char *buf;
    int len;
...
/*-----------------------------------------------------------------
 * ROUTINE NAME - L_IPCIO_DummyHandleIPCIOMsg
 *-----------------------------------------------------------------
 * FUNCTION: A dummy function for process unhandling IPCIO msg.
 * INPUT   : msg_p             - the ipcio msg.
 * OUTPUT  : None.
 * RETUEN  : TRUE  - need to send response.
 *           FALSE - not need to send response.
 * NOTE    : Shall always call this function in user-defined
 *           HandleIPCReqMsg function to handle defaut switch case.
 *----------------------------------------------------------------*/
BOOL_T L_IPCIO_DummyHandleIPCIOMsg(SYSFUN_Msg_T *msg_p)
{
    ...
    {
        /* If not support GETCHAR/GETLINE,
         * work as get EOF and set ipcio_ret to FALSE to indicate error occur.
         */
        ...
        /* If not support PRINT, do nothing.
         */
        case L_IPCIO_CMD_PRINT:
            break;

        /* assertion failed!
         * this function shall handle all commands.
         */
        default:
 ...
 * NOTE    : None.
 *----------------------------------------------------------------*/
void L_IPCIO_ParseKey(SYSFUN_MsgQ_T msgq_handle, char ch, BOOL_T *need_wait_key_p)
{
