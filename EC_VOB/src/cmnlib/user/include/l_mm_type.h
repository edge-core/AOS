...
enum {
    ...
    L_MM_BACKDOOR_CMD_MREF_CORRUPT,
    L_MM_BACKDOOR_CMD_TOGGLEMREFERRDETECTFLAG
};
 ...
 */
#define L_MM_GET_TRAIL_MAGIC_BYTE_ADDR(monitor_p) ((UI8_T*)(monitor_p) + (monitor_p)->buf_len - L_MM_TRAIL_MAGIC_BYTE_LEN)
...
typedef struct
{
    void*  buf_p;      /* used when free_func_type is L_MM_MREF_FREE_FUN_CUSTOM */
    I32_T  buf_offset;
    UI16_T buf_len;
    UI16_T pdu_offset;
    UI16_T pdu_len;
...
#define L_MM_DEBUG_DUMP_MONITOR_INFO(print_fun, monitor_p) \
    do {\
        if (monitor_p!=NULL)\
        {\
            UI32_T current_tick = SYSFUN_GetSysTick();\
            print_fun("\r\n<0>taskid=%d,elapsed_time=%d,buf_len=%d,module_id=%u,pool_id=%u,trace_id=%u\r\n",\
                   (int)monitor_p->task_id,\
                   (monitor_p->allocation_time > current_tick) ? (int)(0xFFFFFFFF - (monitor_p->allocation_time-current_tick)) : (int)(current_tick-monitor_p->allocation_time),\
                   (int)monitor_p->buf_len,\
...
#define L_MM_MREF_PARAMETER_BLOCK_LEN  16 /* size of parameter block(byte) in L_MM_Mref_Handle_T */
...
typedef struct
{
    ...
    UI8_T   cos;            /* rx CPU queue priority */
    BOOL_T  pkt_is_truncated;   /* a oversize pkt (with correct FCS) will be truncated to store in CPU rx buf */
    BOOL_T  not_forward;    /* true means don't do software route */
    BOOL_T  rx_is_tagged;    /* true means recived packet is tag */
    UI32_T  ether_header_length; /* DA+SA+tag+type*/
    UI32_T  rx_reason; /* the reason that the packets are trapped to CPU */	
    UI8_T   rx_unit;        /* packet's receive unit */
    UI8_T   rx_port;        /* packet's receive port */
} L_MM_Mref_PktInfo_T;
...
typedef struct
{
    ...
    L_MM_Mref_PktInfo_T pkt_info;       /* extra pkt info */
} L_MM_Mref_Handle_T;
