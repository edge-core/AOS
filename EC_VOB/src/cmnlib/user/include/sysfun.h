 ...
 * it's used by SYSFUN_InvokeLinuxShell and checked by CLI_API_LinuxShell
 */
#define SYSFUN_FLAG_LINUX_SHELL_RUNNING "/var/run/under_linux_shell"
...
typedef enum
{
    SYSFUN_UART_MODE_CONSOLE_RAW_MODE = 0,
...
#define SYSFUN_OM_LEAVE_CRITICAL_SECTION(sem_id, orig_priority) do { \
    ...
    orig_priority; /* workaround for compiler warning:variable 'original_priority' set but not used */ \
} while (0)
 ...
 */
#if 0
/* when using "(UI32_T)(((SYSFUN_Msg_T *)0)->msg_buf)" in declaration of
 * static array in gcc v4.8.1, it will have error like "variably modified 'array' at file scope"
 */
#define SYSFUN_SIZE_OF_MSG(msg_size) ((UI32_T)(((SYSFUN_Msg_T *)0)->msg_buf) + (UI32_T)(msg_size))
#else
/* sizeof(SYSFUN_Msg_T)-4 is the real size used by SYSFUN_Msg_T.
 * "-4" part is to reduce the filed msg_buf, which is not really used by SYSFUN_Msg_T
 * the filed msg_buf is acctually the start of the buffer used by the SYSFUN caller
 * CSC.
 */
#define SYSFUN_SIZE_OF_MSG(msg_size) ((UI32_T)(sizeof(SYSFUN_Msg_T)-4) + (UI32_T)(msg_size))
#endif
...
/* The definition of msgq
 */
typedef void *SYSFUN_MsgQ_T;
...
typedef struct
{
    long msg_type;           /* The type of this message.
    ...
    UI8_T  eh_buf_idx;       /* EH buffer index bound to the caller thread.
                              * Only take effect when sending a bidirection
                              * request message by a UI thread.
                              */
    UI8_T  reserved[3];      /* padding to keep msg_buf aligned to multiple of 4
                              * bytes
                              */
    UI8_T  msg_buf[4];       /* msg_buf: start address of buffer.
                              *          to store the message payload.
                              */
} SYSFUN_Msg_T;

typedef I64_T SYSFUN_Time_T;
...
#define SYSFUN_DumpCurrentTaskStack()                                               \
    ...
    {                                                                               \
        char commd_line[128];                                                       \
        SYSFUN_Sprintf(commd_line, "cat /proc/%ld/ustack", (long)SYSFUN_TaskIdSelf());    \
        system(commd_line);                                                         \
    } while (0)
...
#if 1 /////////////////////////////////////////////////
UI32_T __SYSFUN_SendRequestMsg(SYSFUN_MsgQ_T msgq_handle,
...
# define SYSFUN_SendRequestMsg(msgq_handle, req_msg_p, wait_time, event, res_msg_size, res_msg_p)       \
    ({                                                                                                  \
        ...
        if (__rc != SYSFUN_OK) {                                                                        \
            printf("%s() line %d:Failed to send/receive IPC message.(msgq_key=%lu, rc=%lu)\r\n",        \
                   __FUNCTION__, __LINE__, (unsigned long)(((SYSFUN_MsgQDesc_T *)msgq_handle)->msgq_key), (unsigned long)__rc);         \
...
# define SYSFUN_SendRequestMsg(msgq_handle, req_msg_p, wait_time, event, res_msg_size, res_msg_p)       \
    ({                                                                                                  \
        ...
        if (rc != SYSFUN_OK)                                                                            \
            printf("%s() line %d:Failed to send/receive IPC message.(rc=%lu)\r\n",                      \
 ...
 */
#if 1 /////////////////////////////////////////////////
UI32_T __SYSFUN_SendResponseMsg(SYSFUN_MsgQ_T msgq_handle, SYSFUN_Msg_T *res_msg_p
#ifdef SYSFUN_DEBUG
...
# define SYSFUN_SendResponseMsg(msgq_handle, res_msg_p)                                            \
    ({                                                                                             \
        ...
        if (__rc != SYSFUN_OK) {                                                                   \
            printf("%s() line %d: Failed to send response IPC message.(msgq_key=%lu, rc=%lu)\r\n", \
                    __FUNCTION__, __LINE__, (unsigned long)(((SYSFUN_MsgQDesc_T *)msgq_handle)->msgq_key), (unsigned long)__rc);   \
        }                                                                                          \
        __rc;                                                                                      \
    })
...
# define SYSFUN_SendResponseMsg(msgq_handle, res_msg_p)                                 \
    ({                                                                                  \
        ...
        if (rc != SYSFUN_OK)                                                            \
            printf("%s() line %d: Failed to send response IPC message.(rc=%lu)\r\n",    \
                    __FUNCTION__, __LINE__, (unsigned long)rc);                                        \
        rc;                                                                             \
    })
#else
...
#if defined (SYSFUN_DEBUG) && defined (SYSFUN_DEBUG_ERROR_CHECK)
# define SYSFUN_ReceiveMsg(msgq_handle, msg_type, wait_time, req_msg_size, req_msg_p)            \
    ({                                                                                           \
        UI32_T __rc;                                                                             \
        __rc = __SYSFUN_ReceiveMsg(msgq_handle, msg_type, wait_time, req_msg_size,               \
                                   req_msg_p, __func__, __LINE__);                               \
        if ((__rc != SYSFUN_OK) && (__rc != SYSFUN_RESULT_NO_MESSAGE)) {                         \
            printf("%s() line %d:Failed to receive IPC message.(msgq_key=%lu, rc=%lu)\r\n",      \
                    __FUNCTION__, __LINE__, (unsigned long)(((SYSFUN_MsgQDesc_T *)msgq_handle)->msgq_key), (unsigned long)__rc); \
        }                                                                                        \
        __rc;                                                                                    \
    })
#else
# define SYSFUN_ReceiveMsg(msgq_handle, msg_type, wait_time, req_msg_size, req_msg_p)   \
    __SYSFUN_ReceiveMsg(msgq_handle, msg_type, wait_time, req_msg_size, req_msg_p)
#endif /* SYSFUN_DEBUG && SYSFUN_DEBUG_ERROR_CHECK */
...
# define SYSFUN_ReceiveMsg(msgq_handle, msg_type, wait_time, req_msg_size, req_msg_p)       \
    ({                                                                                      \
        UI32_T rc;                                                                          \
        rc = _SYSFUN_ReceiveMsg(msgq_handle, msg_type, wait_time, req_msg_size, req_msg_p); \
        if ((rc != SYSFUN_OK) && (rc != SYSFUN_RESULT_NO_MESSAGE))                          \
            printf("%s() line %d:Failed to receive IPC message.(rc=%lu)\r\n",               \
                    __FUNCTION__, __LINE__, (unsigned long)rc);                                            \
        rc;                                                                                 \
    })
#else
# define SYSFUN_ReceiveMsg(msgq_handle, msg_type, wait_time, req_msg_size, req_msg_p)       \
    _SYSFUN_ReceiveMsg(msgq_handle, msg_type, wait_time, req_msg_size, req_msg_p)
#endif
#endif /////////////////////////////////////////////////
...
#if defined (SYSFUN_DEBUG) && defined (SYSFUN_DEBUG_ERROR_CHECK)
#define SYSFUN_ReceiveResponseMsg(msgq_handle, wait_time, res_msg_size, res_msg_p)  \
    ({                                                                              \
        UI32_T __rc;                                                                \
        __rc = __SYSFUN_ReceiveResponseMsg(msgq_handle, wait_time, res_msg_size,    \
                                           res_msg_p, __func__, __LINE__);          \
        if ((__rc != SYSFUN_OK) && (__rc != SYSFUN_RESULT_NO_MESSAGE))              \
        {                                                                           \
            printf("%s() line %d:Failed to receive response message."               \
                "(msgq_key=%lu, rc=%lu)\r\n", __func__, __LINE__,                   \
                (unsigned long)(((SYSFUN_MsgQDesc_T *)msgq_handle)->msgq_key),      \
                (unsigned long)__rc);                                               \
        }                                                                           \
        __rc;                                                                       \
    })
#else
#define SYSFUN_ReceiveResponseMsg(msgq_handle, wait_time, res_msg_size, res_msg_p)  \
        __SYSFUN_ReceiveResponseMsg(msgq_handle, wait_time, res_msg_size, res_msg_p)
#endif /* SYSFUN_DEBUG && SYSFUN_DEBUG_ERROR_CHECK */
 ...
 *      ipc_socket_id_p  --  The id of the created ipc socket will be put to this variable if success.
 *      rcv_buf_size     --
 *
 * OUTPUT:
 *      ipc_socket_id_p  --  The id of the created ipc socket will be put to this variable if success.
