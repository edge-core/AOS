...
l_timer_list_debug(L_TIMTER_LIST_T *list, UI8_T *str, UI32_T line)
{
  ...
  if (i != list->ll_cnt)
  {
      printf("$$$$$$$$$$$$$$$list[%lu] has error, %s[%lu]\n", (unsigned long)list->ll_idx, str, (unsigned long)line);
...
l_timer_master_debug(L_TIMER_MASTER_T *m)
{
  ...
  for (i = 0; i <= L_TIMER_SLOT; i++)
  {
      ...
      printf("index %lu, list %p, head %p, tail %p, cnt %lu\n", 
                  (unsigned long)list->ll_idx, list, list->head, list->tail, (unsigned long)list->ll_cnt);
	  
      t = list->head;

      while (t)
      {
          printf("%p(%lu)\n ", t, (unsigned long)t->lt_expt);
          t = t->next;
          cnt++;
      }
...
void
l_timer_master_clear_list(L_TIMER_MASTER_T *m)
{
  L_TIMTER_LIST_T *list;
  L_Timer_T *t;
  UI32_T i;
