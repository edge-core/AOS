...
#define L_INET_ADDR_IS_IPV6_LOOPBACK(addr) \
    ((addr[0] == 0x00)  && (addr[1] == 0x00) && \
     (addr[2] == 0x00)  && (addr[3] == 0x00) && \
     (addr[4] == 0x00)  && (addr[5] == 0x00) && \
     (addr[6] == 0x00)  && (addr[7] == 0x00) && \
...
#define L_INET_ADDR_IS_IPV4_LOOPBACK(addr) \
    (addr[0] == 0x7f)
...
#if (SYS_CPNT_CRAFT_PORT == TRUE)
#define L_INET_ADDR_IS_VALID_IPV6_LINK_LOCAL_ZONE_ID(id) \
    (((id) > 0 && (id) <= SYS_ADPT_MAX_VLAN_ID) || ((id) == (SYS_ADPT_CRAFT_INTERFACE_IFINDEX-SYS_ADPT_VLAN_1_IF_INDEX_NUMBER+1)))
#define L_INET_ADDR_IS_VALID_IFINDEX(ifindex) \
    ((ifindex >= SYS_ADPT_VLAN_1_IF_INDEX_NUMBER && ifindex <= (SYS_ADPT_VLAN_1_IF_INDEX_NUMBER+SYS_ADPT_MAX_VLAN_ID-1)) || \
     (ifindex == SYS_ADPT_CRAFT_INTERFACE_IFINDEX))
#else
#define L_INET_ADDR_IS_VALID_IPV6_LINK_LOCAL_ZONE_ID(id) \
    ((id) > 0 && (id) <= SYS_ADPT_MAX_VLAN_ID)
#define L_INET_ADDR_IS_VALID_IFINDEX(ifindex) \
        (ifindex >= SYS_ADPT_VLAN_1_IF_INDEX_NUMBER && ifindex <= (SYS_ADPT_VLAN_1_IF_INDEX_NUMBER+SYS_ADPT_MAX_VLAN_ID-1))
#endif

#define L_INET_ZONE_ID_TO_IFINDEX(id)       (L_INET_ADDR_IS_VALID_IPV6_LINK_LOCAL_ZONE_ID(id)?(id+SYS_ADPT_VLAN_1_IF_INDEX_NUMBER-1):0)
#define L_INET_IFINDEX_TO_ZONE_ID(ifindex)  (L_INET_ADDR_IS_VALID_IFINDEX(ifindex)?(ifindex-SYS_ADPT_VLAN_1_IF_INDEX_NUMBER+1):0)

#define L_INET_EXPAND_MAC(mac) ((UI8_T*)mac)[0],((UI8_T*)mac)[1],((UI8_T*)mac)[2],((UI8_T*)mac)[3],((UI8_T*)mac)[4],((UI8_T*)mac)[5]
#define L_INET_EXPAND_IP(ip)  ((UI8_T *)&(ip))[0],((UI8_T *)&(ip))[1],((UI8_T *)&(ip))[2],((UI8_T *)&(ip))[3]
#define L_INET_EXPAND_IPV4(ip) ((UI8_T*)(ip))[0],((UI8_T*)(ip))[1],((UI8_T*)(ip))[2],((UI8_T*)(ip))[3]
#define L_INET_EXPAND_IPV6(ip) (unsigned long)L_STDLIB_Ntoh32(((UI32_T*)(ip))[0]),(unsigned long)L_STDLIB_Ntoh32(((UI32_T*)(ip))[1]),(unsigned long)L_STDLIB_Ntoh32(((UI32_T*)(ip))[2]),(unsigned long)L_STDLIB_Ntoh32(((UI32_T*)(ip))[3])
...
/*-------------------------------------------------------------------------
 * FUNCTION NAME - L_INET_ApplyMask
 *-------------------------------------------------------------------------
 * PURPOSE : Apply mask (i.e. prefix length) to IPv4/v6 address
 * INPUT   : ipaddr_p -- L_INET_AddrIp_T
 * OUTPUT  : ipaddr_p -- L_INET_AddrIp_T (after apply mask)
 * RETURN  : None
 * NOTE    : Refer from apply_mask() in quagga lib
 *-------------------------------------------------------------------------
 */
void
L_INET_ApplyMask(L_INET_AddrIp_T *ipaddr_p);
