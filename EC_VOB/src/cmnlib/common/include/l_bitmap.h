...
#define L_BITMAP_PORT_ISSET(bitmap, port) (((port)>=1&&(port)<=SYS_ADPT_TOTAL_NBR_OF_LPORT)?(((bitmap)[((port) -1) / 8] & (0x80 >> (((port) -1) % 8))) != 0):0)

#define L_BITMAP_PORT_SET(bitmap, port) do {if((port)>=1&&(port)<=SYS_ADPT_TOTAL_NBR_OF_LPORT)(bitmap)[((port) -1) / 8] |= (0x80 >> (((port) -1) % 8));}while(0)

#define L_BITMAP_PORT_UNSET(bitmap, port) do {if((port)>=1&&(port)<=SYS_ADPT_TOTAL_NBR_OF_LPORT)(bitmap)[((port) -1) / 8] &= ~( 0x80 >> (((port) -1) % 8));}while(0)
