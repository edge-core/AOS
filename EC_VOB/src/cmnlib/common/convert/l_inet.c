...
/* Maskbit. */
static const UI8_T maskbit[] = {0x00, 0x80, 0xc0, 0xe0, 0xf0,
                                0xf8, 0xfc, 0xfe, 0xff};
    ...
    if (TRUE == L_INET_IS_IP_ADDR_TYPE(inaddr_p->type))
    {
        ...
        if (TRUE == L_INET_IS_LINK_LOCAL_ADDR_TYPE(ipaddr_p->type))
        {
            ...
            if (ipaddr_p->zoneid != L_INET_ADDR_DFLT_ZONE_ID)
            {
                ...
                snprintf(zoneid_str, sizeof(zoneid_str), "%%%lu", (unsigned long)ipaddr_p->zoneid);
    ...
    char *str_p)
{
    ...
    if (L_INET_IS_IP_ADDR_TYPE(inaddr_p->type))
    {
        ...
        if (L_INET_IS_IPV4_ADDR_TYPE(ipaddr_p->type))
        {
            if (ipaddr_p->preflen > L_INET_MAX_IPV4_PREFIX_LEN)
                return FALSE;
            if (inet_ntop(AF_INET, (void*)ipaddr_p->addr, str_p, (size_t)str_len) == NULL)
                return FALSE;
        }
        else if (L_INET_IS_IPV6_ADDR_TYPE(ipaddr_p->type))
        {
            if (ipaddr_p->preflen > L_INET_MAX_IPV6_PREFIX_LEN)
                return FALSE;
            if (inet_ntop(AF_INET6, (void*)ipaddr_p->addr, str_p, (size_t)str_len) == NULL)
                return FALSE;
        }
    ...
    L_INET_AddrIp_T *ipaddr_p)
{
    ...
    else if(AF_INET6 == sockaddr_p->sa_family)  /* IPv6 */
    {
         ...
         */
        if(0 == memcmp(v4_prefix, &sock_addr6->sin6_addr, v4_prefix_len))
        {
            ipaddr_p->addrlen = SYS_ADPT_IPV4_ADDR_LEN;
            memcpy(ipaddr_p->addr, (UI8_T *)&sock_addr6->sin6_addr + v4_prefix_len, SYS_ADPT_IPV4_ADDR_LEN);
            ipaddr_p->type = (L_INET_ADDR_IS_IPV4_LINK_LOCAL(ipaddr_p->addr)) ?
                                L_INET_ADDR_TYPE_IPV4Z : L_INET_ADDR_TYPE_IPV4;
        }
        else  /* IPv6 */
        {
            ipaddr_p->addrlen = SYS_ADPT_IPV6_ADDR_LEN;
            memcpy(ipaddr_p->addr, &sock_addr6->sin6_addr, SYS_ADPT_IPV6_ADDR_LEN);
    ...
    struct sockaddr *sockaddr_p)
{
    ...
    if (L_INET_IS_IPV4_ADDR_TYPE(ipaddr_p->type))
    {
        if(sizeof(struct sockaddr_in) <= sockaddr_len)
        {
            ...
            sin4->sin_port   = L_STDLIB_Hton16(port_number);
    ...
    else if (L_INET_IS_IPV6_ADDR_TYPE(ipaddr_p->type))
    {
        if(sizeof(struct sockaddr_in6) <= sockaddr_len)
        {
            ...
            sin6->sin6_port   = L_STDLIB_Hton16(port_number);
...
L_INET_ApplyMask(L_INET_AddrIp_T *ipaddr_p)
{
    ...
    if (!ipaddr_p || !L_INET_IS_IP_ADDR_TYPE(ipaddr_p->type))
        return;

    max_addr_len = L_INET_IS_IPV6_ADDR_TYPE(ipaddr_p->type)? SYS_ADPT_IPV6_ADDR_LEN : SYS_ADPT_IPV4_ADDR_LEN;

    index = ipaddr_p->preflen / 8;

    if (index < max_addr_len)
    {
        pnt = ipaddr_p->addr;
        offset = ipaddr_p->preflen % 8;
    ...
    const char *str_p)
{
    ...
    if (NULL != (zoneid_p = strchr(buf, '%')))
    {
        ...
        *zoneid_p = '\0';
        ...
        if (*tmp_p != '\0' ||
            (FALSE == L_INET_ADDR_IS_VALID_IPV6_LINK_LOCAL_ZONE_ID(zoneid)))
        {
            return L_INET_RETURN_FAIL;
        }
    }
