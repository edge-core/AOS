...
int L_PREFIX_AtoIPV6(const UI8_T *strptr, UI8_T *addrptr)
{
    ...
    for (;;)
    {
        ...
        if (*s != ':')
        {
            if (*s == '.' && len >= 2)
            {
                ...
                if (L_INET_Aton(s, (UI32_T *)addrptr) == 1)
                {
                    len -= 2;
                    break;
                }
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        if (i == 6 && isdigit(*s)
            ...
            && L_INET_Aton(s, (UI32_T *) &in6_val[i]) == TRUE)
...
/* ref. BGP_MGR_InaddrToPrefix */
BOOL_T L_PREFIX_InaddrToPrefix(L_INET_AddrIp_T *addr_p, struct prefix *p)
{
    switch (addr_p->type)
    {
        ...
        case L_INET_ADDR_TYPE_IPV4Z:
            p->family = AF_INET;
            p->prefixlen = addr_p->preflen;
            memcpy(&(p->u.prefix), addr_p->addr, SYS_ADPT_IPV4_ADDR_LEN);
            break;
        case L_INET_ADDR_TYPE_IPV6:
        case L_INET_ADDR_TYPE_IPV6Z:
            p->family = AF_INET6;
            p->prefixlen = addr_p->preflen;
            memcpy(&(p->u.prefix), addr_p->addr, SYS_ADPT_IPV6_ADDR_LEN);
            break;
        default:
            return FALSE;
    }
...
/* ref. BGP_MGR_PrefixToInaddr */
/* for IPV6Z, the caller should assign correct zoneid again by itself */ 
BOOL_T L_PREFIX_PrefixToInaddr(struct prefix *p, L_INET_AddrIp_T *addr_p)
{
    ...
    switch (p->family)
    {
        case AF_INET:    
            ...
            memcpy(addr_p->addr, &(p->u.prefix),SYS_ADPT_IPV4_ADDR_LEN);
            addr_p->preflen = p->prefixlen;
            break;
        case AF_INET6:
            //addr = p->u.prefix;
            //if(L_INET_ADDR_IS_IPV6_LINK_LOCAL(p->u.prefix))
            if(L_INET_ADDR_IS_IPV6_LINK_LOCAL((&(p->u.prefix))))
                addr_p->type = L_INET_ADDR_TYPE_IPV6Z;
            else
                addr_p->type = L_INET_ADDR_TYPE_IPV6;

            memcpy(addr_p->addr, &(p->u.prefix), SYS_ADPT_IPV6_ADDR_LEN);
            addr_p->addrlen = SYS_ADPT_IPV6_ADDR_LEN; 
            addr_p->preflen = p->prefixlen;
            break;
        default:
            return FALSE;
