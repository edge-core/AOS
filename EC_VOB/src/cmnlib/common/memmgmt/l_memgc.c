 ...
 */
L_MEMGC_Hdlr_T L_MEMGC_Create(void *given_buffer, UI32_T buffer_length, UI32_T block_size)
{
    /* LOCAL VARIABLES
     */
    Handle_T    handle;
    ...
    /* aligned to 4 bytes boundary */
    {
        ...
        buffer_length -= ( (uintptr_t)buffer - (uintptr_t)given_buffer );
 ...
 *       size is block_size x 32
 */
void *L_MEMGC_Allocate (L_MEMGC_Hdlr_T handle, UI32_T size)
{
 ...
 * RETURN:  TRUE/FALSE
 */
BOOL_T L_MEMGC_Free (L_MEMGC_Hdlr_T handle, void *buffer)
{
    Handle_T *hp = (Handle_T*)handle;
    UI32_T  blockno;
    UI64_T  map;
    ...
    offset = (uintptr_t)buffer - (uintptr_t)hp->buffer;
