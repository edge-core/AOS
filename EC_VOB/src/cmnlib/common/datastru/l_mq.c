...
UI32_T L_MQ_DoOperationAndMoveHead(L_MQ_Desc_T desc, UI16_T qno, UI8_T *endp, void *func)
{
    ...
    ret = L_CIRLST_DoOperationAndMoveHead(id_p->queue[qno], endp, func, L_CVRT_UINT_TO_PTR(qno));
    ...
    const UI16_T queue_length [] )
{
    ...
    /* initiate queue */
    {
        ...
        for ( i=0 ; i<nbr_of_queue ; i++ )
        {
            fail |= (L_CIRLST_ShMem_Create (list_p, L_CVRT_GET_PTR(desc_p, queue_buffer_offset), (UI32_T)queue_length[i], L_MQ_MESSAGE_SIZE) != TRUE);
            list_p++;
...
UI32_T L_MQ_ShMem_DoOperationAndMoveHead(L_MQ_ShMem_Desc_T *desc_p, UI16_T qno, UI8_T *endp, void *func)
{
    ...
    ret = L_CIRLST_ShMem_DoOperationAndMoveHead(SHMEM_Q(desc_p, qno), endp, func, L_CVRT_UINT_TO_PTR(qno));
