...
#define L_HISAM_KEY_COMPARE(ipart_p)   ( __KeyCompare(ipart_p->in_key,  ipart_p->work_key, \
                                                     &ipart_p->key_def, ipart_p->key_type))
...
#define L_HISAM_SHMEM_GET_HASH_TABLE_SIZE(desc)   (desc->total_hash_nbr  * sizeof(Ridx_T))
#define L_HISAM_SHMEM_GET_INDEX_TABLE_SIZE(desc)  (desc->total_index_nbr * sizeof(Index_T))
#define L_HISAM_SHMEM_GET_RLINK_TABLE_SIZE(desc)  (desc->total_record_nbr* sizeof(Ridx_T))
...
#define L_HISAM_SHMEM_HASH_TABLE(ipart)   ((Ridx_T *)L_CVRT_GET_PTR((ipart), (ipart)->hash_table_offset))
#define L_HISAM_SHMEM_RLINK_TABLE(ipart)  ((Ridx_T *)L_CVRT_GET_PTR((ipart), (ipart)->rlink_table_offset))
 ...
 */
#ifdef SYS_ADPT_L_DLST_INDEX_T
typedef SYS_ADPT_L_HISAM_RIDX_T Ridx_T;
#else
typedef UI16_T Ridx_T;
#endif

typedef struct
{
   Ridx_T   ridx_1st;         /* 1st ridx of record-list */
   Ridx_T   ridx_last;        /* last ridx of record-list */
   Ridx_T   nbr;
} Index_T;
...
typedef struct
{
   ...
   L_HISAM_KeyType_T       key_type[L_HISAM_MAX_FIELD_NUMBER_OF_A_KEY];
   Index_T                 *index_table;           /* point to index table       */
   Ridx_T                  *hash_table;            /* point to hash table        */
   Ridx_T                  *rlink_table;           /* point to Record Link table */
   UI16_T                  key_length;
   Ridx_T                  curr_index_nbr;
...
static int __KeyCompare(char *in_key_p, char *work_key_p, L_HISAM_KeyDef_T *key_def_p,
                        L_HISAM_KeyType_T *key_type_p);

#ifdef HISAM_DEBUG
static void    L_HISAM_Dump (L_HISAM_Desc_T *desc, UI32_T kidx);
static void    L_HISAM_Dump1 (L_HISAM_Desc_T *desc, UI32_T kidx);
#endif
static BOOL_T  L_HISAM_InternalDeleteRecord (L_HISAM_Desc_T *desc, UI8_T *key, UI8_T *del_rec);
 ...
 *          key_type_table     -- 2-dimension array for definition of data type
 ...
 *                                key_type_table[0][0] defines the data
 ...
 *          5. Caller must specify correct data type in key_type_table
...
BOOL_T L_HISAM_CreateV2(L_HISAM_Desc_T *desc, UI32_T number_of_key,
                        ...
                        const L_HISAM_KeyType_T key_type_table[][L_HISAM_MAX_FIELD_NUMBER_OF_A_KEY])

{
   ...
   {
      ipart->hash_table  = MyMalloc( desc->total_hash_nbr  * sizeof(Ridx_T)  );
      ipart->index_table = MyMalloc( desc->total_index_nbr * sizeof(Index_T) );
      ipart->rlink_table = MyMalloc( desc->total_record_nbr* sizeof(Ridx_T)  );
      ...
      if ( key_type_table != 0 )
          memcpy (ipart->key_type, key_type_table[i], sizeof(ipart->key_type) );
      else
          memset (ipart->key_type, L_HISAM_NOT_INTEGER, sizeof(ipart->key_type));
 ...
 */
UI32_T L_HISAM_SetRecord (L_HISAM_Desc_T *desc, void *rec, BOOL_T is_static)
{
   /* LOCAL VARIABLES
    */
   Ipart_T  *ipart;
...
BOOL_T L_HISAM_DeleteRecord (L_HISAM_Desc_T *desc, UI8_T *key)
{
    ...
    return L_HISAM_InternalDeleteRecord(desc, key, NULL);
}

/* PURPOSE: Delete a specified record and output the record if
 *          the deletion successes.
 * INPUT:   desc    -- the handle of a HISAM database.
 *          key     -- key of primary key
 *
 * OUTPUT:  del_rec -- A copy of the record which is deleted.
 * RETUEN:  TRUE    -- The record has been deleted sucessfully.
 *          FALSE   -- Failed to delete the record.
 */
BOOL_T L_HISAM_DeleteRecordAndOutput (L_HISAM_Desc_T *desc, UI8_T *key, void* del_rec)
{
    return L_HISAM_InternalDeleteRecord(desc, key, del_rec);
}

/* PURPOSE: Internal function for deleting a specified record
 * INPUT:   desc    -- the handle of a HISAM database.
 *          key     -- key of primary key
 * OUTPUT:  none
 * RETUEN:  TRUE    -- The record has been deleted sucessfully.
...
static BOOL_T L_HISAM_InternalDeleteRecord (L_HISAM_Desc_T *desc, UI8_T *key, UI8_T *del_rec)
{
   ...
   if(del_rec)
      GET_RECORD(del_rec, record_index);
 ...
 * RETUEN:  none
 */
BOOL_T L_HISAM_GetNextRecord (L_HISAM_Desc_T *desc, UI32_T kidx, UI8_T *key, void *rec)
{
 ...
 * RETUEN:  TRUE/FALSE
 */
BOOL_T L_HISAM_GetRecord (L_HISAM_Desc_T *desc, UI32_T kidx, UI8_T *key,  void *rec)
{
   UI32_T iidx, pre_record_index;
   UI32_T record_index;
   Ipart_T *ipart = IPART(kidx);
...
UI32_T L_HISAM_Search (L_HISAM_Desc_T *desc, UI32_T kidx, UI8_T *key,
                     UI32_T (*call_back) (void*rec), UI32_T count)
{
   return  __L_HISAM_Search_WithCookie (desc, kidx, key, (UI32_T (*)(void*, void *)) call_back, count, 0, FALSE);
} /* L_HISAM_Search */
...
UI32_T __L_HISAM_Search_WithCookie (
   ...
   UI32_T (*call_back) (void*rec, void *cookie),UI32_T count, void *cookie, BOOL_T get_next)
{
   Ipart_T  *ipart = IPART(kidx);
   UI32_T iidx, pre_record_index;
   UI32_T record_index;
 ...
 *             the existed static record.
 */
UI32_T L_HISAM_ShMem_SetRecord (L_HISAM_ShMem_Desc_T *desc, void *rec, BOOL_T is_static)
{
   /* LOCAL VARIABLES
    */
 ...
 * RETUEN:  none
 */
BOOL_T L_HISAM_ShMem_GetNextRecord (L_HISAM_ShMem_Desc_T *desc, UI32_T kidx, UI8_T *key, void *rec)
{
   ShMem_Work_T   *wk = L_HISAM_SHMEM_WK(desc);
   UI8_T  *record_table = L_HISAM_SHMEM_RECORD_TABLE(wk);
   UI32_T   iidx, pre_record_index;
 ...
 * RETUEN:  TRUE/FALSE
 */
BOOL_T L_HISAM_ShMem_GetRecord (L_HISAM_ShMem_Desc_T *desc, UI32_T kidx, UI8_T *key,  void *rec)
{
   ShMem_Work_T   *wk = L_HISAM_SHMEM_WK(desc);
   UI8_T  *record_table = L_HISAM_SHMEM_RECORD_TABLE(wk);
   UI32_T iidx, pre_record_index;
...
UI32_T L_HISAM_ShMem_Search (L_HISAM_ShMem_Desc_T *desc, UI32_T kidx, UI8_T *key,
                     UI32_T (*call_back) (void*rec), UI32_T count)
{
   return  L_HISAM_ShMem_Search_WithCookie (desc, kidx, key, (UI32_T (*)(void*, void *)) call_back, count, 0);
      ...
      while ( Abs(ip->nbr,nn) > desc->N1 )
      {
         if ( ip->nbr < nn )
         {
            ...
            {  /* move some record from next list */
               ...
               ip->ridx_last = (Ridx_T)idx;
               ip[1].ridx_1st = ipart->rlink_table[idx];
               ip->nbr = (Ridx_T)nn;
               ip[1].nbr -= (Ridx_T)move_no;
               break;
            }
         } /* END if ( ip->nbr < nn )  */
         else  /* ( ip->nbr > nn ) */
         {  /* split list */
            ...
            if ( ip[1].ridx_last == NULL_INDEX )
            {
               ...
               ipart->curr_index_nbr = (Ridx_T)(iidx +2);
            }

            ip[1].nbr += (ip->nbr-(Ridx_T)nn);
            ip->nbr = (Ridx_T)nn;
            ip->ridx_last = (Ridx_T)idx;
            break;
         } /* END (ip->nbr>nn ) */
...
static void L_HISAM_DeleteIndex (L_HISAM_Desc_T *desc, Ipart_T *ipart, UI8_T *rec, UI32_T record_index)
{
   ...
   {  /* del 1st */
      ...
      ip->ridx_last = (Ridx_T) pre_record_index;
      /* END if */
...
static BOOL_T L_HISAM_Hash_Locate(L_HISAM_Desc_T *desc, Ipart_T  *ipart, UI32_T *ret_hidx)
{
   ...
   for ( i=0; i< desc->hash_depth;  i++ )
   {
      if ( (ridx=ipart->hash_table[hidx]) != NULL_INDEX )
      {
         ...
         if (L_HISAM_KEY_COMPARE(ipart) == 0 )
         {
            *ret_hidx = hidx;
            return TRUE;
         }
...
static void    L_HISAM_Hash_Insert (L_HISAM_Desc_T *desc, Ipart_T *ipart, UI32_T insert_ridx)
{
   ...
   if ( empty_hidx != NULL_INDEX )
      ipart->hash_table[empty_hidx] = (Ridx_T)insert_ridx;
} /* L_HISAM_Hash_Insert */
...
static void L_HISAM_InsertIndex (L_HISAM_Desc_T *desc, Ipart_T *ipart, UI8_T *rec, UI32_T record_index)
{
   ...
   {  /* insert at 1st */
      ...
      ip->ridx_1st = (Ridx_T)record_index;
      if ( ip->nbr == 0 )
         ip->ridx_last = (Ridx_T)record_index;
   } /* END if */
   else
   {
            ...
            == NULL_INDEX)  /* Lewis: check whether to insert after the last valid node in index table */
      {
         if ( (ip->nbr >= desc->N2) &&  (iidx < (desc->total_index_nbr-1)) )
         {
            ...
            ip->ridx_last = ip->ridx_1st = (Ridx_T)record_index;
         }
         else
         {
            ip->ridx_last = (Ridx_T)record_index;
      ...
      ipart->rlink_table [pre_record_index]= (Ridx_T)record_index;
   }
   if ( ++ip->nbr > desc->N2 )
   {
                    ...
                    UI32_T *record_index, UI32_T *pre_record_index)
{
   ...
   if ((ret = L_HISAM_KEY_COMPARE(ipart)) <=0 )
...
static int __KeyCompare(
    char                    *in_key_p,
    char                    *work_key_p,
    L_HISAM_KeyDef_T        *key_def_p,
    L_HISAM_KeyType_T       *key_type_p)
{
    char *p1, *p2;
    int  key_len, i, ret;
    ...
    for (i=0; i< key_def_p->field_number; i++)
    {
        key_len = key_def_p->length[i];
        switch (key_type_p[i])
        {
        ...
        case L_HISAM_4BYTE_INTEGER:
            ...
            ret = v1.l - v2.l;
            break;
        case L_HISAM_INET_ADDR_IP:
        {
            const L_INET_AddrIp_T *addr1 = (const L_INET_AddrIp_T *)p1;
            const L_INET_AddrIp_T *addr2 = (const L_INET_AddrIp_T *)p2;
            if (addr1->type != addr2->type)
            {
                ret = addr1->type - addr2->type;
                break;
            }
            if (addr1->addrlen != addr2->addrlen)
            {
                ret = addr1->addrlen - addr2->addrlen;
                break;
            }
            if (0 != (ret = memcmp(addr1->addr, addr2->addr, addr1->addrlen)))
            {
                break;
            }
            if (addr1->zoneid != addr2->zoneid)
            {
                ret = addr1->zoneid - addr2->zoneid;
                break;
            }
            if (addr1->preflen != addr2->preflen)
            {
                ret = addr1->preflen - addr2->preflen;
                break;
            }
            break;
        }
...
static void    L_HISAM_ShMem_Hash_Insert (L_HISAM_ShMem_Desc_T *desc, ShMem_Ipart_T *ipart, UI32_T insert_ridx)
{
   ...
   if ( empty_hidx != NULL_INDEX )
      L_HISAM_SHMEM_HASH_TABLE(ipart)[empty_hidx] = (Ridx_T)insert_ridx;
...
static void L_HISAM_ShMem_InsertIndex (L_HISAM_ShMem_Desc_T *desc, ShMem_Ipart_T *ipart, UI8_T *rec, UI32_T record_index)
{
   ...
   else
   {
      ...
      L_HISAM_SHMEM_RLINK_TABLE(ipart)[pre_record_index]= (Ridx_T)record_index;
   }
   if ( ++ip->nbr > desc->N2 )
   {
