...
extern int SWDRVL3_CompareTunnelNetRouteByIfindexAddr(void *elm1, void *elm2);
#endif
 ...
 *---------------------------------
 */
static L_SORT_LST_ShMem_CompareFunc_T shmem_compare_func[L_SORT_LST_SHMEM_COMPARE_FUNC_ID_MAX] =
{
#if (SYS_CPNT_SWDRVL3_CACHE == TRUE)
...
#if (SYS_CPNT_IP_TUNNEL == TRUE)
    SWDRVL3_CompareTunnelNetRouteByIfindexAddr, /* L_SORT_LST_SHMEM_COMPARE_FUNC_ID_SWDRVL3_TUNNEL_NET_ROUTE */
#endif

    /* for new COMPARE_FUNC_ID,
     * NEVER add compare func here,
     * please use L_SORT_LST_ShMem_SetCompareFunc instead.
     */
};
 ...
 *          cookie   -- User provided data to pass to a call back
 * OUTPUT:  none
...
L_SORT_LST_List_T *list, void *element, UI32_T count, UI32_T (*fun)(const void *element, void *cookie), void *cookie)
{
   Slist_T  *pre_node;
   Slist_T  *node;
   ...
   {
      if ( (*fun) (node->element, cookie) == L_SORT_LST_SEARCH_BREAK )
 ...
 *---------------------------------
 */
/* PURPOSE: L_SORT_LST_ShMem_SetCompareFunc
 * INPUT:   compare_func_id -- the type of compare function to use
 *          compare         -- compare func
 * OUTPUT:  none
 * RETURN:  TRUE/FALSE
 * NOTE:    CSC need to set compare func in its AttachSystemResources()
 *          that called by SYSRSC.
 */
BOOL_T L_SORT_LST_ShMem_SetCompareFunc (
    UI32_T compare_func_id,
    int  (*compare)(void*inlist_element, void *input_element))
{
    if (compare_func_id >= L_SORT_LST_SHMEM_COMPARE_FUNC_ID_MAX)
    {
        return FALSE;
    }

    shmem_compare_func[compare_func_id] = compare;
...
L_SORT_LST_ShMem_List_T *list, void *element, UI32_T count, UI32_T (*fun)(const void *element, void *cookie), void *cookie)
{
   L_SORT_LST_ShMem_Slist_T  *pre_node;
   L_SORT_LST_ShMem_Slist_T  *node;
   ...
   {
      if ( (*fun) (L_SORT_LST_SHMEM_GET_USERDATA(node), cookie) == L_SORT_LST_SEARCH_BREAK )
         return L_SORT_LST_SEARCH_BREAK;
   }
   return (node==0)? L_SORT_LST_SEARCH_END_OF_LIST:L_SORT_LST_SEARCH_COMPLETED;
