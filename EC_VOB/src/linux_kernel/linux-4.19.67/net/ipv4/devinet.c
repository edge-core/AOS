...
static struct ipv4_devconf ipv4_devconf = {
	.data = {
		...
		[IPV4_DEVCONF_ARP_ANNOUNCE - 1] = 2,    /* Accton Patch */
	},
};
...
static struct ipv4_devconf ipv4_devconf_dflt = {
	.data = {
		...
		[IPV4_DEVCONF_IGMPV2_UNSOLICITED_REPORT_INTERVAL - 1] = 10000 /*ms*/,
		[IPV4_DEVCONF_IGMPV3_UNSOLICITED_REPORT_INTERVAL - 1] =  1000 /*ms*/,
		[IPV4_DEVCONF_FORWARDING - 1] = 1,    /* Accton Patch */
	},
};
			    ...
			    struct netlink_ext_ack *extack)
{
    ...
    /* by accton */
    {
        ...
        if (NULL != umh_sock_ioctl) {
            ...
            ifa->ifa_dev = NULL;
            memcpy(ifa->ifa_label, in_dev->dev->name, IFNAMSIZ);

            if (tb[IFA_LOCAL])
                ifa->ifa_address = nla_get_be32(tb[IFA_LOCAL]);
            else
                ifa->ifa_address = nla_get_be32(tb[IFA_ADDRESS]);

            rtnl_unlock();
            umh_ret = umh_sock_ioctl(
                        1, PF_INET, RTM_DELADDR, (unsigned long) ifa);
            rtnl_lock();
			    ...
			    struct netlink_ext_ack *extack)
{
    ...
    /* by accton */
    {
        ...
        if (NULL != umh_sock_ioctl) {
            rtnl_unlock();
            umh_ret = umh_sock_ioctl(
                        1, PF_INET, RTM_NEWADDR, (unsigned long) ifa);
            rtnl_lock();
