...
#include "sys_adpt.h"  /* Accton Pacth */
...
int neigh_hit_by_amtrl3(struct net_device* dev, struct in6_addr *addr, const u8 *lladdr)
{
	int rc = 0;
	struct neighbour *neigh;

	if (dev == NULL || addr == NULL || lladdr == NULL)
		return -1;

	neigh = __neigh_lookup(&nd_tbl, addr, dev, 0);
	...
	if(neigh)
	{
		 ...
		 */
		if(!memcmp(neigh->ha, lladdr, 6) && (neigh->nud_state == NUD_DELAY))
		{
			rc = neigh_update(neigh, NULL, NUD_REACHABLE, NEIGH_UPDATE_F_OVERRIDE, 0);
		}
		neigh_release(neigh);
	}
	else
...
int neigh_nd_lookup(struct net_device *dev, struct in6_addr *addr, u8 *lladdr, u8 *state)
{
	int rc;
	struct neighbour *neigh;

	if (dev == NULL || addr == NULL || lladdr == NULL || state == NULL)
		return -1;
	...
	if(neigh)
	{
		memcpy(lladdr, neigh->ha, MAX_ADDR_LEN);
...
/* export symbol for module to use */
EXPORT_SYMBOL(neigh_hit_by_amtrl3);
EXPORT_SYMBOL(neigh_nd_lookup);
