...
static int addrconf_fixup_autoconf(struct ctl_table *table, int *p, int newf)
{
	struct net *net;
	int old;
	...
	*p = newf;
	...
	if(newf && ((!newf) ^ (!old)))
	{
		struct inet6_dev *in6_dev = (struct inet6_dev *)table->extra1;
		 ...
		 */
		if (in6_dev->cnf.forwarding != 0
		  ||in6_dev->cnf.rtr_solicits == 0
		  ||(in6_dev->dev->flags&IFF_LOOPBACK) != 0) {
			rtnl_unlock();
			return 0;
		}

		/* check link-local address tentative status
		 */
		...
		if (send_rs)
		{
			ndisc_send_rs(in6_dev->dev, &ifp->addr, &in6addr_linklocal_allrouters);
			  ...
			  struct netlink_ext_ack *extack)
{
	...
	/* The following code is added to fix an issue of DAD configuration.
	 * It is cooperate with the addrconf_ifdown(idev->dev, 0) in inet6_addr_del().
	 * Please refer to the comment above addrconf_ifdown(idev->dev, 0).
	 */
	...
	list_for_each_entry(ifp, &idev->addr_list, if_list) {
		if (ifp->prefix_len == plen &&
			...
			if (list_empty(&idev->addr_list)) {
				/* We change second argument of addrconf_ifdown() from 1 to 0, because if
				 * how=1, idev will be destroyed. We need idev to keep some configurations
				 * especially dad_transmits. If not, when first IPv6 address is added after
				 * idev destroyed, a new idev will be created and DAD is performed using
				 * dad_transmits with default value 1. Core layer will re-configure all
				 * IPv6 settings into kernel when RIF up, but this is too late for
				 * dad_transmits since DAD is performed right after the idev is created.
				 * This modification cooperate with the following code added in
				 * inet6_addr_add() (because addrconf_ifdown() will turn off IF_READY)
				 *     if (!(idev->if_flags & IF_READY)){
				 *         if (netif_carrier_ok(dev) && (dev->flags & IFF_UP))
				 *             idev->if_flags |= IF_READY;
				 *     }
		     ...
		     int plen, int scope)
{
	...
	/* we add link-local address from netcfg_mgr_ip
	 */
#if 0
...
{
	/* We add route from netcfg_mgr_route
	 */
#if 0
	struct inet6_dev *idev;
		  ...
		  struct netlink_ext_ack *extack)
{
	...
	/* by accton */
	{
		...
		int umh_ret;
		struct inet6_ifaddr *ifa_acc_p;

		if (NULL != umh_sock_ioctl) {
			ifa_acc_p = kzalloc(sizeof(struct inet6_ifaddr), GFP_ATOMIC);

			if (ifa_acc_p == NULL) {
				return -ENOBUFS;
			}

			memcpy(&ifa_acc_p->addr, pfx, sizeof(*pfx));
			ifa_acc_p->prefix_len = ifm->ifa_prefixlen;
			ifa_acc_p->cstamp = ifm->ifa_index;

			rtnl_unlock();
			umh_ret = umh_sock_ioctl(
					1, PF_INET6, RTM_DELADDR, (unsigned long) ifa_acc_p);
			rtnl_lock();
		  ...
		  struct netlink_ext_ack *extack)
{
	...
	/* by accton */
	{
		...
		if (NULL != umh_sock_ioctl) {
			...
			memcpy(&ifa_acc_p->addr, cfg.pfx, sizeof(*cfg.pfx));
			ifa_acc_p->prefix_len = ifm->ifa_prefixlen;
			ifa_acc_p->cstamp = ifm->ifa_index;

			rtnl_unlock();
			umh_ret = umh_sock_ioctl(
					1, PF_INET6, RTM_NEWADDR, (unsigned long) ifa_acc_p);
			rtnl_lock();
...
static
int addrconf_sysctl_autoconf(struct ctl_table *ctl, int write,
			   void __user *buffer, size_t *lenp, loff_t *ppos)
{
	int *valp = ctl->data;
	int val = *valp;
	...
	/*
	 * ctl->data points to idev->cnf.autoconf, we should
	 * not modify it until we get the rtnl lock.
	 */
	lctl = *ctl;
	lctl.data = &val;
	...
	if (write)
		ret = addrconf_fixup_autoconf(ctl, valp, val);
	if (ret)
		*ppos = pos;
	return ret;
}
